...
#    - Processes variables starting with DOCLATTICE_
#    - Removes the DOCLATTICE_ prefix from the variable name
...
if [ -f ./.env ]; then
    ...
    # ==================================
    #
    # Read the .env file and process each line, stripping DOCLATTICE_ prefix if present
    ...
    while IFS='=' read -r key value; do
        ...
        # Remove DOCLATTICE_ prefix for keys if present
        cleaned_key=${key#DOCLATTICE_}
    ...
    # - Filter for variables starting with DOCLATTICE_
    # - For each matching variable:
    #   - Remove the DOCLATTICE_ prefix
    #   - Add the cleaned key and its value to env-config.js
    env | grep '^DOCLATTICE_' | while IFS='=' read -r key value; do
        # Remove DOCLATTICE_ prefix
        cleaned_key=${key#DOCLATTICE_}
