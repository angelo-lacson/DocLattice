...
const mockComponents = {
  parsers: [
    ...
    },
    {
      ...
      className: "doclatticeserver.pipeline.parsers.llamaparse.LlamaParser",
      supportedFileTypes: ["PDF"],
      enabled: true,
      settingsSchema: [
        {
  ...
  llmProviders: [
    {
      ...
      className:
        "doclatticeserver.pipeline.llm_providers.anthropic_provider.AnthropicProvider",
  ...
  fileConverters: [
    {
      ...
      className:
        "doclatticeserver.pipeline.file_converters.gotenberg_converter.GotenbergFileConverter",
  ...
  rerankers: [
    {
      ...
      className:
        "doclatticeserver.pipeline.rerankers.cross_encoder_reranker.CrossEncoderReranker",
      enabled: true,
      settingsSchema: [],
    },
  ],
  enrichers: [
    {
      ...
      className:
        "doclatticeserver.pipeline.enrichers.pdf_outline_enricher.PdfOutlineEnricher",
      supportedFileTypes: ["PDF"],
      enabled: true,
      settingsSchema: [],
    },
    {
      ...
      className:
        "doclatticeserver.pipeline.enrichers.metadata.MetadataEnricher",
      supportedFileTypes: ["PDF"],
      enabled: true,
      settingsSchema: [],
    },
...
test.describe("SystemSettings — default embedder modal", () => {
  ...
  test("typing a custom class path directly into the input also works", async ({
    ...
    const customPath = "doclatticeserver.custom.CustomEmbedder";
    const saveMock = {
      request: {
        query: UPDATE_PIPELINE_SETTINGS,
...
test.describe("SystemSettings — advanced config (non-secret) save", () => {
  test("shows Save Configuration after editing a config field and persists it", async ({
    ...
    const workersInput = page.locator(
      "#config-library-doclatticeserver\\.pipeline\\.parsers\\.llamaparse\\.LlamaParser-num_workers"
    );
  ...
  test("changing a bool select dropdown marks the form dirty", async ({
    ...
    const verboseSelect = page.locator(
      "select#config-library-doclatticeserver\\.pipeline\\.parsers\\.llamaparse\\.LlamaParser-verbose"
    );
...
test.describe("SystemSettings — enable/disable transitions", () => {
  test("unchecking a component when all-enabled builds explicit enabled list", async ({
    ...
    const expectedEnabled = allPaths.filter(
      (p) => p !== "doclatticeserver.pipeline.parsers.docling.DoclingParser"
    );
