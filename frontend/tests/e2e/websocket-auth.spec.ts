 ...
 * unit/integration coverage in `doclatticeserver/tests/test_websocket_auth.py`
  ...
  test("agent ws: authenticated query streams AUTH_OK then ASYNC_FINISH (VCR)", async ({
    ...
    test.skip(
      ...
      "Requires OC_LLM_VCR_MODE=replay or record + cassette under " +
        "doclatticeserver/tests/fixtures/cassettes/e2e_websocket_auth/"
    );
    ...
    const result = await page.evaluate(
      async ([wsUrl, token]) => {
        return await new Promise<{
          ...
          const ws = new WebSocket(wsUrl as string, ["doclattice.jwt.v1"]);
          ws.onmessage = (ev) => {
            try {
