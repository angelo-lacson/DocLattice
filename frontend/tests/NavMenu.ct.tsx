...
test.describe("NavMenu Component", () => {
  ...
  test.describe("Branding", () => {
    // Branding assertions updated for the [DocLattice] rebrand: the
    // wordmark is now `[DocLattice]` (brackets preserved per brand spec)
    // and the logo is an inline SVG icon mark rather than a PNG.
    test("should display DocLattice brand name", async ({ mount, page }) => {
      const component = await mount(<NavMenuTestWrapper />);

      await expect(page.locator("text=[DocLattice]")).toBeVisible({
        timeout: 5000,
      });
...
test.describe("NavMenu Overflow", () => {
  test.describe("Desktop", () => {
    ...
    test("should open the overflow menu when triggered (anonymous)", async ({
      ...
      await expect(
        menu.getByRole("menuitem", { name: "About DocLattice" })
      ).toBeVisible();
      await expect(
  ...
  test.describe("Mobile", () => {
    test("should render the 'More' section with essential links (anonymous)", async ({
      ...
      await expect(
        sheet.getByRole("link", { name: "About DocLattice" })
      ).toBeVisible();
      await expect(
