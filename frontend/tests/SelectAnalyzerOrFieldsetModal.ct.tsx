...
const ANALYZERS_WITH_SCHEMA = [
  makeAnalyzer({
    ...
    taskName: "doclatticeserver.tasks.analyzer_tasks.simple_ner",
    isPublic: true,
  }),
  makeAnalyzer({
    id: "a-configurable",
    ...
    taskName: "doclatticeserver.tasks.analyzer_tasks.configurable_summarizer",
    isPublic: false,
    inputSchema: {
      type: "object",
      properties: {
...
const MANY_ANALYZERS = Array.from({ length: 12 }, (_, i) =>
  makeAnalyzer({
    ...
    taskName: `doclatticeserver.tasks.analyzer_tasks.analyzer_${i}`,
  })
);
