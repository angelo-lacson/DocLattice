...
describe("websocketAuth helpers", () => {
  it("uses doclattice.jwt.v1 as the subprotocol marker", () => {
    expect(WS_AUTH_SUBPROTOCOL).toBe("doclattice.jwt.v1");
  });
  ...
  it("builds protocols array with marker only when no token", () => {
    expect(buildAuthProtocols(undefined)).toEqual(["doclattice.jwt.v1"]);
    expect(buildAuthProtocols(null)).toEqual(["doclattice.jwt.v1"]);
    expect(buildAuthProtocols("")).toEqual(["doclattice.jwt.v1"]);
  });
