...
// (doclatticeserver/types/enums.py); values are the display labels.
...
// (doclatticeserver/research/constants.py MAX_RESEARCH_PROMPT_CHARS) so the
...
// (doclatticeserver/research/models.py ResearchReport.title max_length=255)
...
// Keep in sync with doclatticeserver/constants/annotations.py
...
// doclatticeserver/constants/annotations.py.
...
// (``doclatticeserver/constants/annotations.py``) so the placeholder used
...
// sync with doclatticeserver/enrichment/constants.py (LABEL_REF_*). These
...
// pipeline (``doclatticeserver/utils/geocoding``) and store the resolved
...
// doclatticeserver/annotations/services/geographic_service.py).
 ...
 * `doclatticeserver.constants.web_search.WEB_SEARCH_SETTINGS_KEY` on the
 ...
 * `doclatticeserver.constants.web_search.SUPPORTED_PROVIDERS` — update here
 ...
 * ``doclatticeserver/constants/extracts.py`` at the same time.
...
// defaults (doclatticeserver/constants/corpus_categories.py) so a category
 ...
 * in doclatticeserver/constants/corpus_categories.py so the textarea stops
...
// ``doclatticeserver/constants/auth.py`` so the frontend redacted
...
// ``doclatticeserver/documents/models.py``. Used by the document graph to
...
// must match doclatticeserver/enrichment/constants.py GRAPH_* values.
...
// Must match doclatticeserver/enrichment/constants.py ENRICHMENT_ANALYZER_TASK.
export const ENRICHMENT_ANALYZER_TASK_NAME =
  "doclatticeserver.tasks.corpus_analysis_tasks.corpus_reference_enrichment";
