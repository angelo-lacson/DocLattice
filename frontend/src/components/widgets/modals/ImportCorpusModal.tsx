 ...
 *
 * The import uses the importDocLatticeZip mutation, which:
...
export const ImportCorpusModal: React.FC = () => {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (result.data?.importDocLatticeZip?.ok) {
        setUploadProgress(100);
        toast.success("SUCCESS! Corpus file upload and import has started.");
        setTimeout(() => handleClose(), 1500);
      } else {
        setError(
          result.data?.importDocLatticeZip?.message ||
    ...
    <div>
      <AlertBox $variant="warning">
        ...
        <AlertBody>
          ...
          <p>
            This will unpack the DocLattice export ZIP into a brand-new
