    ...
    if (openedCorpus) {
      ...
      if (
        ...
        createAnnotation({
          ...
          .catch((err) => {
            toast.error(
              `Sorry, something went wrong!\nUnable to add that annotation via DocLattice GraphQL endpoint: ${err}`
            );
            return null;
          });
      }
  ...
  const requestCreateRelation = (relation: RelationGroup): void => {
    if (openedCorpus) {
      createRelation({
        ...
        .catch((err) => {
          toast.error(
            "Sorry, something went wrong!\nUnable to add that relationship via DocLattice GraphQL endpoint."
          );
        });
    } else {
      toast.warning("No corpus selected");
  ...
  const requestCreateDocTypeAnnotation = (
    ...
    if (openedCorpus) {
      createDocTypeAnnotation({
        ...
        .catch((err) => {
          toast.error(
            "Sorry, something went wrong!\nUnable to add that document label via DocLattice GraphQL endpoint."
          );
        });
    } else {
      toast.warning("No corpus selected");
