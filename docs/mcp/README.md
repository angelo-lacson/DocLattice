# DocLattice MCP Server

## TL;DR

DocLattice exposes a read-only [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server for AI assistants to access **public** corpuses, documents, annotations, and discussion threads.
...
**Key files**:
- `doclatticeserver/mcp/server.py` - Server setup, ASGI app, URI parsing, transport handlers
- `doclatticeserver/mcp/tools.py` - Tool implementations
- `doclatticeserver/mcp/resources.py` - Resource handlers
