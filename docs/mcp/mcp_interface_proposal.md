...
def list_threads(
    ...
    from doclatticeserver.conversations.models import Conversation, ConversationTypeChoices
    from doclatticeserver.corpuses.models import Corpus
    from doclatticeserver.documents.models import Document
...
```python
# doclatticeserver/mcp/server.py
import asyncio
import json
import re
...
class URIParser:
    ...
    # Slug pattern: alphanumeric and hyphens only (matches DocLattice slug format)
...
def validate_slug(slug: str) -> bool:
    """Validate slug format matches DocLattice pattern"""
...
```python
# doclatticeserver/mcp/tests/test_resources.py
...
```python
# doclatticeserver/mcp/tests/test_integration.py
import pytest
...
async def test_full_corpus_exploration():
    ...
    async with ClientSession("doclattice-mcp") as session:
...
python -m doclatticeserver.mcp.server

# Or via Docker
docker run -p 3000:3000 doclattice-mcp
```
...
```json
{
  "mcpServers": {
    "doclattice": {
      "command": "python",
      "args": ["-m", "doclatticeserver.mcp.server"],
      "env": {
...
✅ **Read-only access** to public DocLattice resources
...
The implementation follows DocLattice' established patterns and leverages the existing permissioning infrastructure for a robust, maintainable solution.
