...
The worker upload system provides a REST API for external pipelines, bulk ingestion scripts, and pre-processed document feeds to push documents, annotations, and embeddings into DocLattice. It is designed for machine-to-machine workflows where documents have already been parsed and annotated outside of DocLattice' built-in pipeline.
...
- A running DocLattice instance with the Celery worker processing the `worker_uploads` queue.
...
Pre-computed embeddings allow you to skip the embedding step in DocLattice. The `embeddings` object has this structure:
...
Worker accounts are Django users with unusable passwords. They cannot log in via the web UI, GraphQL, or any other authentication method. They exist solely for permission compatibility with DocLattice' guardian-based permission system.
