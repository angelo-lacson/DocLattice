# DocLattice vs LegalBench-RAG — Retrieval Benchmark Report
...
> paper**: DocLattice (multi-qa-MiniLM-L6 + paragraph chunker + no reranker)
...
DocLattice has two layers stacked on top of retrieval: an **agent loop**
...
When this report says "DocLattice beats the paper by +9.2 pts," it always
...
`file_path` equality check). Implemented in `doclatticeserver/benchmarks/metrics.py`
...
- **Per-document**: probe scoped to the task's target document
  (`probe_retrieval(corpus_id=..., document_id=...)`). Matches DocLattice'
...
DocLattice' production agent loop runs in per-document mode: each Datacell
...
- **`doclatticeserver/benchmarks/metrics.py`**: added `char_recall`,
  `char_precision`, `char_f1`, plus cross-doc-aware variants for corpus-wide
  scoring.
- **`doclatticeserver/benchmarks/report.py`**: added per-task LLM token
  ...
  spans.
- **`doclatticeserver/benchmarks/runner.py`**: added `retrieval_only=` mode
  (skip agent extract pass) and `corpus_wide=` mode (drop document filter on
  probe).
- **`doclatticeserver/benchmarks/retrieval.py`**: added `corpus_wide=` flag and
  ...
  annotations).
- **`doclatticeserver/benchmarks/management/commands/run_benchmark.py`**: added
  `--retrieval-only` and `--corpus-wide` CLI flags.
- **`doclatticeserver/pipeline/embedders/openai_embedder.py`**: input
...
- **`doclatticeserver/tests/test_benchmarks.py`**: 18 unit tests covering the
...
ps.default_embedder = 'doclatticeserver.pipeline.embedders.sent_transformer_microservice.MicroserviceEmbedder'
ps.parser_kwargs = {'doclatticeserver.pipeline.parsers.oc_text_parser.TxtParser': {'chunkers': [{'name': 'paragraph'}]}}
ps.default_reranker = ''
ps.save()
"
...
> **Scope note**: The numbers in this section measure the *full DocLattice
...
> production DocLattice actually delivers; treat the comparisons here as
> *intra-DocLattice* (probe vs agent, model A vs model B), not vs LB-RAG.

To exercise the production DocLattice pipeline (LLM extracts an answer +
...
- Comparing to the LegalBench-RAG paper or any other retrieval-only
  benchmark → **probe**.
- Reporting what production DocLattice actually surfaces to a user →
  **citation**.
