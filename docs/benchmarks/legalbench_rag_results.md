...
> **The harness reproduces the paper.** DocLattice' Config B (Naive 500
...
1. **Annotation contamination**: a partial DB wipe between runs (Corpus +
   ...
   `doclatticeserver/documents/signals.py` and an idempotent
...
free-tier DocLattice default look like" data point, not a paper
comparison.
...
config across all DocLattice configs. Config B (sliding-500) gets
...
DocLattice Config B is the closest match (sliding-window 500,
...
A complete equivalence between DocLattice' formula and upstream's
...
DocLattice' `char_recall_paper` / `char_precision_paper` in
`doclatticeserver/benchmarks/metrics.py` produce numerically identical
...
3-large @ k=32 (0.5427), where the DocLattice harness is +16 pts
...
> **Scope**: This section measures the full DocLattice production
...
> DocLattice actually surfaces to a user.
