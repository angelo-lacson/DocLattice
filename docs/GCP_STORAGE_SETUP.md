# Google Cloud Storage Setup for DocLattice

*Last Updated: 2026-01-09*

This document provides instructions for configuring Google Cloud Storage (GCS) as the storage backend for DocLattice.
...
DocLattice supports three storage backends, configurable via the `STORAGE_BACKEND` environment variable:
...
```bash
gcloud iam service-accounts create doclattice-storage \
    --display-name="DocLattice Storage Service Account"
```
...
gcloud projects add-iam-policy-binding YOUR-PROJECT-ID \
    --member="serviceAccount:doclattice-storage@YOUR-PROJECT-ID.iam.gserviceaccount.com" \
...
kubectl annotate serviceaccount YOUR-KSA \
    iam.gke.io/gcp-service-account=doclattice-storage@YOUR-PROJECT-ID.iam.gserviceaccount.com

# For Cloud Run
gcloud run services update YOUR-SERVICE \
    --service-account=doclattice-storage@YOUR-PROJECT-ID.iam.gserviceaccount.com
```
...
```bash
gcloud iam service-accounts keys create ~/doclattice-gcs-key.json \
    --iam-account=doclattice-storage@YOUR-PROJECT-ID.iam.gserviceaccount.com
```

2. Set the environment variable:
```bash
export GOOGLE_APPLICATION_CREDENTIALS=~/doclattice-gcs-key.json
```
...
GS_SA_EMAIL=doclattice-storage@YOUR-PROJECT-ID.iam.gserviceaccount.com  # Service account email for signing
```
...
```json
{
  "title": "DocLattice Storage Role",
  "description": "Minimal permissions for DocLattice GCS storage",
  "stage": "GA",
  "includedPermissions": [
    "storage.buckets.get",
...
Apply the role:
```bash
gcloud iam roles create doclatticeStorage \
    --project=YOUR-PROJECT-ID \
    --file=role-definition.json

gcloud projects add-iam-policy-binding YOUR-PROJECT-ID \
    --member="serviceAccount:doclattice-storage@YOUR-PROJECT-ID.iam.gserviceaccount.com" \
    --role="projects/YOUR-PROJECT-ID/roles/doclatticeStorage"
```
