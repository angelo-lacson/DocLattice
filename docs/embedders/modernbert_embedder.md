...
- [`doclatticeserver/pipeline/embedders/modern_bert_embedder.py`](../../doclatticeserver/pipeline/embedders/modern_bert_embedder.py)
...
```python
from doclatticeserver.pipeline.embedders.modern_bert_embedder import ModernBERTEmbedder
...
```
doclatticeserver.pipeline.embedders.modern_bert_embedder.ModernBERTEmbedder
```

By default, DocLattice uses the `MicroserviceEmbedder` for production deployments. To use the ModernBERT embedder directly, update the `PREFERRED_EMBEDDERS` or `DEFAULT_EMBEDDERS_BY_FILETYPE` settings.
