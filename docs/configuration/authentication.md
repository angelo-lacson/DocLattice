...
DocLattice supports two authentication methods:
...
**Production** (Docker, `DOCLATTICE_*` prefix):

```bash
DOCLATTICE_REACT_APP_USE_AUTH0=false
DOCLATTICE_REACT_APP_API_ROOT_URL=https://your-domain.com
```
...
3. Choose a tenant name (e.g., `doclattice-prod`) and a region (US, EU, or AU)
4. Your tenant domain will look like `doclattice-prod.us.auth0.com`
...
2. Configure:
    - **Name**: "DocLattice API" (display name, your choice)
...
3. Name it (e.g., "DocLattice Frontend")
...
3. Name it (e.g., "DocLattice Backend M2M")
...
# AUTH0_ADMIN_CLAIM_NAMESPACE=https://doclattice.opensource.legal/
```
...
```bash
DOCLATTICE_REACT_APP_USE_AUTH0=true
DOCLATTICE_REACT_APP_APPLICATION_DOMAIN=<your-tenant>.us.auth0.com
DOCLATTICE_REACT_APP_APPLICATION_CLIENT_ID=<your-spa-client-id>
DOCLATTICE_REACT_APP_AUDIENCE=https://contracts.your-domain.com
DOCLATTICE_REACT_APP_API_ROOT_URL=https://your-domain.com
```
...
| `AUTH0_ADMIN_CLAIM_NAMESPACE` | No | `https://doclattice.opensource.legal/` | Namespace prefix for admin claims in tokens |
...
|-----------------------|----------------------|----------|-------------|
| `DOCLATTICE_REACT_APP_USE_AUTH0` | `VITE_USE_AUTH0` | Yes | Enable Auth0 on frontend |
| `DOCLATTICE_REACT_APP_APPLICATION_DOMAIN` | `VITE_APPLICATION_DOMAIN` | If Auth0 | Auth0 tenant domain |
| `DOCLATTICE_REACT_APP_APPLICATION_CLIENT_ID` | `VITE_APPLICATION_CLIENT_ID` | If Auth0 | SPA client ID |
| `DOCLATTICE_REACT_APP_AUDIENCE` | `VITE_AUDIENCE` | If Auth0 | API audience |
| `DOCLATTICE_REACT_APP_API_ROOT_URL` | `VITE_API_ROOT_URL` | Yes | Backend URL |
...
3. **Wrong claim namespace**: The Action must use the namespace
   `https://doclattice.opensource.legal/` (with trailing slash) unless you've
