...
| Production (Docker/K8s) | `DOCLATTICE_` | `DOCLATTICE_REACT_APP_USE_AUTH0=true` |

**How it works**: In production, the container entrypoint script writes a runtime-loaded file at `frontend/public/env-config.js` that populates `window._env_` with the `REACT_APP_*` variables converted from `DOCLATTICE_REACT_APP_*`. For example, `DOCLATTICE_REACT_APP_USE_AUTH0` becomes `window._env_.REACT_APP_USE_AUTH0` in the frontend code. The browser fetches `env-config.js` on every page load, so configuration changes do **not** require a frontend rebuild — only a container restart (or `env-config.js` regeneration) is needed.

See [`frontend/env.sh`](https://github.com/angelo-lacson/DocLattice/blob/main/frontend/env.sh), the entrypoint script that generates it, for the file structure and the [Frontend Telemetry doc](../telemetry/Frontend.md) for how the same mechanism is used to inject PostHog keys.

## Available Configuration Options

| Setting | Local (VITE_) | Production (DOCLATTICE_REACT_APP_) | Description |
|---------|---------------|----------------------------------------|-------------|
| Auth0 toggle | `VITE_USE_AUTH0` | `DOCLATTICE_REACT_APP_USE_AUTH0` | Enable Auth0 authentication (default: false) |
| Auth0 domain | `VITE_APPLICATION_DOMAIN` | `DOCLATTICE_REACT_APP_APPLICATION_DOMAIN` | Auth0 domain (required if Auth0 enabled) |
| Auth0 client ID | `VITE_APPLICATION_CLIENT_ID` | `DOCLATTICE_REACT_APP_APPLICATION_CLIENT_ID` | Auth0 client ID (required if Auth0 enabled) |
| Auth0 audience | `VITE_AUDIENCE` | `DOCLATTICE_REACT_APP_AUDIENCE` | Auth0 API audience |
| API root URL | `VITE_API_ROOT_URL` | `DOCLATTICE_REACT_APP_API_ROOT_URL` | Backend API URL |
| Analyzers | `VITE_USE_ANALYZERS` | `DOCLATTICE_REACT_APP_USE_ANALYZERS` | Enable analyzer functionality |
| Imports | `VITE_ALLOW_IMPORTS` | `DOCLATTICE_REACT_APP_ALLOW_IMPORTS` | Enable corpus import from ZIP files |
...
For production, use `DOCLATTICE_*` prefixed variables. The container entrypoint converts these to runtime `REACT_APP_*` config.
...
services:
  frontend:
    environment:
      - DOCLATTICE_REACT_APP_USE_AUTH0=false
      - DOCLATTICE_REACT_APP_USE_ANALYZERS=true
```
