...
Our frontend config file should look like this (The `DOCLATTICE_` prefixes are necessary to get the env variables
injected into the frontend container. The env variable that shows up on `window._env_` in the React frontend will omit
the prefix, however - e.g. `DOCLATTICE_REACT_APP_APPLICATION_DOMAIN` will show up as `REACT_APP_APPLICATION_DOMAIN`):

```
DOCLATTICE_REACT_APP_APPLICATION_DOMAIN=
DOCLATTICE_REACT_APP_APPLICATION_CLIENT_ID=
DOCLATTICE_REACT_APP_AUDIENCE=http://localhost:3000
DOCLATTICE_REACT_APP_API_ROOT_URL=https://doclattice.opensource.legal
...
# DOCLATTICE_REACT_APP_ALLOW_IMPORTS=true
```
...
In production (Kubernetes/Docker), prefer `DOCLATTICE_*` keys in the environment which the container entrypoint converts to the runtime `REACT_APP_*` config. See k8s production README and `02-configmaps.yaml` for examples.

ATM, there are three key configurations:
1. **DOCLATTICE_REACT_APP_USE_AUTH0** - uncomment this / set it to true to switch the frontend login components and
   auth flow from django password auth to Auth0 oauth2. IF this is true, you also need to provide valid configurations
   for `DOCLATTICE_REACT_APP_APPLICATION_DOMAIN`, `DOCLATTICE_REACT_APP_APPLICATION_CLIENT_ID`, and
   `DOCLATTICE_REACT_APP_AUDIENCE`. These are configured
   on the Auth0 platform. We don't have a walkthrough for that ATM.
2. **DOCLATTICE_REACT_APP_USE_ANALYZERS** - allow users to see and use analyzers. False on the demo deployment.
3. **DOCLATTICE_REACT_APP_ALLOW_IMPORTS** - do not let people upload zip files and attempt to import them. Not
...
services:
  frontend:
    ...
    ports:
      - "3000:3000"
    environment:
      - DOCLATTICE_REACT_APP_APPLICATION_DOMAIN=yourdomain.com
      - DOCLATTICE_REACT_APP_APPLICATION_CLIENT_ID=your_client_id
      - DOCLATTICE_REACT_APP_AUDIENCE=http://localhost:3000
      - DOCLATTICE_REACT_APP_API_ROOT_URL=https://doclattice.opensource.legal
      - DOCLATTICE_REACT_APP_USE_AUTH0=true
      - DOCLATTICE_REACT_APP_USE_ANALYZERS=true
