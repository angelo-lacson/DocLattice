## DocLattice Admin Dashboard

DocLattice' backend is built on Django, which has its own powerful admin dashboard. This dashboard is not meant for end-users
and should only be used by admins. You can access the admin dashboard by going to the `/admin` page - e.g.,
`doclattice.opensource.legal/admin` or `http://localhost:8000/admin`. For the most part, you shouldn't need to use
...
By default, DocLattice creates an admin user for you. If you don't specify the username and password
...
To manage users, including changing the password, you'll need to access the backend admin dashboard. DocLattice
