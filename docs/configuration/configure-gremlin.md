...
readily be rendered in DocLattice).

DocLattice is designed to work with Gremlin out-of-the-box. We have a sample compose yaml file showing how to do
...
analyzers and labels. These will then be available within DocLattice, and you can use an analyzer to analyze
any DocLattice corpus.

While we have plans to automatically "install" the default Gremlin on first boot, currently **you must manually go
into the DocLattice admin dash and add the Gremlin.** Thankfully, this is an easy process:
...
1. In your environment file, make sure you set `CALLBACK_ROOT_URL_FOR_ANALYZER`
       ...
       2. For production deploy, use `http://django:5000`. Why the change? Well, in our local
          ...
          production, we want Gremlin to communicate with the DocLattice container ("django") via its
          hostname on the docker compose stack's network. The production DocLattice container also uses
