...
DocLattice supports three storage backends: **Local filesystem**, **Amazon S3 (AWS)**, and **Google Cloud Storage (GCP)**. You select the backend using the `STORAGE_BACKEND` environment variable.
...
| `S3_DOCUMENT_PATH` | `doclattice` | Path for document storage |
...
| `GCS_DOCUMENT_PATH` | `doclattice` | Path for document storage |
