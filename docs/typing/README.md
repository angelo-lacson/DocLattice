...
This directory documents how DocLattice' Python type-checking pipeline is
...
- **CI**: `.github/workflows/backend.yml` runs
  `python -m mypy --config-file mypy.ini doclatticeserver config` as part
...
docker compose -f test.yml run --rm django \
  python -m mypy --config-file mypy.ini doclatticeserver config
```
...
`doclatticeserver.*` or `config.*`.

That matters because:

- **New files ARE type-checked.** A brand-new module at
  `doclatticeserver/new_feature/views.py` is not in the baseline, so mypy
...
- **Existing baselined files are silenced.** Any error inside
  `doclatticeserver/utils/storages.py` (for example) is suppressed until
...
| Worst offender (file) | `doclatticeserver/mcp/tests/test_mcp.py` (319 errors) |
...
1. **Delete the module's section** in `mypy.ini`. Example — to graduate
   `doclatticeserver/constants/annotations.py`:
    ...
    ```ini
    # Before
    [mypy-doclatticeserver.constants.annotations]
    ignore_errors = True
...
4. Commit with a message referencing the tracker issue, e.g.
   `typing: graduate doclatticeserver.constants.annotations (refs #1331)`.
...
1. `doclatticeserver/constants/*` — pure Python constants, no imports.
2. `doclatticeserver/utils/*` helpers that don't touch models.
...
```ini
[mypy-doclatticeserver.legacy_thing]
ignore_errors = True
```
