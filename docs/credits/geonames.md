...
The offline geocoding utility in `doclatticeserver/utils/geocoding/`
...
3. Place the regenerated file at
   `doclatticeserver/utils/geocoding/data/cities.json` and bump
   `_meta.schema_version` if you change the field set.
4. Run the geocoding tests:

   ```bash
   docker compose -f test.yml run django pytest \
     doclatticeserver/tests/test_geocoding_service.py -n 4 --dist loadscope
   ```
