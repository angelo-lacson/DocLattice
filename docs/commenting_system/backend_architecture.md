...
All models are defined in [`doclatticeserver/conversations/models.py`](../../doclatticeserver/conversations/models.py).

### Core Models

#### Conversation

**Source**: [`models.py:602-976`](../../doclatticeserver/conversations/models.py) — Thread or agent-based chat. Links to exactly one of `chat_with_corpus` or `chat_with_document` (enforced by DB constraint). Supports moderation (lock, pin) and soft-delete via `deleted_at`.
...
**Source**: [`models.py:978-1170`](../../doclatticeserver/conversations/models.py) — Individual message within a conversation. Supports threading via `parent_message` self-FK, agent mentions via M2M to `AgentConfiguration`, and denormalized vote counts for read performance.
...
**Source**: [`models.py:1204-1255`](../../doclatticeserver/conversations/models.py) — Tracks individual votes (`UPVOTE`/`DOWNVOTE`) on messages. Unique constraint on `(message, creator)` ensures one vote per user per message.

#### UserReputation

**Source**: [`models.py:1344-1403`](../../doclatticeserver/conversations/models.py) — Cached reputation scores per user, globally (`corpus=NULL`) and per-corpus. Denormalized for read performance; updated automatically via signals. Formula: `reputation_score = total_upvotes_received - total_downvotes_received`.

### Moderation Models

#### CorpusModerator

**Source**: [`models.py:1435-1496`](../../doclatticeserver/conversations/models.py) — Designates users as moderators with granular permissions stored in a JSON array. Unique constraint on `(corpus, user)`.

**Valid Permissions**: `lock_threads`, `pin_threads`, `delete_messages`, `delete_threads`

#### ModerationAction

**Source**: [`models.py:1526-1592`](../../doclatticeserver/conversations/models.py) — Immutable audit log of all moderation actions. Records who did what, when, and why. Indexed for fast reporting.
...
**SoftDeleteQuerySet**: [`models.py:63-106`](../../doclatticeserver/conversations/models.py) — Provides `delete()` (sets `deleted_at`), `hard_delete()` (actual removal), `alive()` (non-deleted), and `dead()` (only deleted) methods.

**SoftDeleteManager**: [`models.py:505-532`](../../doclatticeserver/conversations/models.py) — Extends `BaseVisibilityManager`. When `alive_only=True` (default), automatically filters to non-deleted records.
...
**Source**: [`doclatticeserver/conversations/signals.py`](../../doclatticeserver/conversations/signals.py)

### Vote Count Updates

**Triggers**: `post_save` and `post_delete` on `MessageVote`

Recalculates `upvote_count` and `downvote_count` on the parent `ChatMessage` from scratch using aggregation. See [`signals.py:27-50`](../../doclatticeserver/conversations/signals.py) for `recalculate_message_vote_counts()`.

### Reputation Calculation

**Triggers**: `post_save` and `post_delete` on `MessageVote`

Updates both global (`corpus=None`) and corpus-specific reputation for the message author. Aggregates all votes on the user's messages and stores in `UserReputation` via `update_or_create`. See [`signals.py:86-132`](../../doclatticeserver/conversations/signals.py) for `update_user_reputation()`.
...
**Source**: [`models.py:757-778`](../../doclatticeserver/conversations/models.py) — `Conversation.can_moderate(user)` checks: corpus owner → designated moderator with any permissions → conversation creator (for non-corpus conversations).
...
DocLattice uses Django Guardian for object-level permissions. When threads are created, `view_conversation`, `change_conversation`, and `delete_conversation` permissions are assigned to the creator.
...
**Source**: [`doclatticeserver/corpuses/models.py`](../../doclatticeserver/corpuses/models.py) — OneToOne with `Corpus`. Stores denormalized counts: thread counts (total, active), message counts (total, 7-day, 30-day), contributor counts (unique, 30-day active), total upvotes, and average messages per thread. Auto-updated timestamp via `last_updated`.

### Celery Tasks

#### update_corpus_engagement_metrics

**Source**: [`doclatticeserver/tasks/corpus_tasks.py`](../../doclatticeserver/tasks/corpus_tasks.py) — Calculates and updates metrics for a single corpus. Counts threads, messages, contributors, and upvotes using Django ORM aggregations. Idempotent and safe to run multiple times.

#### update_all_corpus_engagement_metrics

**Source**: [`doclatticeserver/tasks/corpus_tasks.py`](../../doclatticeserver/tasks/corpus_tasks.py) — Batch update that queues individual `update_corpus_engagement_metrics` tasks for all corpuses. Suitable for periodic scheduling via celerybeat.
...
- **Model tests**: `doclatticeserver/tests/test_corpus_engagement_metrics.py` (9 tests)
- **Task tests**: `doclatticeserver/tests/test_engagement_metrics_tasks.py` (13 tests)
- **GraphQL tests**: `doclatticeserver/tests/test_engagement_metrics_graphql.py` (9 tests)
