...
All tests are in `doclatticeserver/tests/`:
...
docker compose -f test.yml run django python manage.py test \
    doclatticeserver.tests.test_threading \
    doclatticeserver.tests.test_voting \
    doclatticeserver.tests.test_voting_mutations_graphql \
    doclatticeserver.tests.test_moderation \
    doclatticeserver.tests.test_conversation_mutations_graphql \
    doclatticeserver.tests.test_conversation_query
```
...
```bash
docker compose -f test.yml run django python manage.py test \
    doclatticeserver.tests.test_voting.VotingTestCase.test_upvote_increases_count
```
