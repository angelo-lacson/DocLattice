...
The notification system provides real-time alerts to users about important events in the DocLattice collaboration platform. Users receive notifications for replies, mentions, votes, badges, moderation actions, and thread participation.

## Architecture

### Components

1. **Notification Model** (`doclatticeserver/notifications/models.py`)
   - Stores notification records
   - Tracks read/unread status
   - Links to related objects (messages, conversations, actors)

2. **Signal Handlers** (`doclatticeserver/notifications/signals.py`)
...
4. **Admin Interface** (`doclatticeserver/notifications/admin.py`)
...
**Location**: `doclatticeserver/tests/test_notifications.py`, `doclatticeserver/tests/test_notification_graphql.py`
...
docker compose -f test.yml run django python manage.py test \
    doclatticeserver.tests.test_notifications \
    doclatticeserver.tests.test_notification_graphql

# Run specific test
docker compose -f test.yml run django python manage.py test \
    doclatticeserver.tests.test_notifications.TestNotificationModel.test_extract_mentions_basic
```
...
| Broadcasting | `doclatticeserver/notifications/signals.py` | `broadcast_notification_via_websocket()` |
...
**Location**: `doclatticeserver/tests/test_notification_websocket.py`

```bash
docker compose -f test.yml run django pytest doclatticeserver/tests/test_notification_websocket.py -v
```
