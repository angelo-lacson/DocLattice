# DocLattice Annotation & Relationship Performance Optimization Guide

## Summary

The DocLattice `GetDocumentKnowledgeAndAnnotations` query suffers from (not unexpected) performance issues (10-30+ seconds) due to loading ALL annotations and relationships for a corpus. This documents the **implemented optimizations**:
...
**Annotation Indexes:** `/doclatticeserver/annotations/migrations/0036_add_performance_indexes.py`
**Relationship Indexes:** `/doclatticeserver/annotations/migrations/0038_add_relationship_performance_indexes.py`

#### 1.2 Create Materialized Views for Aggregations

**Annotation Materialized Views:** `/doclatticeserver/annotations/migrations/0037_add_materialized_views.py`
**Relationship Materialized View:** `/doclatticeserver/annotations/migrations/0039_add_relationship_materialized_view.py`
...
**Implemented in:** `/doclatticeserver/tasks/materialized_view_tasks.py`
...
**Note:** Signal-based refresh triggering is handled within the materialized view tasks. The system uses debouncing and staleness checks rather than immediate signal-based refresh to avoid excessive database operations. Relationship create/update/delete and M2M changes are wired to trigger `refresh_relationship_summary_mv` after transaction commit via signal handlers in `/doclatticeserver/annotations/apps.py` and `/doclatticeserver/annotations/signals.py`.

### Phase 2: Query Optimization Layer

#### 2.1 Query Optimizers

**Implemented in:** `/doclatticeserver/annotations/query_optimizer.py`
...
**Test Coverage:** The implementation includes comprehensive tests in `/doclatticeserver/tests/performance_optimizations/`:
...
from doclatticeserver.tasks.materialized_view_tasks import (
...
1. **Materialized views not updating**
   ```python
   # Check staleness
   from doclatticeserver.tasks.materialized_view_tasks import check_materialized_view_staleness
   stats = check_materialized_view_staleness()
   print(stats)

   # Force refresh
   from doclatticeserver.tasks.materialized_view_tasks import refresh_all_materialized_views
   refresh_all_materialized_views()
   ```
