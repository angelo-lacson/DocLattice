...
This document describes the frontend authentication flow in DocLattice, including how Auth0 integration works, race condition handling, and the coordination between authentication state and Apollo cache operations.

## Overview

DocLattice uses Auth0 for authentication with an AuthGate pattern that ensures authentication is fully initialized before rendering any protected content. This eliminates race conditions where components might try to make authenticated API requests before the auth token is available.
...
if (!authInitialized || authStatus === "LOADING") {
  return <ModernLoadingDisplay type="auth" message="Initializing DocLattice" />;
}
```
