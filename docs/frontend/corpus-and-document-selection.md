...
DocLattice uses a **centralized routing architecture** where `CentralRouteManager` is the single source of truth for all URL-to-state synchronization. Components are "dumb consumers" that read reactive variables and never manipulate routing state directly.
