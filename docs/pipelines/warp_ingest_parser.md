...
The Warp-Ingest Parser is an **alternative PDF parser** for DocLattice based on
...
image. Its defining feature for DocLattice is that it renders **directly to
the DocLattice structural export format** — PAWLS word tokens, per-block
...
sequenceDiagram
    ...
    WS->>WS: Rule-based layout → blocks → DocLattice export
    WS-->>WP: {"page_dim", "num_pages", "result": <DocLatticeDocExport>}
    WP->>WP: Unwrap "result"
    WP->>DB: save_parsed_data (annotations, relationships, PAWLS)
    WP-->>U: DocLatticeDocExport
```

## Implementation

- Parser: `doclatticeserver/pipeline/parsers/warp_ingest_parser.py::WarpIngestParser`
- Base class: `doclatticeserver/pipeline/base/parser.py::BaseParser`
  (non-chunked — see [Chunking](#why-not-chunked) below)
- Auto-discovered by `doclatticeserver/pipeline/registry.py`; no manual
  registration needed.
...
`doclatticeserver/constants/document_processing.py::WARP_INGEST_PARSER_REQUEST_TIMEOUT_SECONDS`
...
logs a loud warning when serving on that fallback). DocLattice sends the key
...
(`use_cloud_run_iam_auth`). Set `WARP_INGEST_API_KEY` on the DocLattice side
...
revision returns the export unwrapped) and returns an `DocLatticeDocExport`:
...
The export already uses DocLattice field names (snake_case top-level keys,
...
- Unit tests (mocked HTTP): `doclatticeserver/tests/test_doc_parser_warp_ingest.py`
