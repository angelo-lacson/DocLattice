...
The Docling Parser is an advanced PDF document parser based on IBM's [docling](https://github.com/DS4SD/docling) document processing pipeline. As of `3.0.0-alpha1`, it is the primary parser for PDF documents in DocLattice.

Perhaps its coolest feature, besides its ability to support multiple cutting-edge OCR engines and numerous formats, is its ability to group document features into groups. We've found this to be particularly useful for contract layouts and setup our Docling integration to import these groups as DocLattice "relationships" - which, if you're not familiar, map N source annotations to N target annotations. In the case of the Docling parser, these look like this (AWESOME):
...
```python
from doclatticeserver.pipeline.parsers.docling_parser import DoclingParser
...
The parser returns an `DocLatticeDocExport` dictionary containing:
