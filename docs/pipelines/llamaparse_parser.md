...
```python
PREFERRED_PARSERS = {
    "application/pdf": "doclatticeserver.pipeline.parsers.llamaparse_parser.LlamaParseParser",
    # ... other mime types
}
```
...
LlamaParse elements are mapped to DocLattice annotation labels:

| LlamaParse Type | DocLattice Label |
|-----------------|---------------------|
| `title` | Title |
