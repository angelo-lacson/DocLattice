...
This guide covers how to configure the document processing pipeline in DocLattice, including first-time setup, upgrades, and runtime configuration.

## Overview

DocLattice uses a **database-backed configuration system** for pipeline settings. This allows superusers to change parsers, embedders, and thumbnailers at runtime without code deployment.
...
----------------------------------------
  ...
  DoclingParser
    Class: doclatticeserver.pipeline.parsers.docling_parser_rest.DoclingParser
  ...
  LlamaParseParser
    Class: doclatticeserver.pipeline.parsers.llamaparse_parser.LlamaParseParser
    Settings (1):
...
When upgrading DocLattice, the migration system preserves your existing configuration (if you've already setup pipeline settings in the DB). However, if Django settings have new defaults you want to adopt, use `--sync-preferences`:
...
(PDF, TXT, DOCX) until you configure a converter. DocLattice ships one
...
DEFAULT_FILE_CONVERTER=doclatticeserver.pipeline.file_converters.gotenberg_converter.GotenbergFileConverter
...
[`gotenberg_converter.py`](../../doclatticeserver/pipeline/file_converters/gotenberg_converter.py).
