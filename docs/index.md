...
DocLattice gives you, in one place:
...
DocLattice uses a portable text-and-layout format derived from AllenAI's PAWLs project — tokens with bounding boxes per page, so annotations carry both their text and their visual position:
...
DocLattice is released under the [MIT License](https://github.com/angelo-lacson/DocLattice/blob/main/LICENSE). Build proprietary products on it, embed it in commercial offerings, fork it, ship it — no copyleft strings attached.
