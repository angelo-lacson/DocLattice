...
> - [`doclatticeserver/corpuses/models.py`](../../doclatticeserver/corpuses/models.py) - `Corpus.add_document()`, `Corpus.get_documents()`, `Corpus.document_count()`
> - [`doclatticeserver/documents/models.py`](../../doclatticeserver/documents/models.py) - `DocumentPath` model
...
1. **`doclatticeserver/corpuses/models.py`**
...
2. **`doclatticeserver/documents/signals.py`**
...
5. **`doclatticeserver/utils/corpus_forking.py`**
...
7. **`doclatticeserver/tasks/extract_orchestrator_tasks.py`**
...
**`doclatticeserver/corpuses/migrations/0038_create_personal_corpuses.py`**
...
**File**: `doclatticeserver/corpuses/models.py`
...
**File**: `doclatticeserver/utils/corpus_forking.py`
...
**File**: `doclatticeserver/documents/signals.py`
...
1. `doclatticeserver/tests/test_document_folder_service.py:1678-1695`
   - Test: `test_add_document_updates_m2m_relationship()`
   - Action: DELETE entirely

2. `doclatticeserver/tests/test_document_versioning.py:1876`
...
docker compose -f test.yml run django pytest \
  doclatticeserver/tests/test_image_tools.py \
  doclatticeserver/tests/test_document_path_migration.py \
  doclatticeserver/tests/permissioning/ \
  -n 4 --dist loadscope
```
...
# Critical path tests
docker compose -f test.yml run django pytest \
  doclatticeserver/tests/test_corpus_fork_round_trip.py \
  doclatticeserver/tests/test_document_versioning.py \
  doclatticeserver/tests/test_document_folder_service.py \
  -n 4 --dist loadscope
```
...
# Integration tests
docker compose -f test.yml run django pytest \
  doclatticeserver/tests/test_document_uploads.py \
  doclatticeserver/tests/test_extract_tasks.py \
  -n 4 --dist loadscope
```
...
```python
from doclatticeserver.utils.corpus_forking import create_fork_corpus_task
...
1. `doclatticeserver/corpuses/models.py` - Remove M2M field and writes
2. `doclatticeserver/documents/signals.py` - Remove M2M signal handler
3. `doclatticeserver/utils/corpus_forking.py` - Remove documents.clear()
...
```bash
grep -r "\.documents\.add\|\.documents\.remove\|\.documents\.clear\|\.documents\.all\|\.documents\.filter" doclatticeserver/tests/ --include="*.py" -l | wc -l
```
...
1. `doclatticeserver/documents/management/commands/sync_m2m_to_documentpath.py` - REMOVE
2. `doclatticeserver/documents/management/commands/validate_v3_migration.py` - UPDATE (remove Check 3)
...
1. `doclatticeserver/tests/test_document_folder_service.py:1678-1695` - DELETE test
2. `doclatticeserver/tests/test_document_versioning.py:1876` - Remove assertion
