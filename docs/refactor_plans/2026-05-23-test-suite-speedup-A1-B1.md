...
**Scope:** `.github/workflows/backend.yml`, `compose/local/django/Dockerfile`, `doclatticeserver/tests/base.py`, `doclatticeserver/conftest.py`, `pytest.ini`, `doclatticeserver/tests/fixtures/test_data.json`. No changes to product code.
...
- The session-scoped `disable_document_processing_signals` fixture in `doclatticeserver/conftest.py` connects/disconnects post_save signals at session boundaries. It's autouse, runs once per worker — already fine. **No change.** Listed here only because a prior audit flagged it; leave alone.
...
- Implement via `pytest_collection_modifyitems` in `doclatticeserver/conftest.py` — auto-tag any `unittest.TestCase` subclass that defines `setUpTestData` (or that subclasses `BaseFixtureTestCase`) with `xdist_group=cls.__qualname__`. No per-class manual annotation needed.
...
- Strategy: keep only the canonical 4 documents + 1 corpus + 1 user + ~20 representative annotations in the JSON. Move the bulk rows to **factory-built per-test or per-class data** under `doclatticeserver/tests/factories.py` using `factory_boy` (already a dependency).
...
- Implementation sketch in `doclatticeserver/tests/base.py`:
...
**Failure mode:** The failures concentrated in **plain `django.test.TestCase` subclasses** (no `setUpTestData`, no `fixtures`) that my auto-tag deliberately skipped: `UserTypePrivacyTestCase`, `TestSearchAgentsForMention`, `TestDocLatticeAnalyzers`, `CreateDefaultLabelsetTestCase`, `TestHybridSearch`, `PermissionFilteringTestCase`, plus `TestNestedApprovalGates` (a `TransactionTestCase`). Worksteal interleaved tests from these classes with tests from other classes on the same worker, exposing state leaks that loadscope's class-pinning was hiding — likely a mix of migration-seeded `admin`/`system`/`Anonymous` users colliding with `UserFactory()` calls, `Celery eager` tasks committing data via `on_commit`, and `User.save()`'s handle/slug auto-generation logic that queries the global user set.
