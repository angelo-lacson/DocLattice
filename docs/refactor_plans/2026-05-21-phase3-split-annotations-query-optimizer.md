...
Both `doclatticeserver/annotations/query_optimizer.py` and `doclatticeserver/extracts/query_optimizer.py` are **deleted** at the end (No-dead-code rule).
...
**New files (8):**
- `doclatticeserver/annotations/services/__init__.py` — re-exports `AnnotationService`, `RelationshipService`.
- `doclatticeserver/annotations/services/annotation_service.py` — `AnnotationService` (from `AnnotationQueryOptimizer`).
- `doclatticeserver/annotations/services/relationship_service.py` — `RelationshipService` (from `RelationshipQueryOptimizer`).
- `doclatticeserver/analyzer/services/__init__.py` — re-exports `AnalysisService`.
- `doclatticeserver/analyzer/services/analysis_service.py` — `AnalysisService` (from `AnalysisQueryOptimizer`).
- `doclatticeserver/extracts/services/__init__.py` — re-exports `ExtractService`, `MetadataService`.
- `doclatticeserver/extracts/services/extract_service.py` — `ExtractService` (from `ExtractQueryOptimizer`).
- `doclatticeserver/extracts/services/metadata.py` — `MetadataService` (from `MetadataQueryOptimizer`).

**Deleted files (2):**
- `doclatticeserver/annotations/query_optimizer.py`
- `doclatticeserver/extracts/query_optimizer.py`
...
- `doclatticeserver/mcp/resources.py`, `doclatticeserver/mcp/tools.py`
- `doclatticeserver/shared/Managers.py`
- `doclatticeserver/documents/query_optimizer.py`
- `doclatticeserver/extracts/diff.py`, `doclatticeserver/utils/importing.py` (stale doc-comment references only)

**Modified test files (12):** see Task 5.

**New test file (1):** `doclatticeserver/tests/test_service_layer_phase3.py`.
...
**Files:**
- Create: `doclatticeserver/annotations/services/__init__.py`
- Create: `doclatticeserver/annotations/services/annotation_service.py`
- Create: `doclatticeserver/annotations/services/relationship_service.py`

- [ ] **Step 1: Confirm dead-code status of deprecated helpers (F3)**

Run:
```bash
grep -rn "_check_document_permission\|_apply_permission_filter" --include=*.py config/ doclatticeserver/
```
...
Copy `doclatticeserver/annotations/query_optimizer.py` lines 1–817 (module docstring + imports + the entire `AnnotationQueryOptimizer` class) into the new file, with these exact changes:
...
- Add to the imports block:
  ```python
  from doclatticeserver.shared.services import BaseService
  ```
...
Copy `doclatticeserver/annotations/query_optimizer.py` lines 820–1093 (the `RelationshipQueryOptimizer` class) into the new file. Add a module docstring and imports header:
...
from doclatticeserver.annotations.services.annotation_service import (
    AnnotationService,
)
from doclatticeserver.shared.services import BaseService
```
...
- All other lines (including the inner `from doclatticeserver.annotations.models import Relationship` etc.) unchanged.
...
from doclatticeserver.annotations.services.relationship_service import (
    RelationshipService,
)
...
Run:
```bash
docker compose -f test.yml run --rm django python -c "from doclatticeserver.annotations.services import AnnotationService, RelationshipService; from doclatticeserver.shared.services import BaseService; assert issubclass(AnnotationService, BaseService); assert issubclass(RelationshipService, BaseService); print('OK')"
```
Expected: `OK`.
...
```bash
git add doclatticeserver/annotations/services/
...
**Files:**
- Create: `doclatticeserver/analyzer/services/__init__.py`
- Create: `doclatticeserver/analyzer/services/analysis_service.py`

- [ ] **Step 1: Create `analysis_service.py`**

Copy `doclatticeserver/annotations/query_optimizer.py` lines 1096–1302 (the `AnalysisQueryOptimizer` class) into the new file. Header:
...
from doclatticeserver.analyzer.services.analysis_service import AnalysisService
...
Run:
```bash
docker compose -f test.yml run --rm django python -c "from doclatticeserver.analyzer.services import AnalysisService; from doclatticeserver.shared.services import BaseService; assert issubclass(AnalysisService, BaseService); print('OK')"
```
Expected: `OK`.
...
```bash
git add doclatticeserver/analyzer/services/
...
**Files:**
- Create: `doclatticeserver/extracts/services/__init__.py`
- Create: `doclatticeserver/extracts/services/extract_service.py`
- Create: `doclatticeserver/extracts/services/metadata.py`

- [ ] **Step 1: Create `extract_service.py`**

Copy `doclatticeserver/annotations/query_optimizer.py` lines 1305–1507 (the `ExtractQueryOptimizer` class) into the new file. Header:
...
Copy `doclatticeserver/extracts/query_optimizer.py` lines 1–583 in full. Changes:
- Replace the module docstring with:
  ...
  """
  ```
- Add `from doclatticeserver.shared.services import BaseService` to the import block.
- `class MetadataQueryOptimizer:` → `class MetadataService(BaseService):`.
- Everything else byte-for-byte identical (the existing `from __future__ import annotations`, `from collections import defaultdict`, `from django.db.models import QuerySet`, `from doclatticeserver.extracts.models import Column, Datacell` all stay).
...
Run:
```bash
docker compose -f test.yml run --rm django python -c "from doclatticeserver.extracts.services import ExtractService, MetadataService; from doclatticeserver.shared.services import BaseService; assert issubclass(ExtractService, BaseService); assert issubclass(MetadataService, BaseService); print('OK')"
```
Expected: `OK`.
...
```bash
git add doclatticeserver/extracts/services/
...
|---|---|---|
| `AnnotationQueryOptimizer` | `AnnotationService` | `from doclatticeserver.annotations.services import AnnotationService` |
| `RelationshipQueryOptimizer` | `RelationshipService` | `from doclatticeserver.annotations.services import RelationshipService` |
| `AnalysisQueryOptimizer` | `AnalysisService` | `from doclatticeserver.analyzer.services import AnalysisService` |
| `ExtractQueryOptimizer` | `ExtractService` | `from doclatticeserver.extracts.services import ExtractService` |
| `MetadataQueryOptimizer` | `MetadataService` | `from doclatticeserver.extracts.services import MetadataService` |
...
Import block at line 298 imports `AnalysisQueryOptimizer, ExtractQueryOptimizer` from `annotations.query_optimizer` → split into two imports: `from doclatticeserver.analyzer.services import AnalysisService` and `from doclatticeserver.extracts.services import ExtractService`. Usages at lines 362 (`AnalysisQueryOptimizer.get_visible_analyses`), 367 (`ExtractQueryOptimizer.get_visible_extracts`) → `AnalysisService.*` / `ExtractService.*`. Import at line 409 (`MetadataQueryOptimizer`) → `from doclatticeserver.extracts.services import MetadataService`; usage on the following lines → `MetadataService.*`. Leave `DocumentRelationshipQueryOptimizer` (lines ~306, 381) untouched.
...
Imports at lines 388 and 473 (`MetadataQueryOptimizer`) → `from doclatticeserver.extracts.services import MetadataService`; rename in-scope usages → `MetadataService.*`.

- [ ] **Step 7: `config/graphql/extract_queries.py`**

- Lines 157, 175, 196 import `ExtractQueryOptimizer` → `from doclatticeserver.extracts.services import ExtractService`; usages → `ExtractService.*`.
- Lines 307, 321, 341 import `MetadataQueryOptimizer` → `MetadataService`; usages → `MetadataService.*`.
- Lines 426, 442 import `AnalysisQueryOptimizer` → `from doclatticeserver.analyzer.services import AnalysisService`; usages → `AnalysisService.*`.
...
- [ ] **Step 9: `doclatticeserver/mcp/resources.py`**
...
- [ ] **Step 10: `doclatticeserver/mcp/tools.py`**
...
- [ ] **Step 11: `doclatticeserver/shared/Managers.py`**

Imports at lines 833 and 1146 (`AnnotationQueryOptimizer`) → `from doclatticeserver.annotations.services import AnnotationService`. Usages `AnnotationQueryOptimizer._compute_effective_permissions` at lines 845 and 1157 → `AnnotationService._compute_effective_permissions`. Update the explanatory comments referencing `AnnotationQueryOptimizer` at lines 660, 827, 915, 1080 → `AnnotationService`. (This is flagged issue **F1** — the inversion is preserved deliberately.)

- [ ] **Step 12: `doclatticeserver/documents/query_optimizer.py`**

This file is itself a Phase-4 optimizer (not moved now), but it *imports the relocated classes*. Update:
- Import block at line 70 (`AnalysisQueryOptimizer, ExtractQueryOptimizer`) → two imports: `from doclatticeserver.analyzer.services import AnalysisService` and `from doclatticeserver.extracts.services import ExtractService`. Usages at 113–114, 122 → `ExtractService.get_visible_extracts`, `AnalysisService.get_visible_analyses`.
...
Run:
```bash
grep -rn "QueryOptimizer\|query_optimizer" --include=*.py config/graphql/ doclatticeserver/mcp/ doclatticeserver/shared/Managers.py | grep -i "annotation\|extract\|analysis\|metadata" | grep -v "DocumentRelationshipQueryOptimizer"
```
...
```bash
git add config/graphql/ doclatticeserver/mcp/ doclatticeserver/shared/Managers.py doclatticeserver/documents/query_optimizer.py
...
- `doclatticeserver/tests/performance_optimizations/test_base.py:10` — `AnnotationQueryOptimizer` → `AnnotationService`.
- `doclatticeserver/tests/permissioning/test_comment_permission.py:25` — `AnnotationQueryOptimizer` → `AnnotationService`.
- `doclatticeserver/tests/permissioning/test_metadata_query_optimizer.py:48` — `MetadataQueryOptimizer` → `MetadataService` (`from doclatticeserver.extracts.services import MetadataService`).
- `doclatticeserver/tests/permissioning/test_version_aware_query_optimizer.py:13` — `AnnotationQueryOptimizer` → `AnnotationService`.
- `doclatticeserver/tests/test_annotation_privacy.py:17` — `AnnotationQueryOptimizer` → `AnnotationService`.
- `doclatticeserver/tests/test_corpus_annotations_query.py:17` — `AnnotationQueryOptimizer` → `AnnotationService`.
- `doclatticeserver/tests/test_get_document_knowledge_optimizations.py:30` — `AnnotationQueryOptimizer` → `AnnotationService`.
- `doclatticeserver/tests/test_analysis_annotation_import.py:161` — `AnnotationQueryOptimizer` → `AnnotationService`.
- `doclatticeserver/tests/test_structural_annotations_graphql_backwards_compat.py:876` — `RelationshipQueryOptimizer` → `RelationshipService`.
- `doclatticeserver/tests/test_visibility_managers.py:626` — `AnnotationQueryOptimizer` → `AnnotationService` (usage `AnnotationQueryOptimizer._compute_effective_permissions` at line 629).

- [ ] **Step 2: Update multi-symbol test imports**

- `doclatticeserver/tests/test_query_optimizer_structural_sets.py:23` imports `AnnotationQueryOptimizer, RelationshipQueryOptimizer` from `annotations.query_optimizer` → single import `from doclatticeserver.annotations.services import AnnotationService, RelationshipService`. Rename usages throughout the file.
- `doclatticeserver/tests/permissioning/test_query_optimizer_methods.py:46` imports `AnnotationQueryOptimizer, ExtractQueryOptimizer, RelationshipQueryOptimizer` → split: `from doclatticeserver.annotations.services import AnnotationService, RelationshipService` **and** `from doclatticeserver.extracts.services import ExtractService`. Also update the three function-local imports at lines 404, 1568, 1631 (`AnalysisQueryOptimizer` → `from doclatticeserver.analyzer.services import AnalysisService`). Rename all in-file usages.

- [ ] **Step 3: Verify no stale test references remain**

Run:
```bash
grep -rn "AnnotationQueryOptimizer\|\bRelationshipQueryOptimizer\|AnalysisQueryOptimizer\|ExtractQueryOptimizer\|MetadataQueryOptimizer\|annotations.query_optimizer\|extracts.query_optimizer" --include=*.py doclatticeserver/tests/
```
Expected: no output.
...
```bash
git add doclatticeserver/tests/
...
Run:
```bash
grep -rn "annotations.query_optimizer\|annotations\.query_optimizer\|extracts.query_optimizer\|extracts\.query_optimizer" --include=*.py config/ doclatticeserver/
```
...
```bash
git rm doclatticeserver/annotations/query_optimizer.py doclatticeserver/extracts/query_optimizer.py
```
...
**Files:**
- Create: `doclatticeserver/tests/test_service_layer_phase3.py`
- Test: same file.
...
class Phase3ServiceRelocationTests(SimpleTestCase):
    def test_annotation_services_importable(self):
        from doclatticeserver.annotations.services import (
            AnnotationService,
            RelationshipService,
        )
    ...
    def test_analysis_service_importable(self):
        from doclatticeserver.analyzer.services import AnalysisService

        self.assertTrue(issubclass(AnalysisService, BaseService))

    def test_extract_services_importable(self):
        from doclatticeserver.extracts.services import (
            ExtractService,
            MetadataService,
        )
    ...
    def test_old_monolith_modules_removed(self):
        ...
        with self.assertRaises(ModuleNotFoundError):
            importlib.import_module("doclatticeserver.annotations.query_optimizer")
        with self.assertRaises(ModuleNotFoundError):
            importlib.import_module("doclatticeserver.extracts.query_optimizer")
```
...
Run:
```bash
docker compose -f test.yml run --rm django python manage.py test doclatticeserver.tests.test_service_layer_phase3 --keepdb
```
...
```bash
git add doclatticeserver/tests/test_service_layer_phase3.py
...
**Files:**
- Modify: `CHANGELOG.md`
- Modify: `doclatticeserver/extracts/diff.py` (docstring at line ~101)
- Modify: `doclatticeserver/utils/importing.py` (comment at line ~168)
- Modify: `doclatticeserver/annotations/models.py` (comment at line ~1363)
- Modify: `doclatticeserver/shared/QuerySets.py` (docstring references at lines ~35, 399, 410)
...
```bash
grep -rn "AnnotationQueryOptimizer\|ExtractQueryOptimizer\|AnalysisQueryOptimizer\|MetadataQueryOptimizer" --include=*.py doclatticeserver/ config/ | grep -v "DocumentRelationship"
```
Expected: no output.
...
```bash
git add CHANGELOG.md doclatticeserver/extracts/diff.py doclatticeserver/utils/importing.py doclatticeserver/annotations/models.py doclatticeserver/shared/QuerySets.py
...
docker compose -f test.yml run --rm django pytest -n 4 --dist loadscope \
  doclatticeserver/tests/permissioning/test_query_optimizer_methods.py \
  doclatticeserver/tests/permissioning/test_metadata_query_optimizer.py \
  doclatticeserver/tests/permissioning/test_version_aware_query_optimizer.py \
  doclatticeserver/tests/permissioning/test_comment_permission.py \
  doclatticeserver/tests/test_query_optimizer_structural_sets.py \
  doclatticeserver/tests/test_corpus_annotations_query.py \
  doclatticeserver/tests/test_annotation_privacy.py \
  doclatticeserver/tests/test_get_document_knowledge_optimizations.py \
  doclatticeserver/tests/test_analysis_annotation_import.py \
  doclatticeserver/tests/test_visibility_managers.py \
  doclatticeserver/tests/test_structural_annotations_graphql_backwards_compat.py \
  doclatticeserver/tests/test_extract_iterations.py \
  doclatticeserver/tests/performance_optimizations/test_base.py \
  doclatticeserver/tests/test_service_layer_phase3.py
```
Expected: all pass. Any failure is a relocation defect — diff the moved class against the original (`git show HEAD~N:doclatticeserver/annotations/query_optimizer.py`) to find the divergence; do NOT change test expectations.
...
```bash
docker compose -f test.yml run --rm django pytest -n 4 --dist loadscope doclatticeserver/tests/ -k "extract or corpus or annotation or relationship or metadata or mcp"
```
Expected: all pass.
...
Run:
```bash
grep -rn "query_optimizer" --include=*.py config/ doclatticeserver/ | grep -i "annotation\|extract\b\|analysis\|metadata"
```
