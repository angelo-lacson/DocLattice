...
**Architecture:** A new `doclatticeserver/shared/services/` package holds (a) `conventions.py` — the `ServiceResult` write-result envelope and the IDOR-safe `get_for_user_or_none` lookup; and (b) `base.py` — `BaseService`, a staticmethod-based base class providing IDOR-safe lookup, permission-filtered queryset access, a uniform permission gate, and structured logging. No existing callers are migrated in this phase; it ships the primitives only, verified by unit tests.

**Tech Stack:** Python 3.12, Django 4.x, `django-guardian` permissions, existing `user_can` / `visible_to_user` manager API (`doclatticeserver/shared/Managers.py`, `shared/QuerySets.py`, `shared/user_can_mixin.py`), `PermissionTypes` enum (`doclatticeserver/types/enums.py`).
...
- **`PermissionTypes`** is a `str` enum (`doclatticeserver/types/enums.py`): `READ`, `CREATE`, `UPDATE`, `EDIT`, `DELETE`, `COMMENT`, `PERMISSION`, `PUBLISH`, `CRUD`, `ALL`. Each member has a `.value` (e.g. `PermissionTypes.READ.value == "READ"`).
...
- Create: `doclatticeserver/shared/services/__init__.py` — package root, re-exports `BaseService`, `ServiceResult`, `get_for_user_or_none`.
- Create: `doclatticeserver/shared/services/conventions.py` — `ServiceResult` dataclass + `get_for_user_or_none` function.
- Create: `doclatticeserver/shared/services/base.py` — `BaseService` class.
- Create: `doclatticeserver/tests/test_base_service.py` — unit tests for all of the above.

`doclatticeserver/shared/` already exists as a package (`__init__.py` present). The new `services/` subdirectory needs its own `__init__.py` (Task 5).
...
**Files:**
- Create: `doclatticeserver/shared/services/conventions.py`
- Test: `doclatticeserver/tests/test_base_service.py`

- [x] **Step 1: Write the failing test**

Create `doclatticeserver/tests/test_base_service.py`:
...
Run: `docker compose -f test.yml run --rm django python manage.py test doclatticeserver.tests.test_base_service.TestServiceResult --keepdb`
Expected: FAIL — `ModuleNotFoundError: No module named 'doclatticeserver.shared.services'`
...
Every service in ``doclatticeserver/*/services/`` returns results through
...
```bash
git add doclatticeserver/shared/services/conventions.py doclatticeserver/tests/test_base_service.py
...
**Files:**
- Modify: `doclatticeserver/shared/services/conventions.py`
- Test: `doclatticeserver/tests/test_base_service.py`

- [x] **Step 1: Write the failing test**

Append to `doclatticeserver/tests/test_base_service.py`:

```python
from doclatticeserver.corpuses.models import Corpus
from doclatticeserver.shared.services.conventions import get_for_user_or_none
...
Run: `docker compose -f test.yml run --rm django python manage.py test doclatticeserver.tests.test_base_service.TestGetForUserOrNone --keepdb`
Expected: FAIL — `ImportError: cannot import name 'get_for_user_or_none'`

- [x] **Step 3: Write minimal implementation**

Append to `doclatticeserver/shared/services/conventions.py`:
...
**Files:**
- Create: `doclatticeserver/shared/services/base.py`
- Test: `doclatticeserver/tests/test_base_service.py`
...
Run: `docker compose -f test.yml run --rm django python manage.py test doclatticeserver.tests.test_base_service.TestBaseServiceLookup --keepdb`
Expected: FAIL — `ModuleNotFoundError: No module named 'doclatticeserver.shared.services.base'`
...
```python
"""``BaseService`` — shared machinery for the DocLattice service layer.

Every concrete service (``doclatticeserver/*/services/*.py``) inherits
...
```bash
git add doclatticeserver/shared/services/base.py doclatticeserver/tests/test_base_service.py
...
**Files:**
- Modify: `doclatticeserver/shared/services/base.py`
- Test: `doclatticeserver/tests/test_base_service.py`
...
Run: `docker compose -f test.yml run --rm django python manage.py test doclatticeserver.tests.test_base_service.TestBaseServiceRequirePermission --keepdb`
Expected: FAIL — `AttributeError: type object 'BaseService' has no attribute 'require_permission'`

- [x] **Step 3: Write minimal implementation**

Append a new method to the `BaseService` class in `doclatticeserver/shared/services/base.py` (after `filter_visible`):
...
**Files:**
- Modify: `doclatticeserver/shared/services/base.py`
- Create: `doclatticeserver/shared/services/__init__.py`
- Test: `doclatticeserver/tests/test_base_service.py`
...
class TestBaseServiceLogAction(TestCase):
    ...
    def test_log_action_emits_info_line_with_action_model_and_user(self):
        with self.assertLogs(
            "doclatticeserver.shared.services.base", level="INFO"
        ) as captured:
...
class TestServicesPackageExports(SimpleTestCase):
    ...
    (``from doclatticeserver.shared.services import BaseService``) so the
...
Run: `docker compose -f test.yml run --rm django python manage.py test doclatticeserver.tests.test_base_service.TestBaseServiceLogAction doclatticeserver.tests.test_base_service.TestServicesPackageExports --keepdb`
Expected: FAIL — `AttributeError: type object 'BaseService' has no attribute 'log_action'`

- [x] **Step 3: Write minimal implementation**

Append a new method to the `BaseService` class in `doclatticeserver/shared/services/base.py` (after `require_permission`):
...
Run: `docker compose -f test.yml run --rm django python manage.py test doclatticeserver.tests.test_base_service --keepdb`
...
pre-commit run --files \
  doclatticeserver/shared/services/__init__.py \
  doclatticeserver/shared/services/base.py \
  doclatticeserver/shared/services/conventions.py \
  doclatticeserver/tests/test_base_service.py
```

Expected: all hooks pass (or auto-fix formatting — if files are modified, re-stage them). Then:

```bash
git add doclatticeserver/shared/services/__init__.py \
  doclatticeserver/shared/services/base.py \
  doclatticeserver/tests/test_base_service.py
