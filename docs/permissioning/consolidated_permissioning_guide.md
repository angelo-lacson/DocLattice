...
**Note**: The Relationship model (for annotation-to-annotation relationships) has the same privacy fields as Annotation: `created_by_analysis`, `created_by_extract`, `structural`, and `is_public`. See model definition at `doclatticeserver/annotations/models.py:155-376`.
...
**Implementation**: `MetadataQueryOptimizer.check_metadata_mutation_permission()` in `doclatticeserver/extracts/query_optimizer.py`
...
```python
# From doclatticeserver/annotations/query_optimizer.py
...
The `DocumentActionsQueryOptimizer` class in `doclatticeserver/documents/query_optimizer.py` provides centralized permission logic for document-related queries:

```python
from doclatticeserver.documents.query_optimizer import DocumentActionsQueryOptimizer
...
- `doclatticeserver/tests/permissioning/test_annotation_privacy_scoping.py` - Proves privacy scoping works
...
- `doclatticeserver/tests/llms/test_agent_permissions.py` - Agent factory permission filtering
- `doclatticeserver/tests/websocket/test_unified_agent_consumer_permissions.py` - WebSocket layer validation
- `doclatticeserver/tests/llms/test_tool_permission_enforcement.py` - Runtime permission checks
