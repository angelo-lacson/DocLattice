# DocLattice Permission System - Complete Guide
...
> **🔴 CRITICAL SECURITY**: Structural annotations and relationships are ALWAYS read-only except for superusers. Even owners with full CRUD permissions cannot modify structural items. This is enforced in `AnnotationManager.user_can` and `RelationshipManager.user_can` (`doclatticeserver/shared/Managers.py`) — the structural-write branch runs before any other permission branch. This superuser write to structural items is the **single retained admin data privilege** (a deliberate break-glass for repairing system-generated structural data); see the Admin (Superuser) Access Model banner below.
...
> **⚠️ DEPRECATION WARNING**: The `resolve_oc_model_queryset` function in `doclatticeserver.shared.resolvers` was DEPRECATED and replaced with `Model.objects.visible_to_user(user)` calls.
...
> **🟣 USER PROFILE PRIVACY**: User profiles have privacy controls via `is_profile_public`. Private profiles are visible only to users who share corpus membership with > READ permission. See `UserService` in `doclatticeserver/users/services/user_service.py`.

> **🟣 BADGE VISIBILITY**: Badge awards follow the recipient's profile privacy rules. Badges are visible if the recipient's profile is visible, or for corpus-specific badges, if the user has access to that corpus. See `BadgeService` in `doclatticeserver/badges/services/badge_service.py`.

> **🟢 SERVICE-LAYER ENTRY (Phase 6 — issue #1720)**: Every consumer of permission-filtered data (GraphQL resolvers, MCP tools, REST views, user-context Celery tasks) reaches models through `doclatticeserver/<app>/services/`. The shared base `doclatticeserver.shared.services.base.BaseService` exposes `get_or_none`, `filter_visible`, `filter_visible_qs` (chains `visible_to_user` onto an existing queryset/related manager in one SQL pass and **fails closed** — raises `TypeError` rather than passing unfiltered rows through), `require_permission`, and `user_has` for cases where a dedicated per-app method is overkill. Direct inline use of the Tier-0 tokens `visible_to_user` / `user_can` / `user_has_permission_for_obj` is forbidden in `config/graphql/` and enforced **twice**: by `doclatticeserver/tests/architecture/test_graphql_service_layer.py` AND by a Django system check (`doclatticeserver/shared/checks.py`, `doclattice.E001`) that fails `manage.py` startup on any violation. (The legacy `user_has_permission_for_obj` helper itself has been deleted; the token remains scanned so it cannot be reintroduced.) **Scope nuance:** both enforcers scan `config/graphql/` only, and only for those three tokens — e.g. the mention-autocomplete resolvers in `config/graphql/search_queries.py` still inline guardian `get_objects_for_user` legally. Treat a green E001 as "no inline Tier-0 in `config/graphql/`", not "everything is service-routed". See `docs/architecture/query_permission_patterns.md` for the full per-app service catalogue.
...
DocLattice implements a sophisticated hierarchical permission system with different rules for different object types:
...
2. **CorpusFolder - INHERITS CORPUS PERMISSIONS**
   ...
   - Implementation: permission gates live in `FolderCRUDService` (`doclatticeserver/corpuses/services/folders.py` — every write method checks `corpus.user_can(user, PermissionTypes.UPDATE)`, which honours creator status and group grants); `config/graphql/corpus_folder_mutations.py` is a thin GraphQL wrapper that delegates to the service
...
3. **Annotations and Relationships - NO INDIVIDUAL PERMISSIONS**
   ...
   - Relationships use the same permission inheritance model as annotations (implemented in `RelationshipManager.user_can` / `RelationshipQuerySet.visible_to_user`, `doclatticeserver/shared/Managers.py`)
   - **Privacy fields**: Both Annotation and Relationship models have `created_by_analysis`, `created_by_extract`, `structural`, and `is_public` fields (see `doclatticeserver/annotations/models.py`)
...
### Backend Enum (doclatticeserver/types/enums.py)
...
**Implementation:**
- Model + signal-driven count maintenance: `doclatticeserver/corpuses/models.py` (`CorpusVote`, `CorpusVoteType`) + `doclatticeserver/corpuses/signals.py` (`update_corpus_vote_counts_on_save` / `_on_delete`)
- Service: `doclatticeserver/corpuses/services/votes.py` (`CorpusVoteService`)
- GraphQL: `config/graphql/voting_mutations.py` (`VoteCorpusMutation`, `RemoveCorpusVoteMutation`)
- Tests: `doclatticeserver/tests/test_corpus_voting.py`
...
For every object type below, permissions are reachable through two paired surfaces, both defined per model in `doclatticeserver/shared/Managers.py`:
...
| **Embedding** | Inherited (parent object) | Parent document/annotation/note visibility | — | `EmbeddingManager` (`doclatticeserver/shared/Managers.py`) |
...
**Implementation**:
- `Corpus._propagate_public_status_to_documents()` in `doclatticeserver/corpuses/models.py`
- `import_document()` in `doclatticeserver/documents/versioning.py` sets `is_public=corpus.is_public`
...
**Service**: Use `DocumentRelationshipService` (`doclatticeserver/documents/services/relationships.py`) for:
...
**Implementation**: `MetadataService.check_metadata_mutation_permission()` in `doclatticeserver/extracts/services/metadata.py`
...
**Implementation**: `ConversationQuerySet.visible_to_user()` in `doclatticeserver/conversations/models.py`
...
- Located in `ChatMessageQuerySet.visible_to_user()` (`doclatticeserver/conversations/models.py`)
...
`doclatticeserver/corpuses/services/corpus_groups.py::CorpusGroupService.get_group_corpora_visible_to_user`.
...
`doclatticeserver/tests/test_corpus_groups.py::CorpusGroupServiceTests::test_update_group_preserves_invisible_member_on_unrelated_edit`
...
- Annotations: `AnnotationManager.user_can` (`doclatticeserver/shared/Managers.py`) — structural-write branch runs before any other permission branch
- Relationships: `RelationshipManager.user_can` (`doclatticeserver/shared/Managers.py`) — same ordering
...
**Moderator Access:**
A user can moderate a thread if any of the following are true (`Conversation.can_moderate`, `doclatticeserver/conversations/models.py`):
...
**In `AnnotationService._compute_effective_permissions()` (`doclatticeserver/annotations/services/annotation_service.py`):**
...
### Core Utilities (doclatticeserver/utils/permissioning.py)
...
```python
# From doclatticeserver/annotations/services/annotation_service.py
...
1. **Structural Annotations** ⚠️ **CRITICAL SECURITY RULE**
   ...
   - This protection is enforced in `AnnotationManager.user_can` (`doclatticeserver/shared/Managers.py`), before any other permission branch
...
2. **Structural Relationships** ⚠️ **CRITICAL SECURITY RULE**
   ...
   - This protection is enforced in `RelationshipManager.user_can` (`doclatticeserver/shared/Managers.py`), before any other permission branch
...
**CRITICAL**: The canonical single-object authorization check is `Model.objects.user_can(user, obj, permission)` (manager surface) / `obj.user_can(user, permission)` (instance surface), defined per model in `doclatticeserver/shared/Managers.py`. Never bypass this API or implement custom permission logic.
...
**Implementation Details:**
- Structural annotation protection: `AnnotationManager.user_can` (`doclatticeserver/shared/Managers.py`)
- Structural relationship protection: `RelationshipManager.user_can` (`doclatticeserver/shared/Managers.py`)
- Both checks happen BEFORE any other permission logic
- `user_can` is the read/check counterpart of the `Model.objects.visible_to_user(user)` queryset filter; the two are pinned to agree by `doclatticeserver/tests/permissioning/test_authorization_invariants.py`
...
`doclatticeserver/utils/source_visibility.py`, and shared by every list
...
from doclatticeserver.utils.source_visibility import (
...
The `UserService` class in `doclatticeserver/users/services/user_service.py` provides centralized user visibility logic:

```python
from doclatticeserver.users.services import UserService
...
The `BadgeService` class in `doclatticeserver/badges/services/badge_service.py` provides centralized badge visibility logic:

```python
from doclatticeserver.badges.services import BadgeService
...
- `doclatticeserver/tests/permissioning/test_user_visibility.py` - 16 tests for user visibility
...
The `DocumentActionsService` class in `doclatticeserver/documents/services/actions.py` provides centralized permission logic for document-related queries:

```python
from doclatticeserver.documents.services import DocumentActionsService
...
- **`ExtractService`** (`doclatticeserver/extracts/services/extract_service.py`): filters visible extracts (hybrid model — extract permission AND corpus READ)
- **`AnalysisService`** (`doclatticeserver/analyzer/services/analysis_service.py`): filters visible analyses (same hybrid model)
...
Comprehensive tests are available in:
- `doclatticeserver/tests/permissioning/test_document_actions_permissions.py` - 11 tests
...
- `doclatticeserver/tests/permissioning/test_authorization_invariants.py` - **The filter/check parity suite** (`visible_to_user ⟺ user_can(READ)` per model, incl. relationship privacy recursion and group-granted sources)
- `doclatticeserver/tests/permissioning/test_annotation_privacy_scoping.py` - Proves annotation privacy scoping works
- `doclatticeserver/tests/permissioning/test_relationship_privacy_scoping.py` - Relationship privacy scoping through the document-view listing (2026-06)
- `doclatticeserver/tests/permissioning/test_source_visibility_group_grants.py` - Group-granted analysis/extract permissions unlock private rows in list queries (2026-06)
- `doclatticeserver/tests/permissioning/test_extract_anonymous_lockdown.py` - Extracts are never anonymous-visible, at manager AND service layers (2026-06)
- `doclatticeserver/tests/permissioning/test_structural_mypermissions_breakglass.py` - Pre-computed myPermissions mirror the structural-write break-glass (2026-06)
- `doclatticeserver/tests/permissioning/test_annotation_permission_inheritance.py` - Validates inheritance model
- `doclatticeserver/tests/permissioning/test_analysis_extract_hybrid_permissions.py` - Tests hybrid permission model
- `doclatticeserver/tests/test_structural_protection.py` - **Tests structural annotation/relationship protection**
- `doclatticeserver/tests/test_relationship_mutation_permissions.py` - Tests relationship permission inheritance
...
- File: `doclatticeserver/tests/test_structural_protection.py`
...
**Backend Tests** (`doclatticeserver/tests/test_mention_permissions.py`):
...
**Location**: `doclatticeserver/llms/agents/agent_factory.py`
...
# doclatticeserver/llms/tools/tool_factory.py.
...
**Location**: `doclatticeserver/llms/tools/pydantic_ai_tools.py`
...
- Vector Store: `doclatticeserver/llms/vector_stores/core_vector_stores.py`
...
- `doclatticeserver/tests/test_agent_factory.py` - Agent factory permission filtering
- `doclatticeserver/tests/websocket/test_unified_agent_consumer.py` - WebSocket layer validation
- `doclatticeserver/tests/websocket/test_agent_permission_escalation.py` - Escalation scenarios across the layers
- `doclatticeserver/tests/test_pydantic_ai_tools_module.py` - Runtime permission checks + fault tolerance
...
`Model.objects.user_can(user, obj, permission)` (manager surface) and `obj.user_can(user, permission)` (instance surface) are the canonical single-object authorization check. They are the read/check counterpart of the `Model.objects.visible_to_user(user)` queryset filter. Both live per model in `doclatticeserver/shared/Managers.py`; the instance surface is provided by `InstanceUserCanMixin` (`doclatticeserver/shared/user_can_mixin.py`), which delegates to `type(obj)._default_manager.user_can`.

**Filter/check parity is an invariant.** For READ, `user_can(u, obj, READ)` must equal `visible_to_user(u).filter(pk=obj.pk).exists()` for every user. This is pinned by the suite in `doclatticeserver/tests/permissioning/test_authorization_invariants.py` — if a manager's `user_can` and its queryset's `visible_to_user` ever drift, that suite fails by design.
...
- **`DocumentService`** (`doclatticeserver/documents/document_service.py`) — the single source of truth for document-level operations: creation, quota, lifecycle, document-level permissions, and standalone single-document lookup. Use it when the document is the noun and corpus context is incidental.
- **The `doclatticeserver/corpuses/services/` package** — the single source of truth for corpus-scoped operations: "give me X inside corpus Y for user Z" (issue #1716 split the former `CorpusObjsService` monolith into segmented services). Import the specific service from `doclatticeserver.corpuses.services`: `CorpusDocumentService` (document-in-corpus reads/writes + membership: `get_corpus_documents`, `get_corpus_documents_visible_to_user`, `get_corpus_document_by_slug`, `get_corpus_document_by_id`, `is_document_in_corpus`, CAML articles), `FolderCRUDService` (folder CRUD, tree, search), `FolderDocumentService` (document-in-folder placement), `DocumentLifecycleService` (soft-delete/restore/trash), `CorpusPathService` (`DocumentPath` disambiguation internals), and `CorpusService` (Corpus-row CRUD: delete, visibility, description versioning).
