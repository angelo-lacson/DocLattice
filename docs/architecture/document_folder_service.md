...
**Location:** `doclatticeserver/corpuses/folder_service.py`

## Overview

`DocumentFolderService` is the centralized service for all document lifecycle and folder operations in DocLattice. It serves as the **single source of truth** for:
...
Comprehensive tests are in `doclatticeserver/tests/test_document_folder_service.py`:
...
Run tests:
```bash
docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_document_folder_service.py -v
```
