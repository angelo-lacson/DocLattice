...
When a corpus is created, DocLattice can automatically generate two
...
(`doclatticeserver/corpuses/signals.py::trigger_corpus_branding_on_creation`)
...
* `generate_corpus_branding` (`doclatticeserver/tasks/corpus_tasks.py`) is a
...
`doclatticeserver/utils/image_generation.py::agenerate_logo_image` is the
...
> calls (`doclatticeserver/tests/test_corpus_branding.py`).
