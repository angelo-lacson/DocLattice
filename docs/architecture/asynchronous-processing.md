# Asynchronous Processing in DocLattice

DocLattice uses Celery for distributed task processing and Django signals for event-driven architecture. This document covers both systems and how they interact.

## Celery Task Queue

DocLattice makes extensive use of Celery, a powerful Python framework for distributed and asynchronous processing. The docker compose stack includes dedicated celeryworkers to handle computationally-intensive and long-running tasks.
...
DocLattice configures Celery for **at-least-once** delivery via two global
...
DocLattice uses Redis as the Celery broker. Redis tracks unacknowledged
...
| Task | File | Risk |
|------|------|------|
| `process_corpus_action` (extract path) | `doclatticeserver/tasks/corpus_tasks.py` | Duplicate `Datacell` rows |
| `process_thread_corpus_action` | `doclatticeserver/tasks/corpus_tasks.py` | Duplicate `CorpusActionExecution` |
| `process_message_corpus_action` | `doclatticeserver/tasks/corpus_tasks.py` | Duplicate `CorpusActionExecution` |
| `generate_agent_response` | `doclatticeserver/tasks/agent_tasks.py` | Duplicate `ChatMessage` |
| Bulk import paths | `doclatticeserver/tasks/import_tasks.py` (raw `.create()` calls) | Duplicate imported objects |
...
DocLattice uses Django signals for event-driven processing. Key signals include:

### Document Processing Signals

**Location**: `doclatticeserver/documents/signals.py`
...
**Location**: `doclatticeserver/corpuses/signals.py`
...
```python
# doclatticeserver/corpuses/apps.py
class CorpusesConfig(AppConfig):
    def ready(self):
        from doclatticeserver.corpuses import signals  # noqa: F401

# doclatticeserver/documents/apps.py
class DocumentsConfig(AppConfig):
    def ready(self):
        from doclatticeserver.documents import signals  # noqa: F401
```
...
- [CorpusAction System](./doclattice-corpus-actions.md)
