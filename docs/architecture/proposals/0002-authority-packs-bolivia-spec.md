...
> `doclatticeserver/enrichment/data/authority_packs/bolivia/` (the
...
`AuthoritySection` (`doclatticeserver/enrichment/authorities.py`), the
`bootstrap_authority` command, and `ALL_AUTHORITY_TYPES`
(`doclatticeserver/enrichment/constants.py`).
...
```
doclatticeserver/enrichment/data/authority_packs/bolivia/
...
#   "from doclatticeserver.enrichment.services.authority_mapping_loader import AuthorityMappingLoader; \
...
doclatticeserver.utils.safe_http (the source host must be on
...
from doclatticeserver.utils.safe_http import safe_fetch_text
...
`PUBLIC_DOMAIN_SOURCE_HOSTS` in `doclatticeserver/constants/safe_http.py` in the
...
docker compose -f local.yml run --rm django python manage.py load_authority_pack \
  --path doclatticeserver/enrichment/data/authority_packs/bolivia \
...
`doclatticeserver/pipeline/authority_source_providers/`, apply the Slot-5
