...
> implemented — see
> `doclatticeserver/enrichment/data/authority_packs/bolivia/` and
> `doclatticeserver/corpuses/management/commands/load_authority_pack.py`
> (tests: `doclatticeserver/tests/test_authority_pack.py`). The live-fetch
...
> (`doclatticeserver/pipeline/base/base_authority_discovery_provider.py`) answers
...
> `CorpusGroup` (`doclatticeserver/corpuses/models.py`) bundles N corpora
...
> `CorpusGroupService`
> (`doclatticeserver/corpuses/services/corpus_groups.py`) resolves the
> group's corpora at **call time**, filtered per-user — never a config-time
> snapshot. The `search_across_corpora` agent tool
> (`doclatticeserver/llms/tools/core_tools/multi_corpus.py`) fans a query
...
> `doclatticeserver/tests/test_corpus_groups.py`,
...
|---|---|---|
| **#1305** | A standalone `doclatticeserver/bolivian_laws/` app: its own `BaseScraper` + 3 site scrapers, SHA-256-dedup ingestion, 11 hard-coded legal areas with personas, a bespoke `AskBolivianLawMutation`, and an orchestrator/specialist agent layer. | Closed — reference implementation |
...
1. **Taxonomy** — `doclatticeserver/enrichment/data/authority_mappings.yaml`
   (`prefixes:` / `equivalences:` / `rewrite_rules:`), loaded idempotently by
   `AuthorityMappingLoader.load_all(path=…)`
   (`doclatticeserver/enrichment/services/authority_mapping_loader.py`) into
   ...
   (`doclatticeserver/enrichment/constants.py`): `statute`, `regulation`,
...
2. **Fetch** — a concrete subclass of `BaseAuthoritySourceProvider`
   (`doclatticeserver/pipeline/base/base_authority_source_provider.py`) dropped
   into `doclatticeserver/pipeline/authority_source_providers/`. The registry
...
3. **Corpus + content** — `bootstrap_authority_corpus()`
   (`doclatticeserver/enrichment/authorities.py`), driven by the
...
4. **Persona** (optional) — `Corpus.corpus_agent_instructions`
   (`doclatticeserver/corpuses/models.py`) consumed by
...
| **Source-host allowlist entry** | `PUBLIC_DOMAIN_SOURCE_HOSTS` in `doclatticeserver/constants/safe_http.py` | **source edit** | Phase 2 ⚠️ |
...
1. **Drop in** — place the provider module(s) in
   `doclatticeserver/pipeline/authority_source_providers/` and the pack's YAML +
