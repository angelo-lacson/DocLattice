...
This document describes the V2 corpus export/import format, which captures the full state of an DocLattice corpus — including features that were added after the original V1 export was designed.
...
The authoritative TypedDict is `DocLatticeExportDataJsonV2Type` in `doclatticeserver/types/dicts.py`. At a high level it carries three groups of fields:
...
- `corpus` — corpus metadata (V2-enhanced via `DocLatticeCorpusV2Type`)
...
`DocLatticeDocExport` (also in `dicts.py`) gains two V2-only fields:
...
- The V1 sub-shape is a subset of `DocLatticeExportDataJsonV2Type`.
...
The existing `StartCorpusExport` mutation accepts an `export_format` argument. When the value is `DOCLATTICE_V2`, the mutation dispatches `package_corpus_export_v2` with the appropriate flags (`include_conversations`, `include_action_trail`, `analysis_pk_list`, `annotation_filter_mode`). No separate V2 export mutation exists — and none is needed.
...
This enables reliable backup/restore and dataset sharing across DocLattice deployments.
