...
> **Status**: The `corpus.documents` M2M has been removed. These tasks now use `Corpus.add_document()` which creates `DocumentPath` records directly. See [`doclatticeserver/corpuses/models.py`](../../doclatticeserver/corpuses/models.py) for the implementation.

| # | Task | File | Status | Notes |
|---|------|------|--------|-------|
| 1 | `import_corpus()` | doclatticeserver/tasks/import_tasks.py | COMPLETED | Uses `corpus.add_document()` |
| 2 | `import_document_to_corpus()` | doclatticeserver/tasks/import_tasks.py | COMPLETED | Uses `corpus.add_document()` |
| 3 | `process_documents_zip()` | doclatticeserver/tasks/import_tasks.py | 346-570 | Direct `Document()` constructor + `.save()` (lines 495-518) | Use `DocumentFolderService.create_document()` then `add_document_to_corpus()` |

### 3.2 Import Tasks V2 (MEDIUM PRIORITY)

| # | Task | File | Lines | Current Behavior | Replace With |
|---|------|------|-------|------------------|--------------|
| 4 | `_import_corpus_v1()` | doclatticeserver/tasks/import_tasks_v2.py | 107-236 | Direct `Document.objects.create()` (line 187) | Consider using service for consistency |
| 5 | `_import_corpus_v2()` | doclatticeserver/tasks/import_tasks_v2.py | 239-460 | Direct `Document.objects.create()` (line 338) | Consider using service for consistency |

### 3.3 Fork Tasks (LOW PRIORITY - Already Good)

| # | Task | File | Lines | Current Behavior | Status |
|---|------|------|-------|------------------|--------|
| 6 | `fork_corpus()` | doclatticeserver/tasks/fork_tasks.py | 24-219 | Uses `corpus.add_document()` correctly | OK - No changes needed |
...
|---|--------|------|-------|------------------|--------|
| 1 | `Corpus.add_document()` | doclatticeserver/corpuses/models.py | 345-506 | Direct `DocumentPath.objects.create()` (line 490) | Keep as internal - called by service |
| 2 | `Corpus.remove_document()` | doclatticeserver/corpuses/models.py | 562-652 | Direct `DocumentPath.objects.create()` (lines 604, 635) | Keep as internal - called by service |
...
|---|----------|------|-------|------------------|--------------|
| 1 | `import_corpus_folders()` | doclatticeserver/utils/import_v2.py | 224-234 | Direct `CorpusFolder.objects.create()` | `DocumentFolderService.create_folder()` (needs `is_public` param added) |
| 2 | `import_document_paths()` | doclatticeserver/utils/import_v2.py | 307-317 | Direct `DocumentPath.objects.create()` | Keep for import - specialized operation |
...
|------|-------|------------|
| doclatticeserver/documents/versioning.py | 176, 232, 267, 291, 339, 373, 407 | `DocumentPath.objects.create()` for versioning |
...
- [x] **4.1** `import_corpus()` (import_tasks.py:40-227)
  - Now uses `corpus.add_document()` - see [`doclatticeserver/corpuses/models.py`](../../doclatticeserver/corpuses/models.py)
