...
DocLattice builds on the work that AllenAI did with [PAWLs](https://github.com/allenai/pawls) to create a consistent
...
PAWLs and DocLattice attempt to solve this by sending every document through a processing pipeline that provides a uniform
...
typically a word) in the OCRed document are stored as JSONs with their page and positional information. In DocLattice, we
...
Thus, in DocLattice, every document has three files associated with it - the original pdf, a json file (the "PAWLs layer"),
