...
| Programmatic utility | `doclatticeserver/utils/corpus_forking.py` (`build_fork_corpus_task`) | Same shell-creation flow without GraphQL/permission plumbing; returns a Celery signature for callers (tests) to apply |
| Object collector | `doclatticeserver/utils/corpus_collector.py` (`collect_corpus_objects`) | Single source of truth for which IDs the fork task receives (see #816) |
| Fork task | `doclatticeserver/tasks/fork_tasks.py` (`fork_corpus`) | Async Celery task that performs the clone inside one `transaction.atomic()` |
...
- `doclatticeserver/tests/test_corpus_fork_round_trip.py` — multi-generation
...
- `doclatticeserver/tests/test_corpus_forking.py` — original feature-level
  test that round-trips a real V1 export through `build_fork_corpus_task`.

### Running

```bash
docker compose -f test.yml run django pytest doclatticeserver/tests/test_corpus_fork_round_trip.py
docker compose -f test.yml run django pytest doclatticeserver/tests/test_corpus_forking.py
```
