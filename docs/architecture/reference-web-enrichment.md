...
(`doclatticeserver/enrichment/grammars.py::_hts` /
...
- **Identifier document citations** (CBP ruling numbers, both the prefixed
  ...
  (`doclatticeserver/enrichment/resolver.py::document_identity_candidates`)
...
`EnrichmentService` (`doclatticeserver/enrichment/services/enrichment_service.py`)
...
- **Within a document**, `LLMCitationExtractor.aextract`
  (`doclatticeserver/enrichment/llm_citation_extractor.py`) enumerates its
