...
for deployments whose acquisition and crawling happen outside DocLattice —
...
`AuthorityNamespace` (`doclatticeserver/annotations/models.py`) is the registry
...
All access routes through `doclatticeserver/enrichment/services/` (per the
...
(`doclatticeserver/enrichment/services/authority_permissions.py`) is the **one**
