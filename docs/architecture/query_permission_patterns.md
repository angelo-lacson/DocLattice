...
|-------|---------------|------|---------|
| Corpus | `PermissionedTreeQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py:30-86` | Guardian via `get_objects_for_user` |
| Document | `DocumentQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py:193-229` | Guardian permission table lookup |
| Document (manager) | `BaseVisibilityManager.visible_to_user` | `doclatticeserver/shared/Managers.py:40-203` | Guardian + model-specific prefetches |
| Annotation | `AnnotationQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py:245-379` | Guardian on doc/corpus + analysis/extract privacy |
| Note | `NoteQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py:368-399` | Document + corpus inheritance |
| UserFeedback | `UserFeedbackQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py:112-133` | Creator + public + annotation visibility |
| Fallback | `PermissionQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py:137-184` | Creator + public only (no guardian) |
...
- Implementation: `doclatticeserver/shared/Managers.py:103-118`
...
- **Tier 2 — Request-scoped optimizer.** `PermissionQueryOptimizer` lives
  ...
  `doclatticeserver/constants/permissioning.py`), keyed by
...
- **No production call site assigns guardian permissions to a `Group`.**
  ...
  `doclatticeserver/utils/permissioning.py:182-248` route through the
...
- Implementation: `doclatticeserver/shared/QuerySets.py:251-291` (AnnotationQuerySet)

### IDOR Protection
Mutations use `visible_to_user()` filtering with unified error messages to prevent object ID enumeration.
- Single-object check: `Manager.user_can()` / `obj.user_can()` (`doclatticeserver/utils/permissioning.py` — `_default_user_can`)
- Permission assignment: `doclatticeserver/utils/permissioning.py:20-163` (`set_permissions_for_obj_to_user`)
...
- Implementation: `doclatticeserver/shared/QuerySets.py:276-278` (AnnotationQuerySet visibility_filter)
