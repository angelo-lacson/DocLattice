...
Reference for how DocLattice filters querysets by user permissions.
...
Tier 1  Service packages         doclatticeserver/<app>/services/
...
`doclatticeserver/tests/architecture/test_graphql_service_layer.py` and
should be extended to other consumer directories as they migrate.

## Migration Recipes — "I just got an `doclattice.E001` error, what do I type?"

If you landed here from a Django `manage.py` error or a pytest failure
mentioning `doclattice.E001` / "Tier-0 permission primitive", the
recipe below is the answer. Both the test and the system check route
through the same helper (`doclatticeserver.shared.architecture_audit.
...
`doclatticeserver.shared.services.base.BaseService` is the generic surface
...
|-------|---------------|------|
| Corpus | `PermissionedTreeQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py` |
| Document | `DocumentQuerySet.visible_to_user` / `BaseVisibilityManager.visible_to_user` | `doclatticeserver/shared/QuerySets.py`, `doclatticeserver/shared/Managers.py` |
| Annotation / Note / Relationship | `*QuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py` |
| UserFeedback | `UserFeedbackQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py` |
| Fallback | `PermissionQuerySet.visible_to_user` | `doclatticeserver/shared/QuerySets.py` |
...
agree by `doclatticeserver/tests/permissioning/test_authorization_invariants.py`.
...
Implementation: `doclatticeserver/shared/QuerySets.py` (AnnotationQuerySet, NoteQuerySet).
...
`doclatticeserver/shared/Managers.py`, `doclatticeserver/shared/QuerySets.py`.
...
Implementation: `doclatticeserver/utils/permission_optimizer.py`,
`doclatticeserver/utils/permissioning.py:get_users_permissions_for_obj`,
`doclatticeserver/utils/permissioning.py:_default_user_can`.

Tests: `doclatticeserver/tests/permissioning/test_permission_optimizer.py`.
...
`doclatticeserver/shared/QuerySets.py` (AnnotationQuerySet).
...
non-superusers. Implementation: `doclatticeserver/shared/QuerySets.py`
...
(`doclatticeserver/shared/architecture_audit.py`) so a violation cannot
slip through either:

1. **Django system check** (`doclatticeserver/shared/checks.py`,
   registered via `users/apps.py:ready()`). Fires on every management
   command — `manage.py runserver`, `migrate`, `shell`, `test`,
   `check --deploy`. Emits `doclattice.E001` and blocks the command
...
2. **Pytest invariant**
   (`doclatticeserver/tests/architecture/test_graphql_service_layer.py`).
...
The allowlist
(`doclatticeserver.shared.architecture_audit.ALLOWED_FILES`) is empty.
...
user-context surfaces — MCP tools (`doclatticeserver/mcp/`), LLM tools
(`doclatticeserver/llms/tools/`), REST views, and user-context Celery
tasks — but the scanner does **not** cover them yet, and they still contain
correct-but-inline Tier-0 calls. A green `doclattice.E001` means "no
