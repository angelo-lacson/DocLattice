...
DocLattice implements a **dual-tree versioning system** based on first principles of immutability and separation of concerns. The Content Tree (Document) tracks what files contain and how content evolves. The Path Tree (DocumentPath) tracks where files live and their lifecycle events. Trees only grow—they never shrink—providing complete audit trails and time-travel capabilities.
