...
- `doclatticeserver/tests/test_schema_parity.py` — structurally compares
...
- `doclatticeserver/utils/ids.py` — opaque global IDs
...
- `core/relay.py` — the `Node` interface, a **type registry** (`register_type`) mapping type names →
  ...
  - A type whose singular `xxx(id:)` lookup must stay permission-scoped
    ...
    `DocLatticeNode`; `MessageType`/`chatMessage`, `datacell`, `badge`,
