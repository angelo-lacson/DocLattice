...
This document describes the architecture for bulk importing documents into DocLattice via ZIP files, including folder structure preservation and document relationship creation.
...
```python
from doclatticeserver.utils.relationship_file_parser import parse_relationship_file
...
| Component | Path |
|-----------|------|
| ZIP Security | `doclatticeserver/utils/zip_security.py` |
| Relationship Parser | `doclatticeserver/utils/relationship_file_parser.py` |
| Metadata Parser | `doclatticeserver/utils/metadata_file_parser.py` |
| Import Task | `doclatticeserver/tasks/import_tasks.py` |
...
docker compose -f test.yml run django pytest doclatticeserver/tests/test_relationship_file_parser.py -v

# Metadata parser tests
docker compose -f test.yml run django pytest doclatticeserver/tests/test_metadata_file_parser.py -v

# ZIP security tests
docker compose -f test.yml run django pytest doclatticeserver/tests/test_zip_security.py -v
```
...
docker compose -f test.yml run django pytest doclatticeserver/tests/test_zip_import_integration.py -v

# Run only relationship tests
docker compose -f test.yml run django pytest doclatticeserver/tests/test_zip_import_integration.py::TestRelationshipFileImport -v

# Run only metadata tests
docker compose -f test.yml run django pytest doclatticeserver/tests/test_zip_import_integration.py::TestMetadataFileImport -v
```
