# DocLattice LLM Framework

Last Updated: 2026-01-09

DocLattice's API for creating document and corpus agents.
...
The `doclatticeserver.llms` module provides several high-level API entry points:
...
```python
from doclatticeserver.llms import agents, embeddings, vector_stores, tools
...
# from doclatticeserver.corpuses.models import Corpus # For corpus_obj
# from doclatticeserver.documents.models import Document # For document_obj
...
# from doclatticeserver.conversations.models import Conversation, ChatMessage
...
`doclatticeserver/llms/context_guardrails.py` provides three layers of
...
WS_ERROR_CONTEXT_EXHAUSTED` (see `doclatticeserver.constants.context_guardrails`).

Tunable constants live in `doclatticeserver/constants/context_guardrails.py`.

#### Memory Curation (cross-conversation learning)

`doclatticeserver/tasks/memory_tasks.py` is a Celery-based pipeline that
...
(`doclatticeserver/llms/tools/core_tools/memory.py`), so a corpus's agents
...
`doclatticeserver/llms/tools/core_tools/`. Highlights:
...
In addition, `doclatticeserver/llms/tools/web_search_tools.py` exposes a
...
```python
from doclatticeserver.llms.tools import create_document_tools # Convenience function
...
[`tools/core_tools/extracts_and_analyzers.py`](../../../doclatticeserver/llms/tools/core_tools/extracts_and_analyzers.py)
...
The framework automatically converts tools to the appropriate format. `CoreTool` objects are converted to framework-specific wrappers via `UnifiedToolFactory.create_tool()` in [`tools/tool_factory.py`](../../../doclatticeserver/llms/tools/tool_factory.py). For PydanticAI, this produces `PydanticAIToolWrapper` instances (see [`tools/pydantic_ai_tools.py`](../../../doclatticeserver/llms/tools/pydantic_ai_tools.py)).
...
Some tools might be *dangerous* (e.g. deleting data) or simply require legal review before execution. DocLattice supports a **durable approval gate** that pauses the agent right before such a tool would run, persists all state, and lets a human approve or reject the call at a later time—even after a server restart.
...
```python
from doclatticeserver.llms import tools
...
- **Veto gate**: [`tools/pydantic_ai_tools.py`](../../../doclatticeserver/llms/tools/pydantic_ai_tools.py) — `PydanticAIToolWrapper._maybe_raise()` raises `ToolConfirmationRequired` before tool execution
- **Pause/resume**: [`agents/pydantic_ai_agents.py`](../../../doclatticeserver/llms/agents/pydantic_ai_agents.py) — `PydanticAICoreAgent.resume_with_approval()` handles state validation and continuation
...
from doclatticeserver.llms.agents.pydantic_ai_agents import PydanticAICorpusAgent
...
- **PydanticAI**: `ask_document_tool` (inline in [`PydanticAICorpusAgent.create()`](../../../doclatticeserver/llms/agents/pydantic_ai_agents.py)) explicitly forwards child events
- **CoreAgentBase**: `_emit_observer_event()` helper (in [`agents/core_agents.py`](../../../doclatticeserver/llms/agents/core_agents.py)) ensures safe forwarding — observer exceptions are caught and logged, never breaking the stream
...
The framework provides sync embeddings APIs via `doclatticeserver.llms.embeddings`:

```python
from doclatticeserver.llms import embeddings
...
Vector stores provide both sync and async search methods, accessible via `doclatticeserver.llms.vector_stores`.

```python
from doclatticeserver.llms import vector_stores
from doclatticeserver.llms.vector_stores.core_vector_stores import VectorSearchQuery
...
> 
> LlamaIndex integration has been removed from the DocLattice LLM framework. All agents now use PydanticAI as the underlying framework. If you need LlamaIndex support, you can implement a custom adapter by following the `CoreAgent` protocol in `agents/core_agents.py`.
...
from doclatticeserver.llms.vector_stores.pydantic_ai_vector_stores import PydanticAIAnnotationVectorStore
...
`doclatticeserver/tests/test_agent_system_prompt_assembly.py`.
...
Use the `a`-prefixed async versions defined in `doclatticeserver/llms/tools/core_tools/`. The sync versions exist for testing and lightweight helpers only.
...
**Location**: `doclatticeserver/consumers/unified_agent_consumer.py`
...
**Location**: [`doclatticeserver/llms/agents/agent_factory.py`](../../../doclatticeserver/llms/agents/agent_factory.py)
...
**Location**: [`doclatticeserver/llms/tools/pydantic_ai_tools.py`](../../../doclatticeserver/llms/tools/pydantic_ai_tools.py)
...
|------|---------|----------------------|
| [`tools/tool_factory.py`](../../../doclatticeserver/llms/tools/tool_factory.py) | Core tool definition with permission flags | `CoreTool` dataclass with `requires_approval`, `requires_corpus`, `requires_write_permission` flags |
| [`tools/tool_registry.py`](../../../doclatticeserver/llms/tools/tool_registry.py) | Metadata registry of available tools | `AVAILABLE_TOOLS` tuple of `ToolDefinition` entries (metadata-only, not used for tool resolution) |
| [`agents/agent_factory.py`](../../../doclatticeserver/llms/agents/agent_factory.py) | Agent creation with tool filtering | `_user_has_write_permission()`, inline filtering in `create_document_agent()`/`create_corpus_agent()` |
| [`agents/pydantic_ai_agents.py`](../../../doclatticeserver/llms/agents/pydantic_ai_agents.py) | Tool assembly with flag assignment | `PydanticAIDocumentAgent.create()`, `PydanticAICorpusAgent.create()` |
| [`tools/pydantic_ai_tools.py`](../../../doclatticeserver/llms/tools/pydantic_ai_tools.py) | Runtime permission validation | `_check_user_permissions()` (module-level), `_validate_resource_id_params()`, `PydanticAIToolWrapper` |
| [`utils/permissioning.py`](../../../doclatticeserver/utils/permissioning.py) | Core permission checking utilities | `_default_user_can()` (the body behind `Manager.user_can` / `obj.user_can`) |
...
```python
from doclatticeserver.llms.tools.tool_factory import CoreTool, ToolMetadata
...
```python
from doclatticeserver.llms.tools import create_document_tools # Assuming this is already imported
# from doclatticeserver.llms.tools.tool_factory import CoreTool # For custom_tools if defined elsewhere
...
- The `deduplicate_tools()` utility in `doclatticeserver/utils/tools.py` documents these security implications
...
# from doclatticeserver.llms import vector_stores # For vector_stores.create()
...
# See doclatticeserver/llms/vector_stores/core_vector_stores.py for the interface
...
# Typically created via doclatticeserver.llms.vector_stores.create().
```
...
1. **`AgentConfig`** (in [`agents/core_agents.py`](../../../doclatticeserver/llms/agents/core_agents.py)): Low-level dataclass used internally by agent implementations.
2. **`AgentAPI.for_document()` / `AgentAPI.for_corpus()`** (in [`api.py`](../../../doclatticeserver/llms/api.py)): High-level API with individual keyword arguments. This is the recommended entry point.
...
```python
# from doclatticeserver.documents.models import Document # For Document.DoesNotExist
# from doclatticeserver.llms.agents.core_agents import FinalEvent, UnifiedStreamResponse # For streaming errors
...
The framework includes a set of moderation tools for threaded discussion management in [`tools/moderation_tools.py`](../../../doclatticeserver/llms/tools/moderation_tools.py). These are used by moderation agents to manage conversations:
...
Moderation tools can be included by name via the `tools=` parameter (e.g., `tools=["lock_thread", "get_thread_context"]`). The `_resolve_tools()` function in [`api.py`](../../../doclatticeserver/llms/api.py) maps these names to their async versions automatically.
...
[`completions.py`](../../../doclatticeserver/llms/completions.py)'s
`agenerate_text`:

```python
from doclatticeserver.llms.completions import agenerate_text
...
the credential-aware [`model_factory`](../../../doclatticeserver/llms/model_factory.py),
...
- [`vector_stores/core_conversation_vector_stores.py`](../../../doclatticeserver/llms/vector_stores/core_conversation_vector_stores.py) — Framework-agnostic conversation search
- [`vector_stores/pydantic_ai_conversation_vector_stores.py`](../../../doclatticeserver/llms/vector_stores/pydantic_ai_conversation_vector_stores.py) — PydanticAI-specific wrapper
...
import pytest
from doclatticeserver.llms import agents
from doclatticeserver.llms.agents.core_agents import UnifiedChatResponse
# from doclatticeserver.documents.models import Document # For test setup
# from doclatticeserver.corpuses.models import Corpus # For test setup
...
1. **Add Framework Enum** (in [`types.py`](../../../doclatticeserver/llms/types.py)):
   ```python
...
2. **Implement Agent Adapters**:
   ...
   - Define classes inheriting from `CoreAgentBase` (from [`agents/core_agents.py`](../../../doclatticeserver/llms/agents/core_agents.py)).
...
3. **Integrate into `UnifiedAgentFactory`** (in [`agents/agent_factory.py`](../../../doclatticeserver/llms/agents/agent_factory.py)):
   ```python
   # elif framework == AgentFramework.LANGCHAIN:
   #     from doclatticeserver.llms.agents.langchain_agents import LangChainDocumentAgent # Or CorpusAgent
   #     if for_document:
...
4. **Add Tool Support**:
   ...
   - Update [`tools/tool_factory.py`](../../../doclatticeserver/llms/tools/tool_factory.py) (`UnifiedToolFactory`) to handle the new framework.

5. **Add Vector Store Support**:
   - Create `vector_stores/langchain_vector_stores.py`.
   - Implement adapter around `CoreAnnotationVectorStore` (see [`vector_stores/core_vector_stores.py`](../../../doclatticeserver/llms/vector_stores/core_vector_stores.py)).
   - Update [`vector_stores/vector_store_factory.py`](../../../doclatticeserver/llms/vector_stores/vector_store_factory.py).
...
By following these steps, you can extend the DocLattice LLM framework to support new LLM technologies while maintaining the consistent, rich API with conversation management, source tracking, and structured responses.
...
This framework represents the evolution of DocLattice' LLM capabilities, providing a foundation for sophisticated document analysis while maintaining simplicity and elegance in its API design.
...
Tools are **not** pre-defined as static `CoreTool` instances. They are assembled dynamically during agent creation in [`PydanticAIDocumentAgent.create()`](../../../doclatticeserver/llms/agents/pydantic_ai_agents.py) and [`PydanticAICorpusAgent.create()`](../../../doclatticeserver/llms/agents/pydantic_ai_agents.py). The table below reflects the tool names as registered by those factory methods.

Additionally, a smaller subset of **static** `CoreTool` instances is available via [`create_document_tools()`](../../../doclatticeserver/llms/tools/tool_factory.py) in `tool_factory.py`. These are lower-level tools used when building agents outside the standard factory path.
...
([`doclatticeserver/enrichment/`](../../../doclatticeserver/enrichment/)):
...
([`core_tools/graph_navigation.py`](../../../doclatticeserver/llms/tools/core_tools/graph_navigation.py))
...
([harness](../../../doclatticeserver/benchmarks/traversal_benchmark.py)).

#### Image Tools (from `create_document_tools()`)

The static [`create_document_tools()`](../../../doclatticeserver/llms/tools/tool_factory.py) function also includes these image tools. They are available when using `create_document_tools()` directly but are **not** currently added by the standard `PydanticAIDocumentAgent.create()` factory:
...
When passing tools by name to `agents.for_document()`, the [`_resolve_tools()`](../../../doclatticeserver/llms/api.py) function in `api.py` supports these string aliases:
...
DocLattice lets you pick the LLM that powers an agent at five layers:
...
4. **Install-wide runtime default** — `PipelineSettings.default_llm`, set live by superusers in System Settings. Threaded into the resolver as `settings_default` (via `doclatticeserver.pipeline.utils.get_default_llm_spec`, an ORM read).
5. **Settings default** — `settings.DEFAULT_LLM` (preferred) → legacy `settings.OPENAI_MODEL` (back-compat) → hard fallback `"gpt-4o"`.

The resolver lives in [`doclatticeserver/llms/llm_registry.py`](../../../doclatticeserver/llms/llm_registry.py):

```python
from doclatticeserver.llms.llm_registry import resolve_model_spec
...
The [agent factory](../../../doclatticeserver/llms/agents/agent_factory.py) calls the resolver automatically. Callers pick the slot that matches semantics:

- `agents.for_*(model="anthropic:claude-opus-4-6")` — per-call override. Wins over every persisted default.
- `agents.for_*(agent_preferred_llm=agent_config.preferred_llm)` — per-agent override. Wins over the corpus default but yields to a per-call `model=`. The @-mention task ([`agent_tasks.py`](../../../doclatticeserver/tasks/agent_tasks.py)) and the delegation sub-agent path ([`delegation_tools.py`](../../../doclatticeserver/llms/tools/delegation_tools.py)) use this slot.
...
`doclatticeserver/tests/test_llm_runtime_retargeting.py` pins all of this —
...
LLM providers are registered as pluggable [pipeline components](../../pipelines/pipeline_overview.md) under [`doclatticeserver/pipeline/llm_providers/`](../../../doclatticeserver/pipeline/llm_providers/). Each subclass of [`BaseLLMProvider`](../../../doclatticeserver/pipeline/base/llm_provider.py) declares:
...
Resolution is **DB-wins / env-fallback**, applied by [`doclatticeserver/llms/model_factory.py`](../../../doclatticeserver/llms/model_factory.py):
