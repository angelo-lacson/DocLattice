...
For use cases that don't need the full agent framework (tool calling, conversation persistence, etc.), [`client.py`](../../../doclatticeserver/llms/client.py) provides a lightweight `SimpleLLMClient`:

```python
from doclatticeserver.llms.client import create_client, Provider
