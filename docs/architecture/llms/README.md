...
```python
from doclatticeserver.llms.tools.tool_factory import CoreTool # Can also use doclatticeserver.llms.tools.create_from_function
...
```python
from doclatticeserver.llms import agents
from doclatticeserver.llms.types import StreamObserver
...
```python
# Examples from doclatticeserver/llms/tools/tool_registry.py
tools_registry = [
...
All three validation layers check the same permission rules using `user_has_permission_for_obj()` from `doclatticeserver.utils.permissioning`:
...
|------|---------|----------------------|
| `doclatticeserver/llms/tools/tool_factory.py` | Core tool definition with permission flags | `CoreTool`, `requires_write_permission` flag |
| `doclatticeserver/llms/tools/tool_registry.py` | Registry of all built-in tools with their permissions | `tools_registry` list with flag definitions |
| `doclatticeserver/llms/agents/agent_factory.py` | Agent creation with tool filtering | `_filter_tools_by_write_permission()`, `_user_has_write_permission()` |
| `doclatticeserver/llms/tools/pydantic_ai_tools.py` | Runtime permission validation wrapper | `PydanticAIToolWrapper`, `_check_user_permissions()` |
| `doclatticeserver/utils/permissioning.py` | Core permission checking utilities | `user_has_permission_for_obj()` |
| `doclatticeserver/consumers/unified_agent_consumer.py` | WebSocket consumer with entry validation | Initial READ permission checks |
...
```python
from doclatticeserver.llms.agents.core_agents import get_default_config, AgentConfig
...
```python
from doclatticeserver.llms import agents
from doclatticeserver.llms.agents.core_agents import AgentError
# from doclatticeserver.documents.models import Document # For Document.DoesNotExist
# from doclatticeserver.corpuses.models import Corpus # For Corpus.DoesNotExist
...
2. **Implement Agent Adapters**:
   ...
   - Inside this file, define classes for your document and/or corpus agents. These classes **must** inherit from `CoreAgentBase` (from `doclatticeserver.llms.agents.core_agents.py`).
