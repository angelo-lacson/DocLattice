...
All validation layers use `user_has_permission_for_obj()` from [`doclatticeserver/utils/permissioning.py`](../../../doclatticeserver/utils/permissioning.py). Layer 2 (factory) checks `PermissionTypes.CRUD` for write-gating, while Layer 3 (runtime) checks `PermissionTypes.READ` on every tool call.
...
| [`utils/permissioning.py`](../../../doclatticeserver/utils/permissioning.py) | Core permission checking utilities | `user_has_permission_for_obj()` |
