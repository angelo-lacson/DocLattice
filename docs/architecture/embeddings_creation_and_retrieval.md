...
DocLattice uses a **dual embedding strategy** to enable both global cross-corpus search and corpus-optimized search:
...
This strategy is implemented in the embedding tasks (`doclatticeserver/tasks/embeddings_task.py`) which automatically handle the dual embedding creation.

### Creating Embeddings

1. **Generate the Embeddings (Text → Vector)**
   - Use the unified utility function **`generate_embeddings_from_text(text, embedder_path=None)`** or **`agenerate_embeddings_from_text(text, embedder_path=None)`** from [`doclatticeserver/utils/embeddings.py`](../../doclatticeserver/utils/embeddings.py).
   - These functions:
...
Embedder settings are managed via the **`PipelineSettings`** database singleton (see [`doclatticeserver/documents/models.py`](../../doclatticeserver/documents/models.py)):
...
Available embedder implementations in [`doclatticeserver/pipeline/embedders/`](../../doclatticeserver/pipeline/embedders/):
...
1. **`CoreAnnotationVectorStore`** - Framework-Agnostic Business Logic
   - Located in [`doclatticeserver/llms/vector_stores/core_vector_stores.py`](../../doclatticeserver/llms/vector_stores/core_vector_stores.py)
...
Models that store embeddings can use `VectorSearchViaEmbeddingMixin` from [`doclatticeserver/shared/mixins.py`](../../doclatticeserver/shared/mixins.py) to enable vector similarity searches (the model must also use `HasEmbeddingMixin`). See the source file for usage examples.
...
2. **PydanticAI Adapter** - `PydanticAIAnnotationVectorStore`
   - Located in [`doclatticeserver/llms/vector_stores/pydantic_ai_vector_stores.py`](../../doclatticeserver/llms/vector_stores/pydantic_ai_vector_stores.py)
...
3. **PydanticAI Tool Creation**
   - Use `create_vector_search_tool()` from [`pydantic_ai_vector_stores.py`](../../doclatticeserver/llms/vector_stores/pydantic_ai_vector_stores.py) to create vector search tools for PydanticAI agents.
...
4. **PydanticAI Response Format**
   ...
   - See [`pydantic_ai_vector_stores.py`](../../doclatticeserver/llms/vector_stores/pydantic_ai_vector_stores.py) for the complete response structure.
...
1. **Database Indexes**: The `Annotation` model includes composite indexes for optimal query performance. See [`doclatticeserver/annotations/models.py`](../../doclatticeserver/annotations/models.py) for index definitions.
