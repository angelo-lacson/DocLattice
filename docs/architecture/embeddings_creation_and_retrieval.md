...
1. **Generate the Embeddings (Text → Vector)**
   - Use the unified utility function **`generate_embeddings_from_text(text, embedder_path=None)`** or **`agenerate_embeddings_from_text(text, embedder_path=None)`** from `doclatticeserver/utils/embeddings.py`.
   - These functions:
...
1. **`CoreAnnotationVectorStore`** - Framework-Agnostic Business Logic
   - Located in `doclatticeserver/llms/vector_stores/core_vector_stores.py`
...
```python
from doclatticeserver.shared.mixins import VectorSearchViaEmbeddingMixin
...
3. **PydanticAI Adapter** - `PydanticAIAnnotationVectorStore`
   - Located in `doclatticeserver/llms/vector_stores/pydantic_ai_vector_stores.py`
