...
1. **Generate the Embeddings (Text → Vector)**
   - Use the unified utility function **`generate_embeddings_from_text(text, corpus_id, mimetype)`** from `doclatticeserver/utils/embeddings.py`.
   - This function:
...
1. **`CoreAnnotationVectorStore`** - Framework-Agnostic Business Logic
   - Located in `doclatticeserver/llms/core_vector_stores.py`
   - Contains all the business logic for vector search without dependencies on specific agent frameworks
   - Key components:
     ```python
     from doclatticeserver.llms.core_vector_stores import (
...
1. **LlamaIndex Adapter** - `DjangoAnnotationVectorStore`
   - Located in `doclatticeserver/llms/vector_stores.py`
