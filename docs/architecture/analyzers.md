...
## Overview
DocLattice supports document analyzers that run as Celery tasks within the main application. These analyzers can automatically process documents and create annotations.
...
analyzer = Analyzer.objects.create(
    ...
    task_name="doclatticeserver.tasks.module.task_name",  # Python import path
    creator=user,
...
For complete implementation details, see the decorator source at [`doclatticeserver/shared/decorators.py`](../../doclatticeserver/shared/decorators.py) and the detailed walkthrough at [`docs/walkthrough/advanced/register-doc-analyzer.md`](../walkthrough/advanced/register-doc-analyzer.md).
...
`UserFeedback` system (`doclatticeserver/feedback/models.py`).
