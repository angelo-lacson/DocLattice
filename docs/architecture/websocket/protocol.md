...
DocLattice uses WebSocket connections to deliver real-time streaming output from AI agents and real-time push of notifications and thread updates. This document covers the WebSocket URL surface, the auth handshake, the message envelope, and the lifecycle of an agent conversation.

The source of truth is [`config/asgi.py`](https://github.com/angelo-lacson/DocLattice/blob/main/config/asgi.py) (routing) and the three consumers under [`config/websocket/consumers/`](https://github.com/angelo-lacson/DocLattice/blob/main/config/websocket/consumers/).
...
All three consumers run behind the shared [`JWTAuthMiddleware`](https://github.com/angelo-lacson/DocLattice/blob/main/config/websocket/middleware.py) (wired in `config/asgi.py:120-122`). It accepts:
...
| LLM context exhausted | `ASYNC_ERROR` with `error = WS_ERROR_CONTEXT_EXHAUSTED` (see `doclatticeserver.constants.context_guardrails`). |
