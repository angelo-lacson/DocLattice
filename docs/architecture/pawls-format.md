...
PAWLs (Page-Aware Word-Level Segmentation) is DocLattice' format for representing document structure with precise token positioning. Each page in a document has tokens (text or image) with bounding box coordinates that enable:
...
```python
from doclatticeserver.utils.compact_pawls import is_compact_pawls_format
...
|------------|------|--------------|
| Parser output | `doclatticeserver/pipeline/base/parser.py` | Compacts after parsing completes |
| Worker uploads | `doclatticeserver/worker_uploads/tasks.py` | Compacts imported PAWLs data |
| V2 import | `doclatticeserver/utils/import_v2.py` | Compacts during v2 corpus import |
| Legacy import | `doclatticeserver/utils/importing.py` | Compacts during legacy corpus import |
| Import tasks | `doclatticeserver/tasks/import_tasks.py` | Compacts during async import jobs |
...
Defined in `doclatticeserver/constants/pawls.py`:
...
|-------|------|-----------|
| Backend (Python) | `doclatticeserver/utils/compact_pawls.py` | Encode + Decode |
| Frontend (TypeScript) | `frontend/src/utils/compactPawls.ts` | Decode only |
| Constants | `doclatticeserver/constants/pawls.py` | Shared constants |
| Tests | `doclatticeserver/tests/test_compact_pawls.py` | Full round-trip coverage |

### Comparison with Annotation Compact Format

A similar v2 compression strategy exists for annotation JSON payloads in `doclatticeserver/annotations/compact_json.py`. It uses the same design principles (version marker, format-agnostic accessor, auto-compact on write) but applies range-encoding for token indices instead of array-based tokens. The annotation format achieves ~75% storage reduction.
