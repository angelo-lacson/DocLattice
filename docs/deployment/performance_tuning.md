...
This document captures what we've measured and changed about the DocLattice
...
1. Wipe the DB:
   ...
   docker compose -f local.yml run --rm django python manage.py shell -c "
     ...
     from doclatticeserver.extracts.models import Extract, Fieldset, Datacell, Column
