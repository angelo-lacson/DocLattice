...
**File**: `doclatticeserver/corpuses/managers.py` (add to existing or create)
...
from doclatticeserver.shared.Managers import BaseVisibilityManager
...
**File**: `doclatticeserver/tasks/corpus_tasks.py`
...
def process_fieldset_action(
    ...
    """
    from doclatticeserver.corpuses.models import CorpusAction, CorpusActionExecution
...
**File**: `doclatticeserver/types/dicts.py`
...
Add to `DocLatticeExportDataJsonV2Type`:

```python
class DocLatticeExportDataJsonV2Type(TypedDict):
    # ... existing V2 fields ...
...
**File**: `doclatticeserver/tasks/export_tasks_v2.py`
...
def build_corpus_export(
    ...
    if include_action_trail:
        from doclatticeserver.utils.export_v2 import package_action_trail
...
**File**: `doclatticeserver/tests/test_corpus_action_execution.py`
...
from doclatticeserver.tests.fixtures import create_test_user, create_test_corpus
...
class TestCorpusActionExecutionIntegration(TransactionTestCase):
    ...
    def test_fieldset_action_creates_execution_trail(self):
        ...
        column = Column.objects.create(
            ...
            name="test_column",
            output_type="str",
            task_name="doclatticeserver.tasks.extract_tasks.llm_extract_cell",
            creator=self.user,
        )
    ...
    def test_analyzer_action_creates_execution_trail(self):
        ...
        analyzer = Analyzer.objects.create(
            name="Test Analyzer",
            task_name="doclatticeserver.tasks.analyzer_tasks.test_analyzer",
            creator=self.user,
        )
        action = CorpusAction.objects.create(
            name="Analyzer Action",
