...
**Files:**
- Modify: `doclatticeserver/documents/models.py:793-834` (add field after `component_settings`)
- Create: `doclatticeserver/documents/migrations/0035_add_enabled_components_to_pipeline_settings.py`

**Step 1: Add the field to the model**

In `doclatticeserver/documents/models.py`, add after the `component_settings` field (line 826):
...
Run: `docker compose -f local.yml run --rm django python manage.py shell -c "from doclatticeserver.documents.models import PipelineSettings; ps = PipelineSettings.get_instance(use_cache=False); print('enabled_components:', ps.enabled_components); print('is_component_enabled test:', ps.is_component_enabled('nonexistent'))"`
Expected: Shows the populated list and `True` (empty list = all enabled for fresh installs).

**Step 7: Commit**

```bash
git add doclatticeserver/documents/models.py doclatticeserver/documents/migrations/0035_*.py
...
**Files:**
- Modify: `doclatticeserver/tests/test_pipeline_settings.py`
...
Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_pipeline_settings.py::TestEnabledComponents -v`
Expected: All 4 tests PASS.

**Step 3: Commit**

```bash
git add doclatticeserver/tests/test_pipeline_settings.py
...
Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_pipeline_component_queries.py doclatticeserver/tests/test_pipeline_settings.py -v`
...
Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_pipeline_settings.py -v`
Expected: All tests PASS.
...
Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_pipeline_settings.py::TestEnabledComponentsMutation -v`
Expected: All tests PASS.
...
Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_pipeline_settings.py doclatticeserver/tests/test_pipeline_component_queries.py -v`
Expected: All PASS.
