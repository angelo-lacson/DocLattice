...
**File:** `doclatticeserver/pipeline/parsers/docling_parser_rest.py:386`
...
**File:** `doclatticeserver/utils/pdf_token_extraction.py`
...
**File:** `doclatticeserver/utils/pdf_token_extraction.py:385`
...
**File:** `doclatticeserver/llms/tools/image_tools.py` (NEW)
...
from doclatticeserver.utils.pdf_token_extraction import (
    get_image_as_base64,
    get_image_data_url,
)
from doclatticeserver.types.dicts import PawlsImageTokenPythonType
...
**File:** `doclatticeserver/llms/tools/tool_factory.py`

Add imports and tool definitions:
```python
from doclatticeserver.llms.tools.image_tools import (
...
**File:** `doclatticeserver/llms/tools/image_tools.py`
...
class Query(graphene.ObjectType):
    ...
    def resolve_document_image(self, info, document_id, page_index, image_index):
        from doclatticeserver.llms.tools.image_tools import get_document_image_with_permission
...
**File:** `doclatticeserver/documents/apps.py`

```python
def ready(self):
    import doclatticeserver.documents.signals  # noqa
```
...
**File:** `doclatticeserver/tests/test_pdf_image_extraction_integration.py` (NEW)
...
from doclatticeserver.utils.pdf_token_extraction import extract_images_from_pdf
...
class TestImageToolsPermissions(TestCase):
    ...
    def test_get_document_image_requires_read_permission(self):
        ...
        from doclatticeserver.llms.tools.image_tools import get_document_image_with_permission
        from doclatticeserver.tests.factories import UserFactory, DocumentFactory
...
**File:** `doclatticeserver/tests/test_document_image_queries.py` (NEW)
...
**File:** `doclatticeserver/tests/test_embedder_multimodal.py` (NEW)
