...
|------|--------|---------------|
| `doclatticeserver/constants/document_processing.py` | Modify (line 25) | Add `EMBEDDING_API_BATCH_SIZE` constant |
| `doclatticeserver/pipeline/base/embedder.py` | Modify (after line 234) | Add `embed_texts_batch()` default on `BaseEmbedder` |
| `doclatticeserver/pipeline/embedders/sent_transformer_microservice.py` | Modify (after line 155) | Add `embed_texts_batch()` override on `MicroserviceEmbedder` |
| `doclatticeserver/pipeline/embedders/test_embedder.py` | Modify (after line 84, after line 137) | Add `embed_texts_batch()` to both test embedders |
| `doclatticeserver/tasks/embeddings_task.py` | Modify (lines 484-521) | Add `_batch_embed_text_annotations()` helper, refactor task |
| `doclatticeserver/tests/test_batch_embedding.py` | Create | Integration + mock tests for batch embedding |
...
**Files:**
- Modify: `doclatticeserver/constants/document_processing.py:25`

- [ ] **Step 1: Add constant after `MAX_REEMBED_TASKS_PER_RUN`**

In `doclatticeserver/constants/document_processing.py`, after line 25 (`MAX_REEMBED_TASKS_PER_RUN = 500`), add:
...
Run: `cd /home/jman/Code/DocLattice && python -c "from doclatticeserver.constants.document_processing import EMBEDDING_API_BATCH_SIZE; print(EMBEDDING_API_BATCH_SIZE)"`
Expected: `50`

- [ ] **Step 3: Commit**

```bash
git add doclatticeserver/constants/document_processing.py
...
**Files:**
- Modify: `doclatticeserver/pipeline/base/embedder.py` (after line 234)
...
Run: `cd /home/jman/Code/DocLattice && docker compose -f local.yml run --rm django python -c "from doclatticeserver.pipeline.base.embedder import BaseEmbedder; print(hasattr(BaseEmbedder, 'embed_texts_batch'))"`
Expected: `True`

- [ ] **Step 3: Commit**

```bash
git add doclatticeserver/pipeline/base/embedder.py
...
**Files:**
- Modify: `doclatticeserver/pipeline/embedders/sent_transformer_microservice.py` (after line 155)
...
Run: `cd /home/jman/Code/DocLattice && docker compose -f local.yml run --rm django python -c "from doclatticeserver.pipeline.embedders.sent_transformer_microservice import MicroserviceEmbedder; print(hasattr(MicroserviceEmbedder, 'embed_texts_batch'))"`
Expected: `True`

- [ ] **Step 3: Commit**

```bash
git add doclatticeserver/pipeline/embedders/sent_transformer_microservice.py
...
**Files:**
- Modify: `doclatticeserver/pipeline/embedders/test_embedder.py` (after line 84, after line 137)
...
Run: `cd /home/jman/Code/DocLattice && docker compose -f local.yml run --rm django python -c "from doclatticeserver.pipeline.embedders.test_embedder import TestEmbedder, TestMultimodalEmbedder; e = TestEmbedder(); r = e.embed_texts_batch(['hello', 'world']); print(len(r), len(r[0]))"`
Expected: `2 384`

- [ ] **Step 4: Commit**

```bash
git add doclatticeserver/pipeline/embedders/test_embedder.py
...
**Files:**
- Modify: `doclatticeserver/tasks/embeddings_task.py` (insert before `calculate_embeddings_for_annotation_batch` at line 423)
...
Run: `cd /home/jman/Code/DocLattice && docker compose -f local.yml run --rm django python -c "from doclatticeserver.tasks.embeddings_task import _batch_embed_text_annotations; print('OK')"`
Expected: `OK`

- [ ] **Step 4: Commit**

```bash
git add doclatticeserver/tasks/embeddings_task.py
...
**Files:**
- Modify: `doclatticeserver/tasks/embeddings_task.py:484-521` (the `embedder_path` branch of the for loop)
...
Run: `cd /home/jman/Code/DocLattice && docker compose -f local.yml run --rm django python -c "from doclatticeserver.tasks.embeddings_task import calculate_embeddings_for_annotation_batch; print('OK')"`
Expected: `OK`

- [ ] **Step 3: Run existing tests to verify no regressions**

Run: `cd /home/jman/Code/DocLattice && docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_personal_corpus.py::TestCalculateEmbeddingsForAnnotationBatch -v --keepdb`
...
**Files:**
- Create: `doclatticeserver/tests/test_batch_embedding.py`
...
from doclatticeserver.annotations.models import Annotation, Embedding
from doclatticeserver.documents.models import Document
from doclatticeserver.pipeline.embedders.test_embedder import TestEmbedder
...
TEST_EMBEDDER_PATH = (
    "doclatticeserver.pipeline.embedders.test_embedder.TestEmbedder"
)
...
class TestBatchTaskIntegration(TestCase):
    ...
    @patch(
        "doclatticeserver.tasks.embeddings_task._apply_dual_embedding_strategy"
    )
    ...
    def test_batch_task_partitions_multimodal(self):
        ...
        multimodal_path = "doclatticeserver.pipeline.embedders.test_embedder.TestMultimodalEmbedder"

        with patch(
            "doclatticeserver.tasks.embeddings_task._create_embedding_for_annotation"
        ) as mock_individual:
            mock_individual.return_value = True

            with patch(
                "doclatticeserver.tasks.embeddings_task._batch_embed_text_annotations"
            ) as mock_batch:
                result = calculate_embeddings_for_annotation_batch(
...
Run: `cd /home/jman/Code/DocLattice && docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_batch_embedding.py -v --keepdb`
...
```bash
git add doclatticeserver/tests/test_batch_embedding.py
...
Run: `cd /home/jman/Code/DocLattice && pre-commit run --all-files`
Expected: All hooks pass. If formatting changes are needed, stage and commit them.

- [ ] **Step 2: Run full affected test suite**

Run: `cd /home/jman/Code/DocLattice && docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_batch_embedding.py doclatticeserver/tests/test_personal_corpus.py doclatticeserver/tests/test_multimodal_embedder_unit.py -v --keepdb`
