...
Run: `python -c "from doclatticeserver.constants.document_processing import EMBEDDING_API_BATCH_SIZE; print(EMBEDDING_API_BATCH_SIZE)"`
Expected: `50`
...
Run: `docker compose -f local.yml run --rm django python -c "from doclatticeserver.pipeline.base.embedder import BaseEmbedder; print(hasattr(BaseEmbedder, 'embed_texts_batch'))"`
Expected: `True`
...
Run: `docker compose -f local.yml run --rm django python -c "from doclatticeserver.pipeline.embedders.sent_transformer_microservice import MicroserviceEmbedder; print(hasattr(MicroserviceEmbedder, 'embed_texts_batch'))"`
Expected: `True`
...
Run: `docker compose -f local.yml run --rm django python -c "from doclatticeserver.pipeline.embedders.test_embedder import TestEmbedder, TestMultimodalEmbedder; e = TestEmbedder(); r = e.embed_texts_batch(['hello', 'world']); print(len(r), len(r[0]))"`
Expected: `2 384`
...
Run: `docker compose -f local.yml run --rm django python -c "from doclatticeserver.tasks.embeddings_task import _batch_embed_text_annotations; print('OK')"`
Expected: `OK`
...
Run: `docker compose -f local.yml run --rm django python -c "from doclatticeserver.tasks.embeddings_task import calculate_embeddings_for_annotation_batch; print('OK')"`
Expected: `OK`

- [ ] **Step 3: Run existing tests to verify no regressions**

Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_personal_corpus.py::TestCalculateEmbeddingsForAnnotationBatch -v --keepdb`
...
Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_batch_embedding.py -v --keepdb`
...
Run: `docker compose -f test.yml run --rm django pytest doclatticeserver/tests/test_batch_embedding.py doclatticeserver/tests/test_personal_corpus.py doclatticeserver/tests/test_multimodal_embedder_unit.py -v --keepdb`
