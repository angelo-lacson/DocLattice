...
Extract the CAML (Corpus Article Markup Language) parser and React renderer from DocLattice into two standalone npm packages under the `@os-legal` scope.

## Motivation

CAML is a human-readable markdown superset for rendering legal articles and knowledge bases. It currently lives inside the DocLattice frontend. Extracting it enables:

- Reuse across projects without pulling in DocLattice
...
## DocLattice Integration (Consumer Side)
...
- Backend `MarkdownParser` (`doclatticeserver/pipeline/parsers/oc_markdown_parser.py`) — unrelated to the frontend library.
