...
The public MCP interface (`doclatticeserver/mcp/`) is meant to let an AI use an
DocLattice corpus as a low-friction knowledge tool. A hands-on evaluation of
...
2. **`search_corpus` always empty** — two compounding bugs in `tools.py:search_corpus`:
   - It searches **document-level** embeddings (`get_corpus_documents().search_by_embedding(...)`),
     filtered by the corpus's current `embedder_path`. DocLattice embeddings live
...
- `doclatticeserver/mcp/tools.py` — tool implementations
- `doclatticeserver/mcp/server.py` — scoped + global tool **schemas** and the
  tool-dispatch error formatting
- `doclatticeserver/mcp/formatters.py` — response formatters
- `doclatticeserver/constants/mcp.py` — new size/limit constants
- `doclatticeserver/mcp/tests/test_mcp.py` — tests
...
Add to `doclatticeserver/constants/mcp.py` (alongside `MAX_THREAD_MESSAGE_LENGTH`):
...
Extend `doclatticeserver/mcp/tests/test_mcp.py` (seed corpus + documents +
