...
DocLattice enforces a small number of cross-cutting rules at the
...
### Service-layer access in `config/graphql/` (`doclattice.E001`)

**Rule.** GraphQL resolvers / mutations / types in `config/graphql/`
must reach models through `doclatticeserver/<app>/services/` —
...
(`doclatticeserver/mcp/`), LLM tools (`doclatticeserver/llms/tools/`),
...
`doclattice.E001` means "no inline Tier-0 in `config/graphql/`," not
...
**Where it fires.**
- **Django system check** —
  `doclatticeserver/shared/checks.py::check_graphql_service_layer`,
  registered from `doclatticeserver/users/apps.py::UsersConfig.ready`.
  Emits `Error` with id `doclattice.E001`. Blocks every
...
**Source of truth.**
`doclatticeserver/shared/architecture_audit.py` holds the AST scanner,
...
(`doclatticeserver.shared.architecture_audit.ALLOWED_FILES`) is a
...
1. **Write the scanner** in `doclatticeserver/shared/`. Pure
...
3. **Register a Django system check** in
   `doclatticeserver/shared/checks.py` (or a sibling module). Use
   `@register("architecture")` and a fresh `doclattice.EXXX` id.
4. **Wire it into an existing `AppConfig.ready`** (the `users` app is
   the canonical home — see how the Phase-6 check is wired in
   `doclatticeserver/users/apps.py`).
5. **Add the pytest invariant** under
   `doclatticeserver/tests/architecture/`. Reuse the same scanner;
