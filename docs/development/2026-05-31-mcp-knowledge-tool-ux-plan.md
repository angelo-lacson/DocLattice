...
**Goal:** Make an DocLattice corpus usable as a low-friction knowledge tool over the public MCP interface — working passage+block search, content-searchable annotations, relationship enumeration, bounded full-text, and honest metadata/errors.

**Architecture:** All changes confined to `doclatticeserver/mcp/` (tool impls, schemas, formatters) + `doclatticeserver/constants/mcp.py` + one new `RelationshipService.get_corpus_relationships` method. Data access reuses existing services (`AnnotationService`, `RelationshipService`, `CoreRelationshipVectorStore`, `CorpusDocumentService`). No new models/migrations.

**Tech Stack:** Django 4.x, pgvector, the MCP `Server`/`Tool` types, `sync_to_async` dispatch (MCP tool handlers are sync). Tests: `doclatticeserver/mcp/tests/test_mcp.py` (Django `TestCase`, `_MCPAsyncRunMixin`).
...
- `doclatticeserver/constants/mcp.py` — add snippet/window size constants.
- `doclatticeserver/mcp/formatters.py` — add `format_search_passage`, `format_search_block`; trim `format_annotation`.
- `doclatticeserver/mcp/tools.py` — rewrite `search_corpus`; extend `list_annotations`; add `list_relationships`; update `get_corpus_info`, `get_document_text`; register `list_relationships` in `get_scoped_tool_handlers`/`TOOL_HANDLERS`.
- `doclatticeserver/mcp/server.py` — update tool schemas (scoped `get_scoped_tool_definitions` + global `list_tools`); humanize `_format_tool_error_text`.
- `doclatticeserver/annotations/services/relationship_service.py` — add `get_corpus_relationships`.
- `doclatticeserver/utils/files.py` — add `read_field_file_text` (Task 6, only if #1841 not yet merged).
- `doclatticeserver/mcp/tests/test_mcp.py` — tests per task.
- `CHANGELOG.md` — entry.
...
**Files:**
- Modify: `doclatticeserver/constants/mcp.py`
- Modify: `doclatticeserver/mcp/formatters.py`
- Test: `doclatticeserver/mcp/tests/test_mcp.py` (`MCPFormattersTest`)

- [ ] **Step 1: Add constants**

```python
# doclatticeserver/constants/mcp.py — append
...
```python
# doclatticeserver/mcp/formatters.py — add near top (after imports)
...
```python
# doclatticeserver/mcp/formatters.py — add new formatters
def format_search_passage(annotation: "Annotation", similarity_score=None) -> dict:
    ...
    from doclatticeserver.constants.mcp import MCP_SEARCH_SNIPPET_MAX_CHARS
...
def format_search_block(result) -> dict:
    ...
    """
    from doclatticeserver.constants.mcp import MCP_BLOCK_SNIPPET_MAX_CHARS
...
```python
# doclatticeserver/mcp/formatters.py — replace format_annotation body
...
- [ ] **Step 5: Run** `docker compose -f test.yml run --rm django pytest doclatticeserver/mcp/tests/test_mcp.py::MCPFormattersTest -q` → PASS (add a `self.annotation` to `MCPFormattersTest.setUpTestData` if missing).

- [ ] **Step 6: Commit**

```bash
git add doclatticeserver/constants/mcp.py doclatticeserver/mcp/formatters.py doclatticeserver/mcp/tests/test_mcp.py
...
**Files:**
- Modify: `doclatticeserver/mcp/tools.py:219-306` (`search_corpus`, `_text_search_fallback`)
- Modify: `doclatticeserver/mcp/server.py` (`get_scoped_tool_definitions` search_corpus schema + global `list_tools` search_corpus schema)
- Test: `doclatticeserver/mcp/tests/test_mcp.py` (`MCPToolsSearchTest`)
...
```python
# doclatticeserver/mcp/tools.py — replace search_corpus and _text_search_fallback
def search_corpus(
    corpus_slug: str,
    query: str,
    limit: int = 10,
    ...
    if granularity in ("block", "both") and query_vector:
        from doclatticeserver.llms.vector_stores.core_relationship_vector_store import (
...
Delete `_text_search_fallback` (now dead). Verify no other references: `grep -rn "_text_search_fallback" doclatticeserver/`.
...
`docker compose -f test.yml run --rm django pytest doclatticeserver/mcp/tests/test_mcp.py::MCPToolsSearchTest -q` → PASS.

- [ ] **Step 7: Commit**

```bash
git add doclatticeserver/mcp/tools.py doclatticeserver/mcp/server.py doclatticeserver/mcp/tests/test_mcp.py
...
**Files:**
- Modify: `doclatticeserver/mcp/tools.py:163-216` (`list_annotations`)
- Modify: `doclatticeserver/mcp/server.py` scoped + global `list_annotations` schema
- Test: `doclatticeserver/mcp/tests/test_mcp.py` (`MCPToolsAnnotationsTest`)
...
**Files:**
- Modify: `doclatticeserver/annotations/services/relationship_service.py` (add `get_corpus_relationships`)
- Modify: `doclatticeserver/mcp/tools.py` (add `list_relationships`, register in `get_scoped_tool_handlers`)
- Modify: `doclatticeserver/mcp/server.py` (scoped + global tool schema + global `TOOL_HANDLERS` registration)
- Modify: `doclatticeserver/mcp/formatters.py` (add `format_relationship`)
- Test: `doclatticeserver/mcp/tests/test_mcp.py` (new `MCPToolsRelationshipsTest`)
...
def get_corpus_relationships(cls, corpus_id: int, user, structural=None) -> QuerySet:
    ...
    from doclatticeserver.annotations.models import StructuralAnnotationSet
...
def format_relationship(rel) -> dict:
    ...
    from doclatticeserver.constants.mcp import MCP_REL_ANNOTATION_TEXT_MAX_CHARS
...
```bash
git add doclatticeserver/annotations/services/relationship_service.py doclatticeserver/mcp/tools.py doclatticeserver/mcp/server.py doclatticeserver/mcp/formatters.py doclatticeserver/mcp/tests/test_mcp.py
...
**Files:**
- Modify: `doclatticeserver/mcp/tools.py:555-610` (`get_corpus_info`)
- Modify: `doclatticeserver/mcp/server.py:359-376` (`_format_tool_error_text`) + tool descriptions
- Test: `doclatticeserver/mcp/tests/test_mcp.py`
...
**Files:**
- Modify: `doclatticeserver/mcp/tools.py:121-160` (`get_document_text`)
- Modify: `doclatticeserver/mcp/server.py` scoped + global `get_document_text` schema
- Modify (only if #1841 NOT merged): `doclatticeserver/utils/files.py` (add `read_field_file_text`)
- Test: `doclatticeserver/mcp/tests/test_mcp.py` (`MCPToolsDocumentsTest`)
...
def get_document_text(
    ...
    from doclatticeserver.constants.mcp import (
...
```bash
git add doclatticeserver/mcp/tools.py doclatticeserver/mcp/server.py doclatticeserver/mcp/tests/test_mcp.py doclatticeserver/utils/files.py
...
- [ ] **Step 2:** Run the full MCP suite: `docker compose -f test.yml run --rm django pytest doclatticeserver/mcp/tests/test_mcp.py -n 4 --dist loadscope -q` → all PASS.
