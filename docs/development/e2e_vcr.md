...
`doclatticeserver/utils/vcr_replay.py` exposes a `maybe_vcr_cassette()` context manager that is wrapped around the agent invocation in `doclatticeserver/tasks/data_extract_tasks.py::doc_extract_query_task`. When the relevant env vars are unset, the manager is a no-op — production behavior is unchanged.
...
1. Bring up the local stack with the VCR env vars set so the worker container picks them up:
   ...
   OC_LLM_VCR_CASSETTE=/app/doclatticeserver/tests/fixtures/cassettes/e2e_extract_pdf_workflow/extract.yaml \
...
3. Verify the cassette landed under `doclatticeserver/tests/fixtures/cassettes/e2e_extract_pdf_workflow/extract.yaml`. Commit it.
