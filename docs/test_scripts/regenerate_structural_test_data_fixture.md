...
One-time regeneration of `doclatticeserver/tests/fixtures/test_data.json`
...
1. From the repository root, run the transform below. It asserts the fixture
   ...
   FIXTURE = Path("doclatticeserver/tests/fixtures/test_data.json")
...
2. Confirm the diff only touches the migrated objects:
   ...
   ```bash
   git diff --stat doclatticeserver/tests/fixtures/test_data.json
   ```
...
3. Validate the regenerated fixture loads cleanly by running any
   ...
   docker compose -f test.yml run --rm --no-deps django \
     pytest doclatticeserver/tests/test_note_tree.py --reuse-db
   ```
