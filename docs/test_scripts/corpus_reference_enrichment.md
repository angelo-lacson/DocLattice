...
## Purpose
Prove the corpus reference enrichment engine (`doclatticeserver/enrichment/`)
...
- An S-1 corpus loaded. NOTE: the local dev DB "thrashes" — corpus IDs get
  ...
  *external manual activity* against the shared local Postgres — the
  ...
  `types=list(__import__('doclatticeserver.enrichment.constants', fromlist=['ALL_REFERENCE_TYPES']).ALL_REFERENCE_TYPES)`
...
docker compose -f local.yml run --rm django python manage.py shell -c "
import warnings; warnings.simplefilter('ignore')
from doclatticeserver.annotations.models import CorpusReference, Annotation
