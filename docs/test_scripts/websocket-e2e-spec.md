...
`doclatticeserver/tests/test_websocket_auth.py` exercises the
...
```
doclatticeserver/tests/fixtures/cassettes/e2e_websocket_auth/agent_chat.yaml
```
...
1. Start the local stack with VCR in `record` mode and a real key:
   ...
   export OC_LLM_VCR_CASSETTE=/app/doclatticeserver/tests/fixtures/cassettes/e2e_websocket_auth/agent_chat.yaml
...
3. Confirm the cassette file exists and commit it:
   ```bash
   git add doclatticeserver/tests/fixtures/cassettes/e2e_websocket_auth/agent_chat.yaml
   ```
