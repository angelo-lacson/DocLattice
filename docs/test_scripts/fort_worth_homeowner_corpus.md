...
- A running local stack (`local.yml`) with `postgres` on the **local** volume
  (`doclattice_local_postgres_data`, not the test volume).
...
```bash
docker cp ~/Code/Authorities/authority-packs/fort_worth doclattice-django-1:/tmp/fw_pack
docker compose -f local.yml -p doclattice exec -T django \
...
```bash
docker compose -f local.yml -p doclattice exec -d django sh -c \
...
```bash
docker compose -f local.yml -p doclattice exec -T django sh -c \
...
```bash
docker compose -f local.yml -p doclattice exec -T django python manage.py shell -c "
from doclatticeserver.annotations.models import AuthorityNamespace as AN
...
```bash
docker cp <staged-pdfs>/ doclattice-django-1:/tmp/fw_corpus
docker compose -f local.yml -p doclattice exec -d django sh -c \
...
```bash
docker cp run_gold_questions.py doclattice-django-1:/tmp/
docker compose -f local.yml -p doclattice exec -T django \
