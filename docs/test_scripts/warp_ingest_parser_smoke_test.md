...
- The `warp-ingest` container running and reachable from the Django container.
  ...
  ```
- A fixture PDF, e.g. `doclatticeserver/tests/fixtures/files/doc_1_pdf_file.pdf`.
...
1. Confirm the service parses the fixture directly (sanity check of the
   ...
   curl -s -H "X-API-Key: smoke-test-key" \
     -F "file=@doclatticeserver/tests/fixtures/files/doc_1_pdf_file.pdf;type=application/pdf" \
...
2. Drive the parser through DocLattice end-to-end. From the Django container
   ...
   from doclatticeserver.pipeline.parsers.warp_ingest_parser import WarpIngestParser
   ...
   with open('doclatticeserver/tests/fixtures/files/doc_1_pdf_file.pdf','rb') as fh:
