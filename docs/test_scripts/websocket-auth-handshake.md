...
async def main():
    async with websockets.connect(
        ...
        subprotocols=['doclattice.jwt.v1', '$TOKEN']
    ) as ws:
...
3. Inspect the `notification-updates/` WebSocket request:
   ...
   - **Request Headers:** must show `Sec-WebSocket-Protocol: doclattice.jwt.v1, <jwt>`.
   - **Response Headers:** must show `Sec-WebSocket-Protocol: doclattice.jwt.v1`.
