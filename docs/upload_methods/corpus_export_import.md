...
DocLattice can export an entire corpus -- documents, annotations, labels,
folder structure, and configuration -- as a ZIP archive. This archive can be
imported into the same or a different DocLattice instance to recreate the
corpus in full.
...
3. Choose an export format:
   - **DocLattice** (default) -- full archive for backup/transfer
