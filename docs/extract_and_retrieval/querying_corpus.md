...
3. On the first question the consumer lazily initialises a **`CoreAgent`** via `doclatticeserver.llms.agents.for_corpus(...)`.
...
* Agent factory implementation – `doclatticeserver.llms.agents.agent_factory.UnifiedAgentFactory`.
* Event classes – `doclatticeserver.llms.agents.core_agents.*Event`.
