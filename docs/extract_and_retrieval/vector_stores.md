...
DocLattice uses a flexible vector store architecture that provides compatibility with multiple agent frameworks (PydanticAI, etc.) while maintaining a clean separation between business logic and framework-specific adapters.
...
```python
from doclatticeserver.llms.vector_stores import UnifiedVectorStoreFactory
...
This design pattern is applied throughout DocLattice to create a comprehensive, framework-agnostic foundation for AI-powered document analysis.
