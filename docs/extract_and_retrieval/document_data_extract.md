...
* Each cell's `column.task_name` is used to look up the Celery task to invoke (defaults to `doclatticeserver.tasks.data_extract_tasks.doc_extract_query_task`).
