...
DocLattice provides real-time, conversational AI for answering questions about entire document collections (corpora). Using WebSockets for streaming responses and unified agents for consistent behavior, users can interactively explore their documents with instant feedback.
...
3. **Agent Initialization**: Consumer lazily creates a `CoreAgent` via `doclatticeserver.llms.agents.for_corpus(...)`
...
```python
from doclatticeserver.llms.agents import UnifiedAgentFactory
