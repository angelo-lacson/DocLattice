...
DocLattice transforms any collection of documents into a **spreadsheet-like data grid**. You define *what* to extract via a **Fieldset**, and the system:
...
All models are defined in [`doclatticeserver/extracts/models.py`](../../doclatticeserver/extracts/models.py).
...
The `data_type` field supports: STRING, TEXT, BOOLEAN, INTEGER, FLOAT, DATE, DATETIME, URL, EMAIL, CHOICE, MULTI_CHOICE, and JSON. See `METADATA_DATA_TYPES` in the [models file](../../doclatticeserver/extracts/models.py).
