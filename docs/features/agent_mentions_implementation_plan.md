...
|------|--------|-------------|
| `doclatticeserver/agents/models.py` | Modified | Added `slug` field (lines 66-73) |
| `doclatticeserver/agents/migrations/0003_add_slug_field.py` | Created | Migration for slug field |
| `doclatticeserver/agents/migrations/0004_ensure_default_agents.py` | Created | Data migration for default agents |
...
| `doclatticeserver/conversations/models.py` | Modified | Added `mentioned_agents` M2M field |
| `doclatticeserver/conversations/migrations/0011_add_mentioned_agents_field.py` | Created | Migration for M2M field |
| `doclatticeserver/utils/mention_parser.py` | Modified | Added agent URL pattern parsing |
...
| `doclatticeserver/tasks/agent_tasks.py` | Created | Celery tasks for agent response generation |
...
**Phase 5:**
- Created `doclatticeserver/tasks/agent_tasks.py` with Celery tasks:
...
1. **AgentConfiguration Model** (`doclatticeserver/agents/models.py:53-146`)
...
2. **ChatMessage Model** (`doclatticeserver/conversations/models.py:603-787`)
...
**File:** `doclatticeserver/agents/migrations/XXXX_create_default_global_agents.py`
...
**File:** `doclatticeserver/agents/models.py`
...
class UnifiedAgentConsumer(AsyncWebsocketConsumer):
    ...
    async def _handle_chat_message(self, data):
        ...
        from doclatticeserver.llms import agents as agent_factory
        from asgiref.sync import sync_to_async
...
**File:** `doclatticeserver/utils/mention_parser.py`
...
**File:** `doclatticeserver/conversations/models.py`
...
**File:** `doclatticeserver/tasks/agent_tasks.py` (NEW)
...
if result.get("agents_linked", 0) > 0:
    from doclatticeserver.tasks.agent_tasks import generate_agent_response
...
**File:** `doclatticeserver/tests/test_agent_mentions.py` (NEW)
...
| `doclatticeserver/tasks/agent_tasks.py` | Celery tasks for agent response generation | 5 |
...
| `doclatticeserver/tests/test_agent_mentions.py` | Backend tests | 8 |
...
| File | Changes | Phase |
|------|---------|-------|
| `doclatticeserver/agents/models.py` | Add `slug` field | 0 |
| `doclatticeserver/agents/migrations/` | Add default global agents | 0 |
...
| `doclatticeserver/conversations/models.py` | Add `mentioned_agents` M2M | 3 |
| `doclatticeserver/utils/mention_parser.py` | Parse `@agent:` mentions | 3 |
