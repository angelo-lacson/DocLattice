...
This document provides practical examples of using the unified metadata fields API in DocLattice.

**Important Note on Field Naming**: DocLattice uses Graphene Django which automatically converts Python `snake_case` field names to GraphQL `camelCase`. Throughout this document:
...
DocLattice uses a **unified data model** for both extracted data and manual metadata entry:
...
class DocLatticeMetadataClient:
...
if __name__ == "__main__":
    client = DocLatticeMetadataClient(
        api_url="https://your-doclattice-instance.com/graphql/",
