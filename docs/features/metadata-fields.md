...
DocLattice provides a unified data model for both extracted data and manual metadata entry at the corpus level. This feature allows you to define structured metadata schemas for documents within a corpus, ensuring consistent data collection and validation across your document sets.
...
DocLattice uses the same `Fieldset` model for both metadata schemas and extraction schemas, but they serve different purposes:
