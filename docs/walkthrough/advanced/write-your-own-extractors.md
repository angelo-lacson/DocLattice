...
## Refresher on What an DocLattice Data Extractor Does
...
This is actually retrieved dynamically from the backend from the tasks in `doclatticeserver.tasks.data_extract_tasks.py`. Every **celery task** in this python module will show up in the GUI,
...
def oc_llama_index_doc_query(cell_id, similarity_top_k=15, max_token_length: int = 512):
    """
    DocLattice' default LlamaIndex and Marvin-based data extract pipeline to run queries specified for a
...
## How DocLattice Integrates with LlamaIndex
...
LlamaIndex use DocLattice as a vector store. Our `DjangoAnnotationVectorStore` in
`doclatticeserver/llms/vector_stores.py` lets you quickly write a LlamaIndex agent or question answering pipeline
that can pull directly from the rich annotations and structural data (like annotation positions, layout class -
e.g. header - and more) in DocLattice. If you want to learn more about LlamaIndex's vector stores, see more in
...
been done by humans (and machines) in an DocLattice corpus to label and tag documents. We're getting the best of
...
Now, we determine the Annotation instance ids we retrieved so these can be linked to the datacell. On the DocLattice
...
DocLattice uses `multi-qa-MiniLM-L6-cos-v1` to generate its embeddings (for now, we can make this modular as well).
...
### Step 3 - Instantiate DocLattice Vector Store
...
Next, let's use DocLattice as a LlamaIndex query engine:
