...
DocLattice supports both exporting and importing corpuses. This functionality is disabled on the public
...
2. **Choose an export format.** Two formats are available:
   - **DocLattice** (default) — Produces a complete archive containing annotated PDFs, PAWLs layers, and full metadata. This is the recommended format for backing up or transferring corpuses between DocLattice instances.
...
## DocLattice Export Format Specification

The DocLattice export is a zip archive containing:
...
* `annotated_docs` (dict): Maps PDF filenames to DocLatticeDocExport objects with annotations for that document.
...
* `corpus` (DocLatticeCorpusType): Metadata about the exported corpus, with fields:
...
* `label_set` (DocLatticeLabelSetType): Metadata about the label set, with fields:
    - `id` (int)
    - `title` (string)
    - `description` (string)
...
### DocLatticeDocExport Format

Each document in `annotated_docs` is represented by an DocLatticeDocExport object with fields:

* `doc_labels` (list[string]): List of document label names applied to this doc
* `labelled_text` (list[DocLatticeAnnotationPythonType]): List of text annotations
...
### DocLatticeAnnotationPythonType Format
...
* `annotation_json` (dict): Maps page numbers to DocLatticeSinglePageAnnotationType

### DocLatticeSinglePageAnnotationType Format
