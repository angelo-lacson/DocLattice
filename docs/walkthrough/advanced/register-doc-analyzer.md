...
The `@doc_analyzer_task` decorator is an integral part of the DocLattice CorpusAction system, which automates
...
write and deploy simple, span-based analytics directly within the DocLattice ecosystem.
...
3. You want a quick and easy way to integrate custom analysis into the DocLattice workflow.
...
1. **Simplicity**: It abstracts away much of the complexity of interacting with the DocLattice system.
2. **Integration**: Tasks are automatically integrated into the CorpusAction workflow.
3. **Consistency**: It ensures that your analysis task produces outputs in a format that DocLattice can readily use.
4. **Error Handling**: It provides built-in error handling and retry mechanisms.

By using this decorator, you can focus on writing the core analysis logic while the DocLattice system handles the
...
data they receive, and how their outputs are processed by the DocLattice system.
...
The `@doc_analyzer_task` decorator in DocLattice expects the decorated function's return value to match a specific
...
directly to the data models used in the DocLattice system.
...
**Relationship to DocLattice Models**:
...
- It can trigger further actions or notifications in the DocLattice system.
...
the broader DocLattice data model, ensuring that the results of document analysis are properly stored, linked, and
...
```python
from doclatticeserver.shared.decorators import doc_analyzer_task
...
By using the `@doc_analyzer_task` decorator, this function is automatically integrated into the DocLattice system,
