# Understanding the PAWLs Format in DocLattice

The DocLattice project utilizes the PAWLs format for representing documents and their annotations. PAWLs is designed to provide a consistent and structured way to store text and layout information for complex documents like contracts, scientific papers, and newspapers.

## PAWLs Layers

In DocLattice, every document is processed through a pipeline that extracts and structures text and layout information into three files:
...
DocLattice allows users to annotate documents using the PAWLs layer. Annotations are stored as a dictionary mapping page numbers to annotation data:

```python
Dict[int, DocLatticeSinglePageAnnotationType]
```

The `DocLatticeSinglePageAnnotationType` represents the annotation data for a single page:
...
The PAWLs format in DocLattice provides a powerful and flexible way to represent and annotate complex documents. By extracting and structuring text and layout information, PAWLs enables efficient and accurate document analysis and annotation tasks. The consistent structure and layout awareness of PAWLs make it an essential component of the DocLattice project.
...
This sample demonstrates the structure and content of a PAWLs layer JSON file, which serves as the foundation for annotation and analysis tasks in the DocLattice project.
