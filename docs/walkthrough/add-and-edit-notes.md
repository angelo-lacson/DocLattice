...
This guide walks you through the note-taking features in DocLattice, including creating notes, editing them, and managing version history.

## Overview

Notes in DocLattice are markdown-based annotations that can be attached to specific locations in documents. Each note maintains a complete version history, allowing you to track changes over time and revert to previous versions if needed.
