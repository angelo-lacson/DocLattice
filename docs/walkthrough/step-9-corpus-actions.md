...
more about the underlying architecture, check out our [deep dive on `CorpusActions`](../architecture/doclattice-corpus-actions.md).
...
1. First, we assume you put a properly written and decorated task in `doclatticeserver.tasks.doc_analysis_tasks.py`.
2. Second, you need to create and configure an Analyzer via the Django admin panel. Click on the `+Add` button next to
   ...
   e.g. `doclatticeserver.tasks.doc_analysis_tasks.contract_not_contract`,
