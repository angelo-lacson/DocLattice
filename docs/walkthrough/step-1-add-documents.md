In order to do anything, you need to add some documents to DocLattice.
...
DocLattice works with PDFs and several other document formats including Word documents (.docx),
PowerPoint presentations (.pptx), and plain text files. It doesn't matter if PDFs are OCRed or not as
DocLattice can perform its own OCR to ensure consistent quality and outputs. Once you've added documents
...
Click upload to upload the documents to DocLattice. **Note:** Once the documents are
...
on the page. This is what powers the DocLattice annotator and allows us to create both layout-aware
