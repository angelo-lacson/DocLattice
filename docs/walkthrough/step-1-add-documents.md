...
DocLattice works with PDFs only (as this helps us have a single file type with predictable
data structures, formats, etc.). In the future, we'll add functionality to convert other files to PDF,
but, for now, please use PDFs. It doesn't matter if they are OCRed or not as DocLattice performs its own
...
powers DocLattice annotator and allows us to create both layout-aware and text-only annotations. **While
