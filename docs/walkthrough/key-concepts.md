...
This page walks through the core data types and patterns in DocLattice. It is the recommended starting point for the [walkthrough](step-1-add-documents.md).

## Data Types

DocLattice organises knowledge into a small set of first-class entities:
...
DocLattice uses [`django-guardian`](https://django-guardian.readthedocs.io/) for per-object permissions, layered with custom queryset managers:
...
DocLattice uses [Graphene](https://graphene-python.org/) to serve GraphQL. The GraphiQL playground lives at the application root URL `/graphql/`:
...
The schema is checked into the repository at [`schema.graphql`](https://github.com/angelo-lacson/DocLattice/blob/main/schema.graphql); GraphiQL's built-in docs explorer is the easiest way to browse types interactively.
