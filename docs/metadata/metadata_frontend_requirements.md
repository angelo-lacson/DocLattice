...
DocLattice uses a **unified data model** where the same components handle both extracted data and manual metadata:
...
**Important**: DocLattice uses Graphene Django which automatically converts Python `snake_case` field names to GraphQL `camelCase`. For example:
