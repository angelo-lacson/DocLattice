# Building Document Imports for DocLattice (with CAML Descriptions)

This guide walks through building a complete document import for DocLattice — from simple document uploads to fully annotated imports with CAML-powered corpus articles. It covers the three import mechanisms, the sidecar annotation format, and how to author CAML descriptions.
...
DocLattice supports three import paths, each suited to different workflows:
...
You can validate a sidecar against its `labels.json` before zipping with `doclatticeserver.utils.validate_export.validate_dumb_anchor_sidecar(sidecar, labels_json)`.
...
Right-click a corpus → **Export** → **DocLattice** format. The export produces a ZIP:
...
```json
{
  "version": "2.0",
  "annotated_docs": {
    "document_a.pdf": { "...DocLatticeDocExport..." },
    "document_b.pdf": { "...DocLatticeDocExport..." }
  },
...
### How CAML Integrates with DocLattice
...
footer:
  nav:
    - label: Documentation
      href: https://github.com/angelo-lacson/DocLattice/tree/main/docs
