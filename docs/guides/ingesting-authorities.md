...
(`doclatticeserver/pipeline/authority_source_providers/`) cover positive law:
...
2. **The source host must be on the public-domain allowlist.** The gate rejects
   ...
   (`doclatticeserver/constants/safe_http.py`) with `blocked_license`. Today
...
- **Payload**: `sections` follows the single section-spec contract
  (`doclatticeserver/enrichment/authorities.py::parse_section_spec` — same
