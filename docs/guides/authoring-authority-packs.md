...
DocLattice install and the authority comes with it.
...
|---|---|---|
| `doclatticeserver/enrichment/data/authority_packs/bolivia/` | v1 | The shipped, installable pack — taxonomy + curated content + a Spanish persona. No scraper: its sources are listing-page, not key-addressable. |
| `doclatticeserver/tests/fixtures/authority_packs/example_utility/` | **v2** | A complete, deliberately fictional multi-corpus pack exercising every v2 slot — slugs, charters, typed metadata, relationships, prefix binding, and a `sources.yaml` source plan. |
...
`doclatticeserver/enrichment/data/authority_mappings.yaml`: `prefixes:`
...
`doclatticeserver/enrichment/constants.py` (`ALL_AUTHORITY_TYPES`).
...
A pack does not need to acquire its corpus content inside DocLattice. For
deployments whose crawlers or collection pipelines run elsewhere, produce a
normal DocLattice corpus-export ZIP for each declared corpus.
...
does not make DocLattice contact the publisher. Provenance collected by the
...
[`scripts/authority_import/README.md`](https://github.com/angelo-lacson/DocLattice/blob/main/scripts/authority_import/README.md);
...
1. Subclass `BaseAuthoritySourceProvider`
   (`doclatticeserver/pipeline/base/base_authority_source_provider.py`):
...
(`doclatticeserver/pipeline/registry.py::_ensure_synthetic_package`). Relative
...
(`from doclatticeserver.enrichment.data.authority_packs.<pack>.helper import …`)
...
`doclatticeserver/enrichment/authority_sources.py`. It is the shared record
...
(`doclatticeserver/pipeline/base/base_authority_discovery_provider.py`)
...
(`doclatticeserver/pipeline/authority_discovery_providers/listing_index_provider.py`),
...
under `doclatticeserver/enrichment/data/authority_packs/` installs it on
...
```bash
docker compose -f local.yml run --rm django python manage.py load_authority_pack \
  --path doclatticeserver/enrichment/data/authority_packs/<pack> \
  --creator <username>
```
...
Tests for the mechanics live in `doclatticeserver/tests/`:
