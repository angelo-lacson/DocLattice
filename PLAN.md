...
Add first-class DOCX support to DocLattice with a parallel ingestion pipeline and rendering tree, modeled after the existing TXT pipeline but with rich DOCX rendering via Docxodus WASM.
...
**File**: `doclatticeserver/pipeline/parsers/docx_parser.py`
...
- Return `DocLatticeDocExport` with:
...
- `doclatticeserver/pipeline/parsers/oc_text_parser.py` — Pattern to follow
- `doclatticeserver/pipeline/base/parser.py` — BaseParser interface
- `doclatticeserver/types/dicts.py` — DocLatticeDocExport type
...
No manual registration needed — auto-discovery will find it in `doclatticeserver/pipeline/parsers/`. Just ensure:
...
In `doclatticeserver/documents/models.py` or initial migration, add default preferred parser mapping:
```python
...
**File**: `doclatticeserver/tests/test_docx_parser.py`
...
| File | Purpose |
|------|---------|
| `doclatticeserver/pipeline/parsers/docx_parser.py` | DocxParser implementation |
...
| `doclatticeserver/tests/test_docx_parser.py` | Backend tests |
