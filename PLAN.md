...
Add first-class DOCX support to DocLattice with a parallel ingestion pipeline and rendering tree. One backend parser (Docxodus .NET microservice), one frontend renderer (Docxodus WASM via react-docxodus-viewer).

**Architecture Summary**:
- **Backend**: `DocxodusServiceParser` — containerized .NET microservice wrapping Docxodus `DocLatticeExporter.Export()`. Returns `DocLatticeDocExport` JSON with structural annotations and character offsets
...
A minimal ASP.NET Core Web API wrapping Docxodus's `DocLatticeExporter.Export()`.
...
Response: DocLatticeDocExport JSON (camelCase)
{
...
app.MapPost("/parse", async (ParseRequest request) =>
{
    try
    {
        ...
        var export = DocLatticeExporter.Export(wmlDoc);
...
**What `DocLatticeExporter.Export()` produces** (already built into Docxodus):
...
**File**: `doclatticeserver/pipeline/parsers/docxodus_parser.py`
...
3. Receive `DocLatticeDocExport` JSON response
...
No manual registration needed — auto-discovery finds `DocxodusServiceParser` in `doclatticeserver/pipeline/parsers/`. The parser declares `supported_file_types = [FileTypeEnum.DOCX]` and gets automatically registered.
...
**File**: `doclatticeserver/pipeline/thumbnailers/docx_thumbnailer.py`
...
**Key file to reference**: `doclatticeserver/pipeline/thumbnailers/text_thumbnailer.py`
...
**File**: `doclatticeserver/tests/test_docxodus_parser.py`
...
- Test that returned `DocLatticeDocExport` integrates correctly with `save_parsed_data()`
...
Create test fixtures in `doclatticeserver/tests/fixtures/`:
  ...
  User uploads .docx
    ...
    → Celery chain:
        ...
        2. ingest_doc → DocxodusServiceParser
               ...
               Microservice calls DocLatticeExporter.Export()
               Returns DocLatticeDocExport JSON:
                 - content (full text)
...
| File | Purpose |
|------|---------|
| `doclatticeserver/pipeline/parsers/docxodus_parser.py` | DocxodusServiceParser (REST microservice client) |
| `doclatticeserver/pipeline/thumbnailers/docx_thumbnailer.py` | Thumbnail generation |
...
| `doclatticeserver/tests/test_docxodus_parser.py` | Backend parser tests |
...
|------|---------------------|
| `doclatticeserver/pipeline/base/file_types.py` | `FileTypeEnum.DOCX` + MIME mapping |
...
| `doclatticeserver/documents/models.py` | `file_type` field, `pdf_file` polymorphic storage |
| `doclatticeserver/documents/versioning.py` | `.docx` extension mapping |
