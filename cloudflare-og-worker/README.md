# DocLattice Social Media Preview Worker

Cloudflare Worker that intercepts social media crawler requests and returns rich Open Graph / Twitter Card metadata for DocLattice deep-links.

## Why This Exists

When someone shares an DocLattice link on Twitter, LinkedIn, Slack, Discord, or other social platforms, the platform's crawler fetches the URL to generate a preview. Since DocLattice is a React SPA, crawlers (which don't execute JavaScript) would only see generic metadata.
...
- DocLattice backend running with the OG metadata GraphQL queries
...
|----------|-------------|---------|
| `SITE_URL` | Base URL of DocLattice site | `http://localhost:3000` |
