...
- **CorpusActionTemplate model** for reusable, agent-based action definitions that auto-clone into new corpuses (`doclatticeserver/corpuses/models.py`)
...
- **Auto-clone signal**: `clone_templates_on_corpus_create` clones all active templates into disabled `CorpusAction` records when a new corpus is created, so users can opt-in per corpus (`doclatticeserver/corpuses/signals.py`)
