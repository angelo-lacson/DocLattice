...
- **Phase F — authorization-invariant gap-fill + permissioning docs refresh** (issue #1660; final phase of the #1637 permission-centralization plan). Closes the remaining gaps in the `user_can` invariant suite and re-points the permissioning docs at the new API.
  ...
  - **Audit finding — group-perm filter/check drift.** The audit's group-shared coverage surfaced a genuine per-model drift: `Manager.user_can` honours group object-permissions (`_default_user_can` resolves with `include_group_permissions=True`) but `QuerySet.visible_to_user` in `doclatticeserver/shared/QuerySets.py` only consults the *user* object-permission table, never the `*groupobjectpermission` table. `Corpus` is unaffected (its queryset handles groups); `Document`, `Annotation`, and `Note` drift. The current behaviour is pinned by `DocumentAuthorizationInvariantsTestCase.test_group_shared_known_drift_user_can_vs_visible_to_user` so it cannot change silently. Closing the drift is a query-path behaviour change left out of this test/docs-only sweep.
...
- **Test suite optimization Phase 3 — stop `BaseFixtureTestCase` reloading the 18 MB fixture on every test** (issue #1711; `doclatticeserver/tests/base.py`, `doclatticeserver/tests/fixtures/test_data.json`, `doclatticeserver/tests/test_fixture_base_classes.py`, `docs/test_scripts/regenerate_structural_test_data_fixture.md`, and 6 dependent test files). `BaseFixtureTestCase` declared `fixtures = ["…/test_data.json"]` **and** extended `TransactionTestCase`, so Django's `loaddata` re-loaded the 7,584-object / 18 MB fixture before *every* test method instead of once per class, and `setUp()` re-ran a structural-annotation migration loop plus per-document file/summary work each time.
...
- **Test suite optimization Phase 2 — convert 160 needlessly-`TransactionTestCase` classes to `TestCase`** (issue #1710; 28 files under `doclatticeserver/tests/` and `doclatticeserver/tests/permissioning/`). `TransactionTestCase` `TRUNCATE`s every table and re-emits the `post_migrate` signal (`create_permissions`, `create_contenttypes`, django-guardian setup) between *every* test method; `TestCase` instead wraps each test in a transaction and rolls it back — far cheaper per-test overhead, and the highest-leverage structural lever for bringing the ~1 hr backend suite under 20 minutes. All 235 classes that extend `TransactionTestCase` directly were audited against the patterns that genuinely require transaction semantics, and the 160 pure-synchronous ORM / GraphQL / permission / management-command classes were reclassified to `django.test.TestCase`.
...
- **Decompose `Documents.tsx` into thinner composer + sibling files** (`frontend/src/views/Documents.tsx`, plus new siblings `Documents.styles.ts`, `DocumentsGridView.tsx`, `DocumentsListView.tsx`, `DocumentsCompactView.tsx`, and an extension to `frontend/src/utils/files.ts`). Final slice of the multi-PR decomposition tracked by #1446 (after #1459 / `ModernDocumentItem`, #1578 / `Corpuses`, #1586 / `ChatMessage`, #1580 / `DocumentKnowledgeBase` partial, #1639 / `ChatTray`). `frontend/src/views/Documents.tsx` had grown to **1,725 lines**. Five extractions land it at **915 lines (-47%)**:
  - **`Documents.styles.ts`** *(new, 581 lines)*: All 50 styled-components moved to a sibling file as named exports. Cross-component selectors (`${DocumentCardWrapper}:hover &` referenced by `CardCheckbox` and `CardMenuButton`; `${ListItem}:hover &` referenced by `ListItemActions`) require same-module scope, so all styles stay in one file (same rule that kept `ModernDocumentItem.styles.ts` monolithic in PR #1459). Renamed three styled-components to free up names taken by the new React components — `DocumentsListContainer` (outer section wrapper) → `DocumentsSection`; `DocumentsListView` (inner list table wrapper) → `DocumentsListContainer`; `DocumentsCompactView` (inner compact list wrapper) → `DocumentsCompactContainer`. Dropped the unused `MetaSeparator` styled-component (declared but never rendered in any view branch — CLAUDE.md "no dead code" rule).
  - **`DocumentsGridView.tsx`** *(new, 145 lines)*: The grid-view branch — `DocumentCardWrapper` cards with checkbox / preview / body / footer. Props: `documents`, `selectedIds`, `activeContextMenuDocId`, `onDocumentClick`, `onSelect`, `onContextMenu`.
  - **`DocumentsListView.tsx`** *(new, 121 lines)*: The list-view branch — table-shaped grid including `ListHeader` with select-all checkbox. Adds `onSelectAll` and pre-computed `allSelected: boolean` to the shared prop shape.
  - **`DocumentsCompactView.tsx`** *(new, 91 lines)*: The compact-view branch — flat row layout. Same props as `DocumentsGridView`.
  - **`frontend/src/utils/files.ts`** *(extended, +27 lines)*: Adds `getDocumentTypeBadge(fileType, title)`, the file-extension/type normalizer previously inlined in `Documents.tsx`. Lives in the existing shared `files.ts` utility module per CLAUDE.md's "utility functions belong in utility files" rule. Consumed by `DocumentsGridView` and `DocumentsListView`.
...
- **Fork ≡ export+import: V2 parity refactor + roundtrip-loss fixes** (`doclatticeserver/tasks/fork_tasks.py`, `doclatticeserver/tasks/export_tasks_v2.py`, `doclatticeserver/utils/{export_v2,import_v2,etl}.py`, `doclatticeserver/tests/{test_corpus_export_import_v2.py,test_corpus_forking.py,test_ingestion_source.py}`, and shared fixture helpers `doclatticeserver/tests/{_corpus_fixture,_corpus_snapshot}.py`). Replaces the legacy bespoke `fork_corpus` machinery with a thin shell that drives the V2 export → V2 import pipeline so the two code paths can no longer drift. `build_corpus_v2_zip` is the pure builder that `package_corpus_export_v2` (Celery task) now wraps; `import_corpus_v2_from_bytes` is the in-process entry point that `fork_corpus` invokes after `build_corpus_v2_zip`. Restores fields that previously dropped on round-trip (manual-metadata `Fieldset` / `Column` / `Datacell` rows, `IngestionSource` rows, `DocumentPath` and `CorpusFolder` snapshots, structural-set membership) and adds a three-roundtrip invariant test (`TestV2ThreeRoundTripDataIntegrity`) plus error-handling coverage (`CorpusForkErrorHandlingTest`).
  - **Behavioural change:** `fork_corpus` no longer respects selective `doc_ids` / `annotation_ids` arguments. Any caller passing those now gets a *full* fork (with a `logger.warning`). No live caller in `doclatticeserver/`, `config/`, or tests still passes selective args; legacy queued Celery tasks would still run safely (full fork instead of partial). A short note here flags the contract change for downstream forks of this repo.
