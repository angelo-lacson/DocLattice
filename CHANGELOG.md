...
- **`pre_authorized_tools` no longer determines tool availability** (`doclatticeserver/tasks/agent_tasks.py`): Previously, `pre_authorized_tools` was used as both the tool set AND the approval gate — if set, it replaced `agent_config.available_tools` entirely. Now `pre_authorized_tools` only controls which tools skip approval gates. Tool availability is determined by `agent_config.available_tools` (if set) or trigger-appropriate defaults. **Breaking change**: Any existing action that relied on `pre_authorized_tools` to restrict tool access will now receive the full `agent_config.available_tools` set instead.
