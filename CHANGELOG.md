...
- **Typing: graduate `doclatticeserver.corpuses.*` from mypy baseline** (refs Issue #1447): removed five `[mypy-…]` `ignore_errors` blocks (`doclatticeserver.corpuses.{admin,checks,folder_service,managers,models}`) and pruned 166 baseline error lines from `docs/typing/mypy_baseline.txt` (7124 → 6958). Per-file fixes:
