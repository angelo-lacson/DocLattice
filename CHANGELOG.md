...
- **Single-select Dropdown missing onBlur in MetadataCellEditor** (PR #1076): Two single-select `Dropdown` instances (STRING with choices, CHOICE type) silently dropped the `onBlur` callback during the SUI→@os-legal/ui migration, causing metadata grid cells to never exit edit mode after selection. Added `onClose={onBlur}` to both cases and removed the `setTimeout` hack from CHOICE. Filed [OS-Legal/OS-Legal-Style#7](https://github.com/Open-Source-Legal/OS-Legal-Style/issues/7) requesting a proper `onBlur` prop on the Dropdown component. (`frontend/src/components/metadata/editors/MetadataCellEditor.tsx`)
