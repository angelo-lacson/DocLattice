...
- **Selected relation hidden when `showStructural=false`** (Issue #1289): In `frontend/src/components/annotator/hooks/useVisibleAnnotations.ts`, `forcedBySelectedRelationIds` was merged into `forcedIds` only inside the `if (showStructural)` block. Clicking a relation in the sidebar while the structural toggle was off left the relation's source/target annotations hidden, which conflicted with the behavior of `forcedBySelection` (individual annotations always bypass every other filter). Moved `forcedBySelectedRelationIds` out of the structural gate so explicit relation selection is always honored, mirroring `forcedBySelection`. The `forcedByRelationships` set (driven by the broader `showStructuralRelationships` toggle) remains gated. Updated the pinned regression test in `frontend/src/components/annotator/hooks/__tests__/useVisibleAnnotations.test.tsx` and added a case for structural relation members being forced visible when their relation is explicitly selected.
