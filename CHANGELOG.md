...
- **Service-layer centralization Phase 6 — follow-up fixes from PR #1780 code review** (issue #1782). Five targeted corrections on top of #1720's mechanical refactor; the migration itself is unchanged.
  - **New `BaseService.filter_visible_qs(queryset, user, *, request=None, **kwargs)`** (`doclatticeserver/shared/services/base.py`). The Phase-6 `get_queryset` migrations re-fetched the full visible set via `BaseService.filter_visible(Model, user).values("pk")` and intersected with the incoming queryset via `queryset.filter(pk__in=<subquery>)` — a correlated subquery over the full model table. The new helper chains the queryset's own `visible_to_user` instead (one `WHERE` expression tree, no subquery). Refactored `get_queryset` / field-resolver call sites in `config/graphql/annotation_types.py` (`AnnotationType`, `NoteType`, `LabelSetType.resolve_corpus_count`), `corpus_types.py` (`CorpusFolderType.resolve_children` / `get_queryset`, `CorpusType.resolve_folders` / `get_queryset`), `document_types.py` (`DocumentType.get_queryset`), `conversation_types.py` (`ConversationType.get_queryset`), and `user_types.py` (`UserFeedbackType.get_queryset` — prefetch-cache pass-through preserved). New regression tests in `doclatticeserver/tests/test_base_service.py` (`test_filter_visible_qs_chains_on_incoming_queryset` etc.) pin the single-pass-SQL contract by asserting the compiled query contains no `pk IN (SELECT`/`id IN (SELECT` subqueries against the full table.
...
- **Backend test suite speedup — Phase A3b: portable buildx cache, shared across CI + local dev + prod** (issue #1769). Swaps the GitHub-Actions-only buildx cache backend (`type=gha`, landed in Phase A3 / PR #1774) for a portable registry-backed cache at `ghcr.io/angelo-lacson/doclattice/django-{local,production}-cache:main`. GHA cache only works inside GitHub Actions, so it could not help contributors building from a fresh checkout. The registry cache is pullable from any docker buildx invocation that can reach ghcr.
  ...
  *Visibility note:* the ghcr cache packages inherit the repo's visibility on first publish (public, in this repo) — if either package is created private by mistake, change visibility in the Packages settings once.
