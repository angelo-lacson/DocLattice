...
- **PostgreSQL HNSW config warning on startup** (Closes #1074): Fixed `invalid configuration parameter name 'hnsw.iterative_scan'` warning caused by database-level `ALTER DATABASE SET` GUC defaults persisting in `pg_db_role_setting`. The docker-entrypoint-initdb.d phase runs in a temporary postgres without the user's command-line args, so `hnsw.*` GUCs aren't registered when the database-level defaults are later applied on connections. Moved `hnsw.iterative_scan=relaxed_order` and `hnsw.ef_search=64` to postgres command-line `-c` flags in all compose files (processed after `shared_preload_libraries=vector` loads pgvector). Removed `ALTER DATABASE SET` from `init.sql`. Added migration 0066 to RESET stale database-level defaults. (`local.yml`, `production.yml`, `test.yml`, `compose/production/postgres/init.sql`, `doclatticeserver/annotations/migrations/0066_reset_database_level_hnsw_settings.py`)
