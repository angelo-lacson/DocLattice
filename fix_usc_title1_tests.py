...
# Read the test file
with open("doclatticeserver/tests/test_structured_response_api.py", "r") as f:
    content = f.read()
...
replacements = [
    ...
    (r'"Provide a comprehensive analysis of this contract\."',
     '"Perform a comprehensive analysis of the \'Respect for Marriage Act\' amendment to Section 7. Extract its public law number, the year it was amended, and a summary of the change."'),
    ...
    (r'"Extract financial and signature information from this document"',
     '"Based on the text of Title 1, extract the total monetary penalty specified and its currency. The text contains no monetary values."'),
    ...
    (r'"What is the square root of 16\?"',
     '"Using all available overrides (custom prompt, LLM, embedder, etc.), ask the model to act as a child and explain what the \'Born-Alive Infants Protection Act of 2002\' does."'),
    
    # No conversation persistence test
    (r'"What color is the sky\?"',
     '"Extract the definition of \'vehicle\'."'),
    (r'"What color is grass\?"',
     '"Extract the definition of \'county\'."'),
    
    # Multiple extractions test
    (r'"Extract the title"',
     '"Extract the definition of \'writing\'."'),
    (r'"Extract the first paragraph"',
     '"Extract the definition of \'oath\'."'),
    
    # Nested pydantic test
    (r'"Extract information about the main company mentioned"',
     '"Extract information about the \'Respect for Marriage Act\' into a nested model that includes details about its enactment year and findings."'),
    
    # Optional fields test
    (r'"Extract document metadata"',
     '"Extract metadata about the \'21st Century Language Act of 2012\'. The model has optional fields for \'sponsor\' and \'committee\'."'),
    
    # Enum extraction test
    (r'"What type of document is this\?"',
     '"Classify the legal status of this title based on the introductory notes. Use an enum of \'POSITIVE_LAW\', \'PROPOSED_BILL\', or \'REPEALED\'."'),
    
    # Concurrent extractions test  
    (r'"Extract the title"',
     '"The official citation format for this title"'),
    (r'"Count the pages"',
     '"The number of sections defined in Chapter 1"'),
    (r'"Is this a contract\?"',
     '"Whether this document is the US Constitution"'),
    ...
    (r'"What type of document is this and how accurate is your assessment\?"',
     '"Using a custom prompt for a legal scholar, identify the primary purpose of this text and the legal principle it establishes."'),
...
content = re.sub(
    ...
    'assert all(isinstance(item, str) for item in result)\n            # Should include corporations, companies, associations, etc.\n            assert any("corporation" in item.lower() for item in result)',
    content,
    flags=re.MULTILINE
)

# Write the updated content
with open("doclatticeserver/tests/test_structured_response_api.py", "w") as f:
    f.write(content)
