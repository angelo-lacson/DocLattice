...
Every document cites other documents. _cite_ turns a repository of those documents into an open citation graph that humans and AI agents can read, reason over, and contribute back to. It is the codebase formerly released as **DocLattice**, rebranded for v3 as part of [opensource.legal](https://opensource.legal).

> Renaming, not rewriting. The GitHub repository keeps the `DocLattice` name through the v3 release line so existing forks, clones, and CI integrations don't break. Inside the product, the new name is _cite_ and the new home is `cite.opensource.legal`. See the [About page](https://cite.opensource.legal/about) for the full story.
...
The underlying engine — annotation, corpus management, AI agents, MCP server, vector search — is the same platform DocLattice has shipped since 2019. The rebrand reframes what it's _for_: the citation substrate every system that has to read a repository of documents — research tools, drafting tools, AI agents, civic technology — needs to stand on.
...
```bash
# The repo name stays `DocLattice` through the v3 release line so
