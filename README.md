...
# DocLattice ([Demo](https://doclattice.opensource.legal))
...
Production (Kubernetes/Docker): set `DOCLATTICE_*` variables which are converted at runtime into the `REACT_APP_*` payload consumed by the app. See `k8s/production/02-configmaps.yaml` for examples and `k8s/production/README.md` for details.
...
| |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --- |--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Backend CI/CD | [![codecov](https://codecov.io/gh/angelo-lacson/DocLattice/branch/main/graph/badge.svg?token=RdVsiuaTVz)](https://codecov.io/gh/angelo-lacson/DocLattice)                                                                                                                                                                                                                                                                                                                  |
...
DocLattice provides several key features:
...
We recommend you [browse our docs](https://jsv4.github.io/DocLattice/) via our Mkdocs Site. You can also view the 
docs in the repo:
...
DocLattice features a powerful, modular pipeline system for processing documents. The architecture supports easy creation and integration of custom parsers, embedders, and thumbnail generators:
...
For production deployments, DocLattice includes a dedicated migration service to ensure database schema updates are applied correctly and efficiently:
...
The company was ahead of its time, and, while the product is no longer available, DocLattice aims to take some of its [best and most innovative features](https://youtu.be/lX9lynpQwFA) and make them open source and available to the masses!
