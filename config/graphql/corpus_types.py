...
from doclatticeserver.utils.auth import is_authenticated_user
...
def _resolve_CorpusType_my_vote(root, info):
    ...
    from doclatticeserver.corpuses.services import CorpusVoteService
...
def _get_node_CorpusType(info, pk):
    ...
    cache apply (also required by the ``doclattice.E001`` system check).
    """
    try:
        pk = int(pk)
...
# ``DocLatticeNode.Field`` — an UNCACHED ``BaseService.get_or_none``
...
def _resolve_CorpusFolderType_my_permissions(root, info):
    ...
    ``doclatticeserver/corpuses/models.py`` ``CorpusFolder`` class
