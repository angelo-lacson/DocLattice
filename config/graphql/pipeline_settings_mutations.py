...
from doclatticeserver.pipeline.base.settings_schema import get_secret_settings
...
def validate_enricher_mapping(mapping: dict, registry) -> str | None:
    ...
    ``doclatticeserver.pipeline.utils.run_enrichers``).
    ...
    Returns:
        Error message if invalid, None if valid
    """
    from doclatticeserver.pipeline.registry import ComponentType
...
def _mutate_UpdatePipelineSettingsMutation(
    ...
    from doclatticeserver.documents.models import PipelineSettings
    from doclatticeserver.pipeline.registry import ComponentType, get_registry
...
def m_update_pipeline_settings(
    ...
    preferred_parsers: Annotated[
        ...
        strawberry.argument(
            ...
            description="Mapping of MIME types to preferred parser class paths. Example: {'application/pdf': 'doclatticeserver.pipeline.parsers.docling_parser_rest.DoclingParser'}",
        ),
    ] = strawberry.UNSET,
