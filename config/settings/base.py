...
# doclatticeserver/
APPS_DIR = ROOT_DIR / "doclatticeserver"
...
# Activate DocLattice Analyzer Functionality
...
LOCAL_APPS = [
    ...
    "doclatticeserver.document_imports",
    "doclatticeserver.discovery",
    "doclatticeserver.benchmarks",
    "doclatticeserver.research",
]
...
MIGRATION_MODULES = {"sites": "doclatticeserver.contrib.sites.migrations"}
...
# pipeline component registry (see doclatticeserver.pipeline.registry.get_allowed_mime_types).
...
if USE_AUTH0:
    ...
    # ``ADMIN_CLAIMS_CACHE_TTL`` in ``doclatticeserver/constants/auth.py``.
    from doclatticeserver.constants.auth import (
...
if STORAGE_BACKEND == "LOCAL":
    ...
    STORAGES = {
        "default": {
            "BACKEND": "doclatticeserver.utils.enhanced_storages.PooledMediaRootS3Storage",
        },
        "staticfiles": {
            "BACKEND": "doclatticeserver.utils.enhanced_storages.PooledStaticRootS3Storage",
        },
    }
    ...
    S3_DOCUMENT_PATH = env("S3_DOCUMENT_PATH", default="doclattice")
    MEDIA_ROOT = str(APPS_DIR / "media")
    ...
    STORAGES = {
        "default": {
            "BACKEND": "doclatticeserver.utils.storages.MediaRootGoogleCloudStorage",
        },
        "staticfiles": {
            "BACKEND": "doclatticeserver.utils.storages.StaticRootGoogleCloudStorage",
        },
    }
    ...
    GCS_DOCUMENT_PATH = env("GCS_DOCUMENT_PATH", default="doclattice")
    MEDIA_ROOT = str(APPS_DIR / "media")
...
from doclatticeserver.utils.files import (  # noqa: E402  (pure helper, no app/model imports at module level)
...
# DocLattice uses Redis as the Celery broker. Unlike RabbitMQ, Redis tracks
...
CELERY_TASK_ROUTES = {
    ...
    "doclatticeserver.worker_uploads.tasks.*": {"queue": "worker_uploads"},
    ...
    "doclatticeserver.tasks.doc_tasks.extract_thumbnail": {"queue": "doc_parse"},
    "doclatticeserver.tasks.doc_tasks.ingest_doc": {"queue": "doc_parse"},
    "doclatticeserver.tasks.doc_tasks.remap_pending_annotations": {
        "queue": "doc_parse"
    },
    "doclatticeserver.tasks.doc_tasks.set_doc_lock_state": {"queue": "doc_parse"},
    ...
    "doclatticeserver.tasks.doc_tasks.finalize_corpus_import_relationships": {
        "queue": "doc_parse"
    },
}
...
CELERY_BEAT_SCHEDULE = {
    "worker-uploads-drain-pending": {
        "task": "doclatticeserver.worker_uploads.tasks.process_pending_uploads",
        "schedule": 60.0,
        "options": {"queue": "worker_uploads"},
    },
    "worker-uploads-recover-stalled": {
        "task": "doclatticeserver.worker_uploads.tasks.recover_stalled_uploads",
        "schedule": 300.0,  # every 5 minutes
        "options": {"queue": "worker_uploads"},
    },
    "worker-uploads-drain-section-batches": {
        "task": "doclatticeserver.worker_uploads.tasks.process_pending_section_batches",
        "schedule": 60.0,
        "options": {"queue": "worker_uploads"},
    },
    ...
    "chunked-uploads-purge-stale": {
        "task": "doclatticeserver.document_imports.tasks.purge_stale_chunked_uploads",
    ...
    "documents-reconcile-stuck-processing": {
        "task": "doclatticeserver.tasks.doc_tasks.reconcile_stuck_documents",
        "schedule": 300.0,  # every 5 minutes
        "options": {"queue": "celery"},
    },
...
# Zip Import Limits
# ------------------------------------------------------------------------------
# These limits are consumed by doclatticeserver/constants/zip_import.py via
...
# consulted by ``doclatticeserver.llms.llm_registry.resolve_model_spec``
...
# a corpus is created (see ``doclatticeserver/corpuses/signals.py`` and
# ``doclatticeserver/corpuses/services/branding.py``). Per-corpus opt-out lives
...
_PDF_PARSER_MAP = {
    "docling": "doclatticeserver.pipeline.parsers.docling_parser_rest.DoclingParser",
    "llamaparse": "doclatticeserver.pipeline.parsers.llamaparse_parser.LlamaParseParser",
    "warp_ingest": "doclatticeserver.pipeline.parsers.warp_ingest_parser.WarpIngestParser",
}
...
PREFERRED_PARSERS = {
    ...
    "application/vnd.openxmlformats-officedocument.wordprocessingml.document": "doclatticeserver.pipeline.parsers.docxodus_parser.DocxodusServiceParser",  # noqa
...
# Each enricher transforms the parsed DocLatticeDocExport and passes it to
# the next. Empty by default — enrichment is strictly opt-in. Example:
#   PREFERRED_ENRICHERS = {
#       "application/pdf": [
#           "doclatticeserver.pipeline.enrichers.pdf_outline_enricher.PdfOutlineEnricher",  # noqa
#       ],
#   }
...
PREFERRED_EMBEDDERS = {
    "application/pdf": "doclatticeserver.pipeline.embedders.sent_transformer_microservice.MicroserviceEmbedder",  # noqa:
    "text/plain": "doclatticeserver.pipeline.embedders.sent_transformer_microservice.MicroserviceEmbedder",  # noqa:
    ...
    # "text/html": "doclatticeserver.pipeline.embedders.sent_transformer_microservice.MicroserviceEmbedder",  # Removed as we don't support HTML  # noqa:
}
...
PARSER_KWARGS = {
    "doclatticeserver.pipeline.parsers.llamaparse_parser.LlamaParseParser": {
...
#   ENABLED_COMPONENTS = ["doclatticeserver.pipeline.parsers.docling_parser_rest.DoclingParser"]
...
#   DEFAULT_FILE_CONVERTER=doclatticeserver.pipeline.file_converters.gotenberg_converter.GotenbergFileConverter  # noqa: E501
...
# Deep Research Agent
# ------------------------------------------------------------------------------
# Budget knobs for ``doclatticeserver.tasks.research_tasks.run_deep_research``.
...
# (doclatticeserver.enrichment.constants.LLM_MAX_CONCURRENCY, 8). Raise it for
...
# default (doclatticeserver.enrichment.constants.DOC_MAX_CONCURRENCY). Tune
...
# doclatticeserver/enrichment/data/authority_packs/ — for provider modules under
...
TELEMETRY_IP_SALT = env.str(
    "TELEMETRY_IP_SALT", default="doclattice-default-ip-salt-change-in-production"
)
MODE = "LOCAL"
...
# (doclatticeserver/mcp/server.py), which also merges in CORS_ALLOWED_ORIGINS
