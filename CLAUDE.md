...
DocLattice is an AGPL-3.0 enterprise document analytics platform for PDFs and text-based formats. It features a Django/GraphQL backend with PostgreSQL + pgvector, a React/TypeScript frontend with Jotai state management, and pluggable document processing pipelines powered by machine learning models.
...
docker compose -f test.yml run django python manage.py test doclatticeserver.tests.test_notifications --keepdb

# Run specific test file in parallel
docker compose -f test.yml run django pytest doclatticeserver/tests/test_notifications.py -n 4 --dist loadscope

# Run specific test class/method
docker compose -f test.yml run django python manage.py test doclatticeserver.tests.test_notifications.TestNotificationModel.test_create_notification --keepdb
...
4. **Django Signal Handlers**:
   - Automatic notification creation on model changes (see `doclatticeserver/notifications/signals.py`)
...
5. **Pluggable Parser Pipeline**:
   - Base classes in `doclatticeserver/pipeline/base/`
...
2. **Permission Checks**:
   ...
   - Check `user_has_permission_for_obj()` for individual objects (in `doclatticeserver.utils.permissioning`)
...
4. No magic numbers - we have constants files in doclatticeserver/constants/ (backend) and frontend/src/assets/configurations/constants.ts (frontend). Use them for any hardcoded values.
...
6. Utility functions belong in utility files:
   - **Before writing new utilities**: Check existing utility files first (`frontend/src/utils/`, `doclatticeserver/utils/`)
   ...
   - **Backend utilities**: `doclatticeserver/utils/` contains permissioning, PDF processing, and other shared utilities

## Testing Patterns

### Backend Tests

**Location**: `doclatticeserver/tests/`
