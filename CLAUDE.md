...
DocLattice is a GPL-3 enterprise document analytics platform that supports PDF and text-based document analysis with a pluggable architecture. It combines Django backend with React frontend to provide document annotation, vector embeddings, LLM integration, and data extraction capabilities.
...
### LLM Framework
DocLattice includes a unified LLM framework (`llms/`) supporting multiple backends with these core principles:
