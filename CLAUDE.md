...
7. **Corpus / document access — always via the service layer.** Code with a user context (request handlers, GraphQL resolvers, MCP tools, LLM tools, REST views, Celery tasks invoked with a user) **must** reach documents through a service:
   ...
   - **Corpus-scoped operations** ("give me X inside corpus Y for user Z") — covered today by documents-in-corpus and folders, growing to annotations, notes, relationships, extracts, and analyses. **Issue #1716 split the former `CorpusObjsService` monolith into the segmented `doclatticeserver/corpuses/services/` package — import the specific service from `doclatticeserver.corpuses.services`:** `CorpusDocumentService` (document-in-corpus reads/writes + membership), `FolderCRUDService` (folder CRUD, tree, search), `FolderDocumentService` (document-in-folder placement), `DocumentLifecycleService` (soft-delete/restore/trash), `CorpusPathService` (`DocumentPath` disambiguation internals), and `CorpusService` (Corpus-row CRUD: delete, visibility, description versioning). The legacy `CorpusObjsService` facade and its `corpus_objs_service.py` shim have been removed (Phase 2C).
