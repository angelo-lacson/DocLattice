...
from doclatticeserver.types.dicts import DocLatticeDocExport, PawlsTokenPythonType

logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)

class DoclingToDocLatticeConverter:
    """
    Converts PDF documents processed by Docling into the DocLattice format.
    ...
    def convert_pdf(self, pdf_path: str) -> DocLatticeDocExport:
        """
        Convert a PDF file to DocLattice format using Docling's processing pipeline.

        Args:
            pdf_path (str): Path to the PDF file to convert.

        Returns:
            DocLatticeDocExport: Document content, structure, and annotations.
        ...
        return {
            ...
            "doc_labels": [],  # From DocLatticeDocAnnotations
            "labelled_text": []  # From DocLatticeDocAnnotations
        }
...
def parse_with_docling(user_id: int, doc_id: int) -> Optional[DocLatticeDocExport]:
    """
    ...
    converter = DoclingToDocLatticeConverter()
    try:
