"""
Standalone validation utility for DocLattice corpus export ZIPs.
...
or run directly from the command line::
    ...
    python -m doclatticeserver.utils.validate_export corpus_export.zip
...
# doclatticeserver/constants/zip_import.py — that module has a Django
...
# ``doclatticeserver.utils.importing.VALID_LABEL_TYPES_FOR_IMPORT``). Kept as
...
def validate_export(zip_path: str | os.PathLike[str]) -> ValidationResult:
    """
    Validate an DocLattice corpus export ZIP file.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args or args[0] in ("-h", "--help"):
        print(
            "Usage: python -m doclatticeserver.utils.validate_export "
            "<export.zip> [export2.zip ...]\n\n"
            "Validates DocLattice corpus export ZIP files.\n"
