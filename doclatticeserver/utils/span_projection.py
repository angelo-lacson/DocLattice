...
(``doclatticeserver/pipeline/base/parser.py``), so offsets into that text
project onto token bounding boxes losslessly via PlasmaPDF.

This module is the single home for that projection. Consumers:

* datacell auto-grounding (``doclatticeserver/utils/extraction_grounding.py``)
* the reference-enrichment writer (``doclatticeserver/enrichment/writer.py``)
"""
...
def span_annotation_payload(start: int, end: int, text: str) -> tuple[dict, int]:
    ...
    """
    from doclatticeserver.constants.annotations import SPAN_NO_PAGE
