...
from doclatticeserver.constants.permissioning import INSTANCE_PERMS_CACHE_ATTR
...
class _InstancePermsCache(dict):
    ...
    ``doclatticeserver/utils/permission_optimizer.py``.
    """
...
def set_permissions_for_obj_to_user(
    ...
    **REPLACE** current permissions with specified permissions. Pass empty list to permissions to completely
    ...
    if request is not None:
        from doclatticeserver.utils.permission_optimizer import (
...
def _default_user_can(
    ...
    *,
    ...
    # worth caching.
    from doclatticeserver.shared.permission_cache import (
        MISS,
        cached_user_can,
        store_user_can,
    )
...
def get_for_user_or_none(
    ...
    :meth:`doclatticeserver.corpuses.services.corpus_documents.CorpusDocumentService.get_corpus_document_by_id`
