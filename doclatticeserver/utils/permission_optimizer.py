...
``doclatticeserver.utils.permissioning.get_users_permissions_for_obj``.
...
- A ``get_request_optimizer(request)`` helper that lazy-attaches the
  ...
  :mod:`doclatticeserver.constants.permissioning`.
...
from doclatticeserver.constants.permissioning import REQUEST_OPTIMIZER_ATTR
