...
from doclatticeserver.notifications.signals import (
    broadcast_notification_via_websocket,
)
from doclatticeserver.pipeline.base.exceptions import (
...
from doclatticeserver.utils.annotation_anchoring import (
    anchor_annotations,
...
from doclatticeserver.utils.importing import import_annotations, import_relationships
...
def _queue_embeddings_for_unlocked_document(
    *,
    ...
    pipeline run.
    """
    from doclatticeserver.tasks.corpus_tasks import ensure_embeddings_for_corpus
...
def convert_document_to_pdf(self, user_id: int, doc_id: int) -> dict[str, Any]:
    ...
    Raises:
        FileConversionError: After marking the document FAILED, to halt the
            ingest chain.
    """
    from doclatticeserver.pipeline.utils import get_default_file_converter_instance
...
def ingest_doc(self, user_id: int, doc_id: int) -> dict[str, Any]:
    ...
    if (
        ...
        if chunk_inputs and chunk_count <= max_chord_tasks:
            from doclatticeserver.tasks.chunk_tasks import (
...
def _remap_one_pending_row(
    ...
    with transaction.atomic():
        ...
        annot_id_map = import_annotations(
            ...
            doc_obj=doc,
            corpus_obj=corpus,
            annotations_data=cast(list[DocLatticeAnnotationPythonType], anchored),
...
def finalize_corpus_import_relationships(run_id: str) -> dict[str, Any]:
    ...
    from doclatticeserver.tasks.import_tasks_v2 import _import_v2_relationships
