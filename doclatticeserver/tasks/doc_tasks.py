...
def nlm_ingest_pdf(user_id: int, doc_id: int) -> list[tuple[int, str]]:
    ...
    files = {'file': doc_file}
    params = {'calculate_doclattice_data': 'yes'}  # Ensures calculate_doclattice_data is set to True
    ...
    doclattice_data: Optional[DocLatticeDocExport] = response_data.get('return_dict', {}).get('doclattice_data', None)

    document = Document.objects.get(pk=doc_id)

    # Create new labels if needed
    if doclattice_data is not None:

        # Get PAWLS layer and text contents
        pawls_string = json.dumps(doclattice_data['pawls_file_content'])
        pawls_file = ContentFile(pawls_string.encode("utf-8"))
        txt_file = ContentFile(doclattice_data['content'].encode("utf-8"))

        document.txt_extract_file.save(f"doc_{doc_id}.txt", txt_file)
        document.pawls_parse_file.save(f"doc_{doc_id}.pawls", pawls_file)
        document.page_count = len(doclattice_data['pawls_file_content'])

        existing_text_labels: dict[str, AnnotationLabel] = {}

        # Now, annotate the document with any annotations that bubbled up from parser.
        for label_data in doclattice_data['labelled_text']:
