...
from doclatticeserver.constants.document_processing import BLOB_HASH_CHUNK_BYTES
...
from doclatticeserver.utils.zip_security import read_zip_member_bounded

if TYPE_CHECKING:
    from doclatticeserver.corpuses.models import CorpusFolder
    from doclatticeserver.users.models import User as UserModel
...
def _read_guarded_source_bytes(
    ...
    (see ``doclatticeserver.utils.zip_security``) — used by BOTH the
...
def _setup_corpus_and_labels(
    ...
    # is structurally compatible with the DocLatticeLabelSetType TypedDict
    ...
    # ``prepare_import_labels`` expects ``DocLatticeExportDataJsonPythonType``.
...
def _retire_prior_authority_versions_if_current(
    *,
    ...
    from doclatticeserver.enrichment.authorities import AuthorityCorpusBootstrapper
...
def _reconcile_imported_authority_metadata(
    *,
    ...
    from doclatticeserver.annotations.models import AuthorityNamespace
    from doclatticeserver.enrichment.authorities import AuthorityCorpusBootstrapper
    from doclatticeserver.enrichment.authority_sources import SourceRelationship
    from doclatticeserver.enrichment.services.authority_permissions import (
        is_authority_admin,
    )
    from doclatticeserver.enrichment.services.authority_relationship_service import (
...
def _import_corpus(
    ...
    try:
        ...
        if is_v2 and not reingest_and_remap:
            v2_struct_data = cast(DocLatticeExportDataJsonV2Type, data_json)
        ...
        if is_v2:
            ...
            v2_data = cast(DocLatticeExportDataJsonV2Type, data_json)
            ...
            from doclatticeserver.corpuses.services.description_cache import (
...
def _import_v2_relationships(
    relationships_data: list[DocLatticeRelationshipPythonType],
    corpus_obj: Corpus,
...
def _build_folder_path_lookup(
    ...
    The canonical DocLattice exporter (``utils/export_v2.py``) writes the
