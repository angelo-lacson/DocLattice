# doclatticeserver/tasks/data_extract_tasks.py
import json
import logging
import os
...
from doclatticeserver.shared.decorators import async_celery_task
...
async def oc_llama_index_doc_query(
    ...
    """
    DocLattice' default LlamaIndex and Marvin-based data-extraction pipeline.
    ...
    from doclatticeserver.utils.etl import parse_model_or_primitive
    ...
    def get_filtered_annotations_with_similarity(
        ...
        from doclatticeserver.tasks.embeddings_task import get_embedder
...
def annotation_window(document_id: int, annotation_id: str, window_size: str) -> str:
    ...
    """
    from doclatticeserver.annotations.models import Annotation
    from doclatticeserver.types.dicts import PawlsPagePythonType, TextSpanData
    ...
    try:
        if file_type.startswith("text/"):
            ...
            pages_dict: dict[int, DocLatticeSinglePageAnnotationType] = {}
            try:
                if is_single_page_annotation(anno_json):
                    page_index = annotation.page
                    pages_dict[page_index] = DocLatticeSinglePageAnnotationType(
                        **anno_json
                    )
                else:
