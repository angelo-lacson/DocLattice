...
**Not a general DocLattice feature.** This is purpose-built for corpora of
...
open-vocabulary authority-discovery system (:mod:`doclatticeserver.enrichment.services.enrichment_service`),
...
ever sees it, so offsets are always computed against the text DocLattice
...
* Ruling-number citations are cross-document references, so they are modeled
  as :class:`~doclatticeserver.enrichment.extractor.Candidate` /
  :class:`~doclatticeserver.enrichment.resolver.Resolution` objects
  (``reference_type=REF_DOCUMENT``) and persisted via
  :class:`~doclatticeserver.enrichment.writer.EnrichmentWriter` — the same
...
ANALYZER_TASK_NAME = "doclatticeserver.enrichment.customs_ruling_citations"
