...
:class:`~doclatticeserver.pipeline.base.llm_provider.BaseLLMProvider`
...
``doclatticeserver.pipeline`` package) and are performed lazily so the
...
from doclatticeserver.llms.llm_registry import parse_model_spec
...
def _provider_class_path(provider_key: str) -> str | None:
    ...
    from doclatticeserver.pipeline.registry import get_llm_provider_by_key_cached
...
# embedder instance caches use (``doclatticeserver/pipeline/utils.py``).
#
...
#:
#: Every DocLattice agent carries function tools, and turning reasoning off
