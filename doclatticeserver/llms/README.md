...
```
┌─────────────────────────────────────────┐
│        DocLattice LLM API            │  ← You are here (api.py)
...
- **API Layer**: The beautiful, simple interface (`agents.for_document`, `embeddings.generate`, etc.) exposed in `doclatticeserver.llms.api`.
...
1.  **API Call**: You call a function like `await agents.for_document(...)` from `doclatticeserver.llms.api`.
...
```python
from doclatticeserver.llms import agents # Entry point for agent creation
from doclatticeserver.llms.agents.core_agents import CoreAgent # Protocol for agent instances
...
# from doclatticeserver.llms.agents import create_document_agent # Older factory
...
The previous `create_document_agent` and `create_corpus_agent` functions (now found in `doclatticeserver.llms.agents`) are still available for backward compatibility. They have been refactored to use the new `UnifiedAgentFactory` internally, defaulting to LlamaIndex.

```python
# Still works (uses the old function now pointing to the new factory with LlamaIndex default)
from doclatticeserver.llms.agents import create_document_agent 
agent = await create_document_agent(document_id=document_id) 

# But this is the recommended new API:
from doclatticeserver.llms import agents # Import the API module
agent = await agents.for_document(document=document_id)
```

The new `agents.for_document()` and `agents.for_corpus()` methods in `doclatticeserver.llms.api` (exposed via `doclatticeserver.llms.agents`) provide a more streamlined and flexible interface, and are the preferred way to create agents.
...
Here's a step-by-step guide to integrate a new LLM framework into DocLattice:

1. **Define Framework Enum**:
    *   Open `doclatticeserver/llms/types.py`.
...
2. **Implement Agent Adapters**:
    *   Create a new Python file in the `doclatticeserver/llms/agents/` directory. Name it descriptively, e.g., `my_new_framework_agents.py`.
    *   Inside this file, define classes for your document and/or corpus agents. These classes **must** inherit from `CoreAgent` (from `doclatticeserver.llms.agents.core_agents.py`).
        ...
        from doclatticeserver.documents.models import Document # If creating document agent
...
3. **Integrate into `UnifiedAgentFactory`**:
    *   Open `doclatticeserver/llms/agents/agent_factory.py`.
    *   Import your new agent adapter class(es) at the top of the file.
        ```python
        # Example import
        # from doclatticeserver.llms.agents.my_new_framework_agents import MyNewFrameworkDocumentAgent 
        ```
    *   In the `UnifiedAgentFactory.create_document_agent` method (and `create_corpus_agent` if applicable), add an `elif` block to handle your new framework:
        ...
        if framework == AgentFramework.LLAMA_INDEX:
            ...
            from doclatticeserver.llms.agents.my_new_framework_agents import MyNewFrameworkDocumentAgent # Ensure this import is here or at top
...
4. **Implement Tool Conversion (If Needed)**:
    ...
    *   If your new framework requires tools in a unique format not already handled by `UnifiedToolFactory`:
        *   Open `doclatticeserver/llms/tools/tool_factory.py`.
...
6. **Testing**:
    *   Thoroughly test your new integration. Write unit tests for your agent adapter class(es) in `doclatticeserver/tests/llms/agents/`.
...
By following these steps, you can extend the DocLattice LLM framework to support a wider range of LLM technologies while maintaining a consistent and simple API for users.
