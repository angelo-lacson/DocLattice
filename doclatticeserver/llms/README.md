...
- **API Layer**: The simple interface (`agents.for_document`, `embeddings.generate`, etc.) exposed in `doclatticeserver.llms.api`.
...
```python
from doclatticeserver.llms.agents import AgentConfig
...
```python
# Old way (still works)
from doclatticeserver.llms.agents import create_document_agent, create_corpus_agent
...
```python
# Old way
from doclatticeserver.llms.vector_stores.vector_stores import DjangoAnnotationVectorStore
