...
class CoreTool:
    ...
    def from_function(
        ...
        if not parameter_descriptions:
            parameter_descriptions = _extract_parameter_descriptions_from_docstring(func)
...
class UnifiedToolFactory:
    ...
    def create_tool(tool: CoreTool, framework: AgentFramework) -> Any:
        ...
        if framework == AgentFramework.LLAMA_INDEX:
            from doclatticeserver.llms.tools.llama_index_tools import LlamaIndexToolFactory
            return LlamaIndexToolFactory.create_tool(tool)
        elif framework == AgentFramework.PYDANTIC_AI:
            from doclatticeserver.llms.tools.pydantic_ai_tools import PydanticAIToolFactory
