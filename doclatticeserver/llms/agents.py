...
async def create_document_agent(*args, **kwargs):
    ...
    agent = await unified_create_document_agent(*args, framework=AgentFramework.LLAMA_INDEX, **kwargs)
    return DocLatticeDbAgent(agent)


async def create_corpus_agent(*args, **kwargs):
    """Create a corpus agent using the unified factory (backward compatibility)."""
    agent = await unified_create_corpus_agent(*args, framework=AgentFramework.LLAMA_INDEX, **kwargs)
    return DocLatticeDbAgent(agent)
