...
from doclatticeserver.llms.model_factory import abuild_agent_model
...
from doclatticeserver.utils.tools import deduplicate_tools, get_tool_name
...
class PydanticAICoreAgent(CoreAgentBase, TimelineStreamMixin):
    ...
    async def _get_message_history(self) -> _HistoryResult:
        """Convert DocLattice ``ChatMessage`` history to Pydantic-AI format.
    ...
    async def _stream_core(
        ...
        **kwargs: Any,
        ...
        # processor in ``doclatticeserver.llms.history_processors`` is
...
class PydanticAIDocumentAgent(PydanticAICoreAgent):
    ...
    async def create(
        ...
        **kwargs: Any,
        ...
        # see doclatticeserver/constants/context_guardrails.py) so a
        ...
        **kwargs,
        ...
        async def ask_document_tool(
            ...
            if document_id not in {d.id for d in context.documents}:
                if corpus_group:
                    from doclatticeserver.llms.tools.core_tools.multi_corpus import (
        ...
        if config.user_id is not None:
            ...
            from doclatticeserver.llms.tools.research_tools import (
