...
class CoreAgentBase(ABC):
    ...
    async def complete_message(
        ...
        logger.error(f"[DIAGNOSTIC complete_message]   sources: {sources is not None} (count: {len(sources) if sources else 0})")
        if sources:
            logger.error(f"[DIAGNOSTIC complete_message]   First source: {sources[0].to_dict()}")
        logger.error(f"[DIAGNOSTIC complete_message]   metadata keys: {metadata.keys() if metadata else 'None'}")
        logger.error(f"[DIAGNOSTIC complete_message]   Calling conversation_manager.complete_message()...")
        await self.conversation_manager.complete_message(
            message_id, content, sources, metadata
        )
        logger.error(f"[DIAGNOSTIC complete_message]   conversation_manager.complete_message() returned successfully")
