...
class ContractSummary(BaseModel):
    ...
    effective_date: Optional[str] = Field(None, description="When contract becomes effective") 
...
async def example_document_extraction():
    ...
    title = await agent.structured_response(
    ...
    if contract_summary:
        ...
        print(f"Value: ${contract_summary.value:,.2f}" if contract_summary.value else "Value: Not specified")
...
async def example_corpus_extraction():
    ...
    agent = await agents.for_corpus(
        corpus=456,  # Your corpus ID
        framework=AgentFramework.PYDANTIC_AI
    )
    
    # Extract corpus-wide statistics
    stats = await agent.structured_response(
        "Analyze this corpus and provide statistics",
        CorpusStatistics
    )
...
async def example_batch_extraction():
    ...
    for doc_id in document_ids:
        ...
        task = agent.structured_response(
