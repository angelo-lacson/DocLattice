...
class DocumentQuerySet(PermissionQuerySet, VectorSearchViaEmbeddingMixin):
    ...
    def visible_to_user(
        ...
        (doclatticeserver/shared/Managers.py). Prefetch optimisation is
...
class AnnotationQuerySet(PermissionQuerySet, VectorSearchViaEmbeddingMixin):
    ...
    def visible_to_user(
        ...
        visibility_filter = (
            ...
            Q(structural=True)
            |
            # User's own annotations. Mirrors ``AnnotationManager.user_can``'s
            # row-creator source-privacy exemption; the doc/corpus filters
            # below still apply.
            Q(creator=user)
            |
...
class CachedCountAnnotationQuerySet(CachedCountQuerySetMixin, AnnotationQuerySet):
    ...
    ``doclatticeserver.constants.annotations`` so the value and the
