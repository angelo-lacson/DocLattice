...
from doclatticeserver.types.enums import JobStatus, LabelType
...
def doc_analyzer_task(
    ...
    The wrapped function is expected to return either:
        1) A 4-element tuple:
           (List[str], List[DocLatticeAnnotationPythonType], List[Dict[str, Any]], bool)
           If only four elements are returned, we will set the message to "No Return Message"
        2) A 5-element tuple:
           (List[str], List[DocLatticeAnnotationPythonType], List[Dict[str, Any]], bool, str)
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(self: Any, *args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                # ``create_doclattice_annotation_from_span`` is called below
                    ...
                    with transaction.atomic():
                        for span_label_pair in span_label_pairs:
                            ...
                            if doc.file_type in ["application/pdf"]:
                                ...
                                annotation_data = pdf_data_layer.create_doclattice_annotation_from_span(
        ...
        async def async_wrapper(self, *args, **kwargs) -> Any:
            ...
            try:
                ...
                if not isinstance(result, tuple) or len(result) != 4:
                    raise ValueError(
                        "Function must return a tuple of (List[str], List[DocLatticeAnnotationPythonType], "
                ...
                if task_pass:
                    ...
                    for span_label_pair in span_label_pairs:
                        ...
                        if doc.file_type in ["application/pdf"]:
                            span, label_text = span_label_pair
                            annotation_data = (
                                pdf_data_layer.create_doclattice_annotation_from_span(
                                    {"span": span, "annotation_label": label_text}
                                )
                            )
                            logger.info(
