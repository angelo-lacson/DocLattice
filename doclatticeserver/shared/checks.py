"""Django system checks enforcing the DocLattice architecture invariants.
...
Wired in by ``doclatticeserver.users.apps.UsersConfig.ready`` (the same
...
def check_graphql_service_layer(app_configs: Any, **kwargs: Any) -> list[Error]:
    ...
    Severity is ``Error`` (``doclattice.E001``): Django blocks any
    ...
    for module_path, lineno, name in audit_graphql_modules():
        ...
        issues.append(Error(short, hint=hint, id="doclattice.E001"))
    return issues
