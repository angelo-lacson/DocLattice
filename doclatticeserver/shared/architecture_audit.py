...
- ``doclatticeserver/shared/checks.py`` — Django system check that fires
...
# and therefore the ``doclattice.E001`` system check and the pytest
...
# the other user-context surfaces too (MCP tools in ``doclatticeserver/mcp/``,
# LLM tools in ``doclatticeserver/llms/tools/``, REST views, and
...
# lives at ``<repo-root>/doclatticeserver/shared/architecture_audit.py``,
...
def scan_forbidden(source: str) -> list[tuple[int, str]]:
    ...
    Detects:
        ...
        - ``from doclatticeserver... import visible_to_user``  → Import alias
...
_REQUIRED_IMPORT = "from doclatticeserver.shared.services.base import BaseService"
