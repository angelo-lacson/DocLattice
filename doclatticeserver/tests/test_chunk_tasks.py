...
from doclatticeserver.tests.test_chunked_parser import (
    _FakeChunkedParser,
    _make_chunk_result,
)
...
_FAKE = "doclatticeserver.tests.test_chunked_parser._FakeChunkedParser"
...
_NONE_PARSER = "doclatticeserver.tests.test_chunk_tasks._NoneReturningChunkedParser"
...
_KWREC_PARSER = "doclatticeserver.tests.test_chunk_tasks._KwargRecordingChunkedParser"
...
class TestChunkTasks(TestCase):
    ...
    def test_load_chunked_parser_rejects_non_chunking_parser(self):
        from doclatticeserver.tasks.chunk_tasks import _load_chunked_parser

        with self.assertRaises(ValueError):
            _load_chunked_parser(
                "doclatticeserver.pipeline.parsers.oc_text_parser.TxtParser"
            )
    ...
    def test_parse_chunk_threads_parser_kwargs(self):
        ...
        in_key = write_chunk_pdf(doc.id, 0, make_test_pdf(2))
        with patch(
            "doclatticeserver.documents.models.PipelineSettings.get_parser_kwargs",
    ...
    def test_ingest_doc_large_pdf_replaces_with_chord_when_not_eager(self):
        ...
        from doclatticeserver.tasks import doc_tasks
        ...
        with override_settings(CELERY_TASK_ALWAYS_EAGER=False):
            with patch.object(
                ...
                return_value=(
                    "doclatticeserver.tests.test_chunked_parser._FakeChunkedParser",
                    parser,
                    {},
                ),
            ), patch(
                "doclatticeserver.pipeline.base.parser.BaseParser.process_document"
    ...
    def test_ingest_doc_large_pdf_falls_back_when_chunk_count_exceeds_limit(self):
        ...
        from doclatticeserver.pipeline import chunk_artifacts
        from doclatticeserver.tasks import doc_tasks
