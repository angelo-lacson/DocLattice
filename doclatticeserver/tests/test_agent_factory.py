...
MOCK_LLAMA_INDEX_CORPUS_AGENT_PATH = "doclatticeserver.llms.agents.llama_index_agents.LlamaIndexCorpusAgent"
MOCK_PYDANTIC_AI_CORPUS_AGENT_PATH = "doclatticeserver.llms.agents.pydantic_ai_agents.PydanticAICorpusAgent"
...
class TestAgentFactorySetup(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = User.objects.create_user(username="factory_testuser", password="password", email="factory@test.com")
        cls.corpus1 = Corpus.objects.create(title="Factory Test Corpus 1", creator=cls.user)
...
class TestUnifiedAgentFactory(TestAgentFactorySetup):
    ...
    @patch(MOCK_LLAMA_INDEX_DOC_AGENT_PATH, new_callable=MagicMock) # Using MagicMock for .create if it's a classmethod
    @patch(f'{UnifiedAgentFactory.__module__}.get_default_config')
    async def test_create_document_agent_llama_index(self, mock_get_config: MagicMock, MockLlamaDocAgent: MagicMock):
        mock_config = AgentConfig(user_id=self.user.id)
        mock_get_config.return_value = mock_config
        mock_agent_instance = AsyncMock(spec=CoreAgent)
        MockLlamaDocAgent.create = AsyncMock(return_value=mock_agent_instance) # Mock the create classmethod
    ...
    async def test_create_document_agent_pydantic_ai_with_tools(self, mock_convert_tools: MagicMock, mock_get_config: MagicMock, MockPydanticDocAgent: MagicMock):
        ...
        mock_get_config.assert_called_once_with(user_id=None, system_prompt=None, conversation=None, loaded_messages=None, embedder_path=None)
        mock_convert_tools.assert_called_once_with(raw_tools, AgentFramework.PYDANTIC_AI)
        MockPydanticDocAgent.create.assert_called_once_with(self.doc1, mock_config, converted_framework_tools)
        self.assertIs(agent, mock_agent_instance)

    @patch(MOCK_LLAMA_INDEX_CORPUS_AGENT_PATH, new_callable=MagicMock)
    @patch(f'{UnifiedAgentFactory.__module__}.get_default_config')
    async def test_create_corpus_agent_llama_index(self, mock_get_config: MagicMock, MockLlamaCorpusAgent: MagicMock):
        mock_config = AgentConfig()
        mock_get_config.return_value = mock_config
        mock_agent_instance = AsyncMock(spec=CoreAgent)
        ...
        MockLlamaCorpusAgent.create.assert_called_once_with(self.corpus1.id, mock_config, [])
        self.assertIs(agent, mock_agent_instance)

    @patch(MOCK_PYDANTIC_AI_CORPUS_AGENT_PATH, new_callable=MagicMock)
    @patch(f'{UnifiedAgentFactory.__module__}.get_default_config')
    async def test_create_corpus_agent_pydantic_ai(self, mock_get_config: MagicMock, MockPydanticCorpusAgent: MagicMock):
        mock_config = AgentConfig()
        mock_get_config.return_value = mock_config
        mock_agent_instance = AsyncMock(spec=CoreAgent)
        ...
        mock_get_config.assert_called_once()
        MockPydanticCorpusAgent.create.assert_called_once_with(self.corpus1.id, mock_config, [])
        self.assertIs(agent, mock_agent_instance)

    async def test_unsupported_framework_raises_error(self):
        with self.assertRaises(ValueError):
            await UnifiedAgentFactory.create_document_agent(self.doc1, framework="invalid_framework_name")
        with self.assertRaises(ValueError):
            await UnifiedAgentFactory.create_corpus_agent(self.corpus1.id, framework="invalid_framework_name")
...
class TestConvertToolsForFramework(TestAgentFactorySetup):
    ...
    def test_converts_callable_and_coretool_correctly(self, mock_core_create_tools: MagicMock, mock_coretool_from_function: MagicMock):
        ...
        mock_core_create_tools.assert_called_once_with(
            [mocked_core_tool_from_callable, self.core_tool_instance],
            target_framework
        )
    ...
    def test_ignores_invalid_tool_type(self, mock_core_create_tools: MagicMock):
        ...
        logger_name = 'doclatticeserver.llms.agents.agent_factory'
        ...
        self.assertEqual(len(passed_core_tools_list), 2) # callable_tool (converted) and core_tool_instance
...
# If your LlamaIndexDocumentAgent is, for example, directly in doclatticeserver.llms.agents, the path would change.
