...
from doclatticeserver.constants.document_processing import MAX_FILENAME_LENGTH
...
from doclatticeserver.shared.mixins import (
    HasEmbeddingMixin,
...
class TestHasEmbeddingMixinDimensionMapping(TestCase):
    ...
    @patch(
        "doclatticeserver.shared.mixins.HasEmbeddingMixin.get_embedding_reference_kwargs"
    )
    def test_get_embedding_not_found(self, mock_kwargs):
        mock_kwargs.return_value = {"document_id": 999}
        mixin = HasEmbeddingMixin()
        with patch(
            "doclatticeserver.annotations.models.Embedding.objects.get"
        ) as mock_get:
