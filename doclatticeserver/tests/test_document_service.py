...
- Backwards-compat: ``DocumentFolderService`` exposes the full merged surface
  so existing ``from doclatticeserver.corpuses.folder_service import
...
class TestDocumentService_CreateDocument(TestCase):
    ...
    def test_db_error_inside_atomic_block_is_surfaced_not_swallowed(self):
        ...
        from doclatticeserver.documents import models as doc_models
...
class TestDocumentService_SetDocumentPermissions(TestCase):
    ...
    def test_grant_call_exception_is_caught_and_returned(self):
        ...
        with patch(
            "doclatticeserver.documents.document_service.set_permissions_for_obj_to_user",
