...
from doclatticeserver.pipeline.embedders.minn_modern_bert_embedder import MinnModernBERTEmbedder, MinnModernBERTEmbedder768
from doclatticeserver.pipeline.base.file_types import FileTypeEnum


class TestMinnModernBERTEmbedder(unittest.TestCase):
    ...
    @patch('doclatticeserver.pipeline.embedders.minn_modern_bert_embedder.SentenceTransformer')
    def test_initialization(self, mock_transformer):
        ...
        self.assertEqual(embedder.model_path, "/models/sentence-transformers/teraflop-minn-caselaw")
        
        # Model should not be loaded on initialization
        self.assertIsNone(embedder.model)
        mock_transformer.assert_not_called()

    @patch('doclatticeserver.pipeline.embedders.minn_modern_bert_embedder.os.path.exists')
    @patch('doclatticeserver.pipeline.embedders.minn_modern_bert_embedder.SentenceTransformer')
    ...
    def test_minn_modernbert_embedder_768(self):
        ...
        # Check attributes
        self.assertEqual(embedder.title, "Minnesota Case Law ModernBERT Embedder (768d)")
        self.assertEqual(embedder.vector_size, 768)
