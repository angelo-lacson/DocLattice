...
PR #1543 (typing graduation of doclatticeserver.llms.*) so codecov reports
...
class TestExtractToolResultSummary(TestCase):
    ...
    def test_summary_from_dict_answer_key(self) -> None:
        from doclatticeserver.llms.agents.pydantic_ai_agents import (
    ...
    def test_summary_from_truncation(self) -> None:
        ...
        from doclatticeserver.llms.agents.timeline_utils import (
...
class TestUsageToDict(TestCase):
    def test_none_returns_none(self) -> None:
        from doclatticeserver.llms.agents.pydantic_ai_agents import _usage_to_dict
...
class TestSimpleLLMClientChatOpenAI(TestCase):
    def _make_client(self, max_tokens: int | None = None):
        from doclatticeserver.llms.client import SimpleLLMClient
    ...
    def test_chat_without_max_tokens_omits_param(self) -> None:
        from doclatticeserver.llms.client import ChatMessage
...
class TestCoreAnnotationVectorStoreCtor(TestCase):
    def test_explicit_embedder_path_only(self) -> None:
        ...
        with patch(
            "doclatticeserver.llms.vector_stores.core_vector_stores.get_embedder",
...
class TestCoreConversationVectorStoreCtor(TestCase):
    def test_explicit_path_with_corpus_id_uses_both(self) -> None:
        ...
        from doclatticeserver.llms.vector_stores.core_conversation_vector_stores import (  # noqa: E501
            CoreConversationVectorStore,
        )

        with patch(
            "doclatticeserver.llms.vector_stores.core_conversation_vector_stores.get_embedder",
...
class TestTimelineStreamMixin(TestCase):
    def test_stream_injects_timeline_into_final_event(self) -> None:
        ...
        from doclatticeserver.llms.agents.timeline_stream_mixin import (
            TimelineStreamMixin,
        )
    ...
    def test_stream_calls_finalise_helper_when_present(self) -> None:
        ...
        from doclatticeserver.llms.agents.core_agents import FinalEvent
        from doclatticeserver.llms.agents.timeline_stream_mixin import (
            TimelineStreamMixin,
        )
...
class TestIsPublicHelper(TestCase):
    ...
    def test_none_object_is_not_public(self) -> None:
        from doclatticeserver.llms.agents.core_agents import _is_public
...
class TestAssertAccessHelper(TestCase):
    ...
    def test_anonymous_blocked_on_private(self) -> None:
        from doclatticeserver.llms.agents.core_agents import _assert_access
...
class TestResolveToolsCallerForms(TestCase):
    ...
    def test_string_resolves_via_registry(self) -> None:
        ...
        from doclatticeserver.llms.tools import core_tools as _core_tools_module
        ...
        with patch(
            "doclatticeserver.llms.api.ToolAPI.from_function",
            side_effect=lambda fn, **kw: fn,
        ):
            with patch(
                "doclatticeserver.llms.api._registered_tools",
    ...
    def test_callable_resolves_via_from_function(self) -> None:
        ...
        # actual call site.
        with patch(
            "doclatticeserver.llms.api.CoreTool.from_function",
...
class TestVectorStoreAPICreate(TestCase):
    ...
    def test_create_uses_dedicated_vector_store_setting(self) -> None:
        from doclatticeserver.llms.api import VectorStoreAPI

        sentinel = object()
        with patch(
            "doclatticeserver.llms.api.UnifiedVectorStoreFactory.create_vector_store",
            return_value=sentinel,
        ) as mock_create:
