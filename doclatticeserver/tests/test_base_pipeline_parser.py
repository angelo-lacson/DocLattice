...
from doclatticeserver.annotations.models import Annotation, Relationship, AnnotationLabel
...
class MockParser(BaseParser):
    ...
    an DocLatticeDocExport with both annotations and relationships.
    """
    ...
    def parse_document(
        ...
        annotation_data: List[DocLatticeAnnotationPythonType] = [
            {
                "id": "a1",
        ...
        relationship_data: List[DocLatticeRelationshipPythonType] = [
            {
                "id": "r1",
        ...
        # Build the DocLatticeDocExport
        export_data: DocLatticeDocExport = {
...
class TestMockParser(TestCase):
    ...
    def test_mock_parser_relationship_import(self):
        ...
        self.assertIn(self.doc, self.corpus.documents.all(), "Document should be in the specified corpus.")

        # 4) Verify annotations
        self.assertEqual(Annotation.objects.count(), 2, "Should have created two annotations.")
        ...
        self.assertEqual(ann_b.parent, ann_a, "Second annotation should reference first as its parent.")
        self.assertTrue(ann_b.structural, "Second annotation's 'structural' flag should be True.")

        # 5) Verify relationship creation
        self.assertEqual(Relationship.objects.count(), 1, "Should have created one relationship.")
        relationship = Relationship.objects.first()
        self.assertEqual(relationship.relationship_label.text, "MockRelLabelA", "Relationship should have correct label.")

        self.assertEqual(relationship.source_annotations.count(), 1)
        self.assertEqual(relationship.target_annotations.count(), 1)

        self.assertIn(ann_a, relationship.source_annotations.all(), "ann_a should be in relationship's source annotations.")
        self.assertIn(ann_b, relationship.target_annotations.all(), "ann_b should be in relationship's target annotations.")
