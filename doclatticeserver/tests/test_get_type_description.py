...
from doclatticeserver.llms.agents.pydantic_ai_agents import _get_type_description
...
class TestGetTypeDescription(TestCase):
    ...
    def test_list_types(self):
        ...
        self.assertEqual(result, "a JSON array where each element is a plain string value")
        ...
        self.assertEqual(result, "a JSON array where each element is a JSON array where each element is a plain string value")
        ...
        self.assertIn("a JSON array where each element is a JSON object matching the 'SimpleModel' model with this schema:", result)
    ...
    def test_tuple_types(self):
        ...
        self.assertEqual(
            result, 
            "a JSON array with exactly 3 elements: [a plain string value, an integer value, a boolean value (true or false)]"
        )
        
        # Variable length tuple (with ellipsis)
        result = _get_type_description(Tuple[str, ...])
        self.assertEqual(result, "a JSON array of variable length where each element is a plain string value")
        ...
        self.assertEqual(result, "a JSON array with exactly 1 elements: [an integer value]")

    def test_set_types(self):
        """Test descriptions for Set types."""
        # Set of strings
        result = _get_type_description(Set[str])
        self.assertEqual(result, "a JSON array of unique values where each element is a plain string value")
        
        # Set of integers
        result = _get_type_description(Set[int])
        self.assertEqual(result, "a JSON array of unique values where each element is an integer value")
    ...
    def test_pydantic_models(self):
        ...
        self.assertIn("a JSON object matching the 'SimpleModel' model with this schema:", result)
        ...
        self.assertIn("a JSON object matching the 'NestedModel' model with this schema:", result)
    ...
    def test_complex_nested_types(self):
        ...
        expected = "a JSON array where each element is a JSON array with exactly 2 elements: [a plain string value, an integer value]"
        ...
        expected = "a JSON array of unique values where each element is a JSON array with exactly 2 elements: [an integer value, a plain string value]"
        self.assertEqual(result, expected)
        ...
        for type_to_test in schema_types:
            with self.subTest(type=type_to_test):
                ...
                if "schema:" in result:
                    ...
                    if "type" in parsed:
                        self.assertIn(parsed["type"], ["object", "array", "string", "number", "integer", "boolean", "null"]) 