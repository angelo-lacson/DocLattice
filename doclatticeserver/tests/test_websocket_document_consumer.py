        ...
        for framework in ("llama_index", "pydantic_ai"):
            ...
            with self.subTest(default_framework=framework):
                with override_settings(
                    ...
                    encoded_corpus_id = quote(
                        to_global_id("CorpusType", self.corpus.id)
                    )
                    ws_path = f"ws/document/{encoded_graphql_doc_id}/query/corpus/{encoded_corpus_id}/?token={self.token}"  # noqa: E501

                    communicator = WebsocketCommunicator(self.application, ws_path)
                    connected, _ = await communicator.connect()
                    self.assertTrue(
                        connected, "WebSocket for new conversation should connect."
                    )
        ...
        for framework in ("llama_index", "pydantic_ai"):
            ...
            with self.subTest(default_framework=framework):
                with override_settings(
                    ...
                    actual_history_for_log = (
                        await history_check_agent.get_conversation_messages()
                    )
                    print(
                        f"Fetched {len(actual_history_for_log)} messages via agent API for "
                        "conversation {conversation.id} for logging."
                    )
                    ...
                    connected, _ = await communicator.connect()
                    self.assertTrue(
                        connected, "WebSocket for loaded conversation should connect."
                    )
...
class ConversationSourceLoggingTestCase(DocumentConversationWebsocketTestCase):
    ...
    async def _assert_sources_persisted(self, conversation: Conversation) -> None:
        ...
        llm_messages = await database_sync_to_async(
            lambda: list(
                conversation.messages.filter(msg_type="LLM").exclude(
                    Q(metadata__sources=None) | Q(metadata__sources=[])
                )
            )
        )()
        self.assertTrue(
            llm_messages,
            "Expected at least one LLM message with non-empty `metadata['sources']`",
        )
    ...
    async def test_sources_are_logged_for_new_conversation(self) -> None:
        """
        Same happy-path flow as `test_document_conversation_ws` but
        with additional assertions about sources.
        """
        conversation = await self._run_full_conversation_flow()  # reuse helper in base
    ...
    async def test_sources_are_logged_for_loaded_conversation(self) -> None:
        conversation = await self._run_loaded_conversation_flow()  # reuse helper
        await self._assert_sources_persisted(conversation)
