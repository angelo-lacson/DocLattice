...
class ConversationSourceLoggingTestCase(DocumentConversationWebsocketTestCase):
    ...
    async def _test_sources_for_framework(self, framework: str, cassette_name: str, test_type: str) -> None:
    ...
    async def test_sources_are_logged_for_loaded_conversation(self) -> None:
        ...
        for framework in ("llama_index", "pydantic_ai"):
            with self.subTest(framework=framework):
                await self._test_sources_for_framework(
                    framework=framework,
                    cassette_name=f"fixtures/vcr_cassettes/test_document_conversation_sources_ws_loaded_{framework}.yaml",
                    test_type="loaded"
                )
