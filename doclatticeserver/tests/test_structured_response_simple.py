...
def log_structured_result_simple(test_name: str, result: any):
    ...
    try:
        ...
        structured_data_logger_simple.info(f"RESULT DATA:")
        structured_data_logger_simple.info(json.dumps(json_result, indent=2, ensure_ascii=False))
        structured_data_logger_simple.info("")
        
    except Exception as e:
        structured_data_logger_simple.error(f"Error logging result for {test_name}: {str(e)}")
...
class TestStructuredResponseBasic(BaseFixtureTestCase):
    ...
    async def test_llama_index_returns_none(self):
        ...
        # Log the structured result for inspection
        log_structured_result_simple("test_llama_index_returns_none", result)
    ...
    async def test_custom_vs_default_prompt(self):
        ...
        log_structured_result_simple("test_custom_vs_default_prompt", {
