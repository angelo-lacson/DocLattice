...
Covers the three tools registered by ``doclatticeserver.llms.tools.core_tools.caml_article``:
...
from doclatticeserver.llms.tools.core_tools.caml_article import (
...
from doclatticeserver.llms.vector_stores.core_vector_stores import VectorSearchResult
...
def _create_caml_doc(corpus: Corpus, user, *, content: str = SAMPLE_CAML) -> Document:
    ...
    (:func:`doclatticeserver.documents.versioning.import_document` with
...
class ReadCorpusCamlArticleTests(TransactionTestCase):
    ...
    def setUp(self):
        ...
        #     autouse ``media_storage`` fixture in doclatticeserver/conftest.py)
...
class ProposeCamlCitationMatchTests(TransactionTestCase):
    ...
    def _patch_async_search(self, results):
        ...
        return patch.multiple(
            "doclatticeserver.llms.vector_stores.core_vector_stores"
...
class ApplyCamlArticleEditCreatesVersionTreeSiblingTest(TransactionTestCase):
    ...
    ``doclatticeserver.documents.versioning.import_document`` (the canonical
