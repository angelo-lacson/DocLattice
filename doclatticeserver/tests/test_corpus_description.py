...
were relocated to ``doclatticeserver.corpuses.services.description_cache``
...
:func:`doclatticeserver.documents.versioning.import_document`. The classes
...
class UpdateDescriptionWritesThroughCamlTest(TestCase):
    ...
    :func:`doclatticeserver.documents.versioning.import_document`. The
    ...
    def test_update_description_creates_caml_doc_if_missing(self):
        from doclatticeserver.corpuses.services.corpus_service import (
            CorpusService,
        )
        ...
        # ``doclatticeserver/corpuses/signals.py``). Under ``TestCase``
