"""Coverage tests for under-tested modules inside ``doclatticeserver/analyzer``.
...
from doclatticeserver.analyzer import checks as analyzer_checks
from doclatticeserver.analyzer import startup as analyzer_startup
from doclatticeserver.analyzer.admin_views import AnalyzerSyncView
from doclatticeserver.analyzer.models import Analysis, Analyzer, GremlinEngine
from doclatticeserver.analyzer.signals import (
    handle_analysis_completion,
    install_gremlin_on_creation,
)
from doclatticeserver.analyzer.utils import (
...
class InstallGremlinOnCreationTests(TransactionTestCase):
    ...
    def test_created_true_schedules_on_commit_chain(self) -> None:
        ...
        with patch("doclatticeserver.analyzer.signals.transaction") as mock_tx, patch(
            "doclatticeserver.analyzer.signals.request_gremlin_manifest"
        ) as mock_req, patch(
            "doclatticeserver.analyzer.signals.install_analyzer_task"
        ) as mock_install, patch(
            "doclatticeserver.analyzer.signals.chain"
        ) as mock_chain:
...
class HandleAnalysisCompletionTests(TestCase):
    ...
    def test_logs_when_status_is_complete_literal(self) -> None:
        with self.assertLogs("doclatticeserver.analyzer.signals", level="INFO") as cm:
            handle_analysis_completion(
                sender=Analysis,
    ...
    def test_silent_when_status_is_completed_enum_value(self) -> None:
        ...
        signals_logger = logging.getLogger("doclatticeserver.analyzer.signals")
        with self.assertLogs("doclatticeserver.analyzer.signals", level="INFO") as cm:
...
class CheckUnsyncedAnalyzersTests(TestCase):
    ...
    def test_returns_warning_when_task_not_in_db(self) -> None:
        ...
        with patch(
            "doclatticeserver.utils.celery_tasks.celery_app"
        ) as mock_celery, patch(
            "doclatticeserver.utils.celery_tasks.get_doc_analyzer_task_by_name"
        ) as mock_get, patch(
            "doclatticeserver.analyzer.models.Analyzer"
        ) as mock_analyzer:
    ...
    def test_swallows_runtime_errors_silently(self) -> None:
        ...
        with patch("doclatticeserver.utils.celery_tasks.celery_app", new=broken):
...
class AnalyzerSyncViewTests(TestCase):
    ...
    def test_get_available_analyzers_shape(self) -> None:
        ...
        with patch(
            "doclatticeserver.analyzer.admin_views.celery_app"
        ) as mock_celery, patch(
            "doclatticeserver.analyzer.admin_views.get_doc_analyzer_task_by_name"
        ) as mock_get, patch(
            "doclatticeserver.analyzer.admin_views.Analyzer"
        ) as mock_analyzer:
            mock_celery.tasks = {
                SYNCED: fake_task,
                UNSYNCED: fake_task,
    ...
    def test_post_with_perm_invokes_auto_create(self) -> None:
        ...
        with patch(
            "doclatticeserver.analyzer.admin_views.auto_create_doc_analyzers"
        ) as mock_auto:
            view = AnalyzerSyncView.as_view()
            resp = view(request)
        mock_auto.assert_called_once()
...
class AutoCreateDocAnalyzersEdgeCases(TestCase):
    ...
    def test_no_users_returns_early(self) -> None:
        ...
        with patch(
            ...
            "doclatticeserver.analyzer.utils.get_doc_analyzer_task_by_name",
    ...
    def test_celery_app_unavailable_short_circuits(self) -> None:
        with patch("doclatticeserver.analyzer.utils.celery_app", None):
...
class SyncDocAnalyzersCommandTests(TestCase):
    ...
    def test_dry_run_does_not_create(self) -> None:
        ...
        with patch(
            ...
            "doclatticeserver.analyzer.management.commands.sync_doc_analyzers."
            "auto_create_doc_analyzers"
        ) as mock_auto:
...
class AnalysisCallbackViewHappyPathTests(TransactionTestCase):
    ...
    def _post_callback(self, body: dict, token: str | None) -> Any:
        ...
        from doclatticeserver.analyzer.views import AnalysisCallbackView
    ...
    def test_valid_callback_with_valid_body_succeeds(self) -> None:
        ...
        # Body must conform to ``DocLatticeGeneratedCorpusPythonType``:
        ...
        with patch(
            "doclatticeserver.analyzer.views.import_analysis"
        ) as mock_import, patch(
            "doclatticeserver.analyzer.views._create_analysis_notification"
        ) as mock_notif:
...
class CreateAnalysisNotificationTests(TransactionTestCase):
    ...
    def test_success_creates_complete_notification(self) -> None:
        from doclatticeserver.analyzer import views

        with patch(
            "doclatticeserver.analyzer.views.broadcast_notification_via_websocket"
        ) as mock_bcast:
    ...
    def test_swallows_exception_and_logs(self) -> None:
        ...
        with patch(
            "doclatticeserver.analyzer.views.Notification.objects.create",
            side_effect=RuntimeError("db down"),
        ), self.assertLogs("doclatticeserver.analyzer.views", level="WARNING") as cm:
