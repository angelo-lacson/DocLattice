...
``doclatticeserver/tests/test_llm_runtime_config.py`` pins the *resolver*
...
class MemoryCurationModelResolutionTests(RetargetingTestCase):
    ...
    def _run_curation(self) -> list[str]:
        ...
        with patch(
            "doclatticeserver.llms.model_factory.abuild_agent_model",
            side_effect=_capture,
        ), patch(
            "doclatticeserver.llms.agents.pydantic_ai_factory."
...
class CitationExtractorModelResolutionTests(RetargetingTestCase):
    ...
    def _resolve(self, model=None) -> str:
        ...
        with patch(
            "doclatticeserver.enrichment.llm_citation_extractor.abuild_agent_model",
            side_effect=_capture,
        ):
...
class WebLocatorModelResolutionTests(RetargetingTestCase):
    ...
    def test_install_default_is_honoured(self):
        from doclatticeserver.pipeline.authority_source_providers.agentic_web_locator_provider import (  # noqa: E501
...
class HotSwapTests(RetargetingTestCase):
    ...
    def test_credential_cache_invalidates_on_provider_key_rotation(self):
        ...
        from doclatticeserver.llms.model_factory import (
            _get_db_credentials,
