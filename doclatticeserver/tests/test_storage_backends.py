...
class StorageBackendConfigTest(TestCase):
    ...
    def test_aws_storage_configuration(self):
        ...
        if settings.STORAGE_BACKEND == "AWS":
            ...
            from doclatticeserver.utils.storages import MediaRootS3Boto3Storage, StaticRootS3Boto3Storage
    ...
    def test_gcp_storage_configuration(self):
        ...
        if settings.STORAGE_BACKEND == "GCP":
            ...
            # Just verify the storage classes exist and can be imported
            from doclatticeserver.utils.storages import MediaRootGoogleCloudStorage, StaticRootGoogleCloudStorage
...
class StorageUtilsTest(TestCase):
    ...
    def test_analyzer_utils_gcp_storage(self):
        ...
        mock_obj = Mock()
        mock_obj.test_field = Mock()
        mock_obj.test_field.url = "https://storage.googleapis.com/test-bucket/media/test.pdf"
        
        # With GCP storage, should return the GCS URL directly
        url = get_django_file_field_url("test_field", mock_obj)
        self.assertEqual(url, "https://storage.googleapis.com/test-bucket/media/test.pdf")