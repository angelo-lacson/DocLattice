...
from doclatticeserver.tasks.doc_tasks import ingest_doc, retry_document_processing
...
class IngestDocPermissionCheckTests(TestCase):
    ...
    def test_ingest_doc_refuses_when_user_lacks_permission(self):
        ...
        with self.assertLogs(
            "doclatticeserver.tasks.doc_tasks", level=logging.ERROR
        ) as captured:
    ...
    def test_retry_document_processing_refuses_for_nonexistent_document(self):
        ...
        with self.assertLogs(
            "doclatticeserver.tasks.doc_tasks", level=logging.WARNING
        ) as captured:
