...
from doclatticeserver.tasks.telemetry_tasks import send_usage_heartbeat
from doclatticeserver.users.models import Installation, User
...
class TelemetryTestCase(TestCase):
    ...
    def test_record_event_success(self):
        ...
        self.assertEqual(call_args["event"], "doclattice.test_event")
        self.assertEqual(call_args["properties"]["package"], "doclattice")
        self.assertEqual(
...
class UsageHeartbeatTestCase(TestCase):
    ...
    @patch("doclatticeserver.tasks.telemetry_tasks.record_event")
...
class TelemetryMigrationTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.migration_module = importlib.import_module(
            "doclatticeserver.users.migrations.0024_setup_telemetry_periodic_task"
        )
    ...
    def test_setup_telemetry_task_creates_periodic_task(self):
        ...
        self.assertEqual(
            task.task,
            "doclatticeserver.tasks.telemetry_tasks.send_usage_heartbeat",
        )
