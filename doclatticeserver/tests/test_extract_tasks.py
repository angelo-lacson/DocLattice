...
from doclatticeserver.tasks.data_extract_tasks import doc_extract_query_task
...
class ExtractOrchestrationTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.doc1 = Document.objects.create(
            ...
            creator=self.user,
            file_type="text/plain",
            txt_extract_file=self._create_txt_file("Document 1 content with payment terms")
        )
        self.doc2 = Document.objects.create(
            title="Test Document 2", 
            creator=self.user,
            file_type="text/plain",
            txt_extract_file=self._create_txt_file("Document 2 content with delivery terms")
        )
        self.doc3 = Document.objects.create(
            title="Test Document 3",
            creator=self.user,
            file_type="text/plain",
            txt_extract_file=self._create_txt_file("Document 3 content with warranty terms")
        )
    ...
    def test_extract_with_vcr(self):
        ...
        with vcr.use_cassette(
            ...
            assert extract.extracted_datacells.count() == len(self.fieldset.columns.all())
