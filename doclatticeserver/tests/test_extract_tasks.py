...
class ExtractsTaskTestCase(BaseFixtureTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.column = Column.objects.create(
            ...
            agentic=True,
            creator=self.user,
            task_name="doclatticeserver.tasks.data_extract_tasks.llama_index_react_agent_query",
        )
        # Additional column
        self.column = Column.objects.create(
            fieldset=self.fieldset,
    ...
    @patch(
        "doclatticeserver.tasks.data_extract_tasks.StructuredPlannerAgent.chat",
        return_value="mocked agent response",
    )
    @patch("marvin.cast", return_value="value1")
    @patch("marvin.extract", return_value=["value1", "value2"])
    @patch("doclatticeserver.tasks.extract_orchestrator_tasks.chord")
    @patch("doclatticeserver.tasks.extract_orchestrator_tasks.group")
    @patch("doclatticeserver.tasks.extract_orchestrator_tasks.get_task_by_name")
    @patch("doclatticeserver.tasks.data_extract_tasks.oc_llama_index_doc_query.delay")
    @patch("doclatticeserver.tasks.data_extract_tasks.llama_index_react_agent_query")
    @patch("doclatticeserver.shared.decorators.asyncio.new_event_loop")
    ...
    @patch("doclatticeserver.utils.embeddings.calculate_embedding_for_text")
    @patch("tiktoken.encoding_for_model")
    @patch("doclatticeserver.extracts.models.Extract.objects.get")
    @patch("doclatticeserver.extracts.models.Datacell.objects.create")
    @patch("doclatticeserver.documents.models.DocumentAnalysisRow.objects.filter")
    @patch("doclatticeserver.extracts.models.Extract.refresh_from_db")
    @patch("doclatticeserver.extracts.models.Datacell.refresh_from_db")
    @patch("doclatticeserver.extracts.models.Datacell.save")
    @patch("doclatticeserver.documents.models.DocumentAnalysisRow.save")
    @patch("asgiref.sync.sync_to_async")
    @patch(
        "doclatticeserver.tasks.extract_orchestrator_tasks.set_permissions_for_obj_to_user"
    )
    @patch("doclatticeserver.tasks.extract_orchestrator_tasks.transaction.atomic")
    @patch("doclatticeserver.tasks.extract_orchestrator_tasks.DocumentAnalysisRow")
