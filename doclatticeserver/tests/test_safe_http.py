...
Run with:
    docker compose -f test.yml -p doclattice run --rm django pytest \
        doclatticeserver/tests/test_safe_http.py -q
"""
...
from doclatticeserver.utils import safe_http as _safe_http_module
...
class TestSafeFetchBytesCredentialStripping:
    def test_credentials_stripped_on_cross_host_redirect(self):
        ...
        assert "DocLattice" in captured[1].get("User-Agent", "")
