...
check (``doclatticeserver/shared/checks.py``) so violations also fail
...
``doclatticeserver/shared/architecture_audit.py``.
"""
...
from doclatticeserver.shared.architecture_audit import (
    ALLOWED_FILES,
    GRAPHQL_DIR,
    format_violation,
...
def test_graphql_module_uses_service_layer(module_path: Path) -> None:
    ...
    ``doclattice.E001``) so a dev who's never seen this rule before
...
def test_django_system_check_is_registered() -> None:
    ...
    from doclatticeserver.shared.checks import check_graphql_service_layer

    assert tag_exists("architecture"), (
        "The ``architecture`` system-check tag is not registered. Confirm "
        "``doclatticeserver.users.apps.UsersConfig.ready`` still imports "
        "``doclatticeserver.shared.checks``."
    )
    ...
    assert check_graphql_service_layer in architecture_checks, (
        ...
        "decorator on ``doclatticeserver.shared.checks.check_graphql_service_layer`` "
        "is still in place."
    )
...
def test_django_system_check_uses_same_audit() -> None:
    ...
    ``doclatticeserver.shared.architecture_audit.audit_graphql_modules`` —
    running the registered check and the audit function side-by-side
    pins them to agree.
    """
    from django.core.checks import run_checks

    from doclatticeserver.shared.architecture_audit import audit_graphql_modules

    audit_hits = audit_graphql_modules()
    check_results = run_checks(tags=["architecture"])
    arch_errors = [r for r in check_results if r.id == "doclattice.E001"]
...
def test_scan_forbidden_detects_import_from_alias() -> None:
    ...
    source = (
        "from doclatticeserver.permissions import visible_to_user\n"
        "\n"
...
def test_format_violation_for_each_known_identifier(tmp_path: Path) -> None:
    ...
    from doclatticeserver.shared.architecture_audit import FORBIDDEN_NAMES
...
def test_iter_graphql_modules_returns_empty_when_dir_missing(
    ...
    from doclatticeserver.shared import architecture_audit
...
def test_django_check_emits_error_when_audit_reports_violations(
    ...
    from doclatticeserver.shared import checks as arch_checks
    ...
    error = errors[0]
    assert error.id == "doclattice.E001"
