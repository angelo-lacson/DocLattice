...
discover the central ``doclatticeserver/tasks/`` package directly —
``autodiscover_tasks()`` only finds per-app ``tasks.py`` modules, and the
only one that exists is ``doclatticeserver/users/tasks.py``. The central
package is imported as a *side effect* of Django boot (admin/signal/service
imports eventually trigger ``doclatticeserver/tasks/__init__.py``), and
...
def test_required_tasks_register_on_fresh_worker_boot() -> None:
    ...
    imported in ``doclatticeserver/tasks/__init__.py``), so the real
    ...
    assert not missing, (
        ...
        "doclatticeserver/tasks/__init__.py (and __all__) so the "
