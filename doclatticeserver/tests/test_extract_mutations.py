...
from doclatticeserver.extracts.models import Extract, Fieldset, LanguageModel
...
class ExtractsMutationTestCase(TestCase):
    ...
    def test_start_extract_mutation(self):
        ...
        with patch("doclatticeserver.tasks.extract_orchestrator_tasks.run_extract.s") as mock_task:
            result = self.client.execute(mutation)
            self.assertIsNone(result.get("errors"))
