"""Unit tests for ``doclatticeserver.llms.history_processors``.
...
def test_no_shrinkable_content_logs_info(caplog):
    ...
    with caplog.at_level(
        logging.INFO, logger="doclatticeserver.llms.history_processors"
    ):
        result = _run(messages, deps)
...
def test_callback_exception_does_not_propagate(caplog):
    ...
    with caplog.at_level(
        logging.ERROR, logger="doclatticeserver.llms.history_processors"
    ):
        result = _run(messages, deps)
