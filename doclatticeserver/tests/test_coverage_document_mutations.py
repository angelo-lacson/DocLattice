...
from doclatticeserver.documents.versioning import delete_document, import_document
...
class RetryDocumentProcessingMutationTests(TestCase):
    ...
    def test_success_queues_retry_task(self):
        ...
        with patch(
            "doclatticeserver.tasks.doc_tasks.retry_document_processing.delay"
        ) as mock_delay:
            result = self._execute(self.owner, doc)
...
class StartCorpusExportEdgeCaseTests(BaseFixtureTestCase):
    ...
    def setUp(self):
        ...
        # The real DOCLATTICE export chain (build_label_lookups_task)
    ...
    def test_doclattice_v2_dispatches_export_task(self):
        with patch(
            "doclatticeserver.tasks.export_tasks_v2.package_corpus_export_v2.delay"
        ) as mock_delay:
            result = self._execute("DOCLATTICE_V2")
