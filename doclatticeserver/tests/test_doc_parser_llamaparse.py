...
from doclatticeserver.pipeline.parsers.llamaparse_parser import LlamaParseParser
from doclatticeserver.types.dicts import BoundingBoxPythonType, TokenIdPythonType
from doclatticeserver.utils.pdf_token_extraction import find_image_tokens_in_bounds
...
class TestLlamaParseParser(TestCase):
    ...
    @patch("doclatticeserver.pipeline.parsers.llamaparse_parser.find_tokens_in_bbox")
    @patch(
        "doclatticeserver.pipeline.parsers.llamaparse_parser.extract_pawls_tokens_from_pdf"
    )
    @patch("llama_parse.LlamaParse")
    @patch("doclatticeserver.pipeline.parsers.llamaparse_parser.default_storage.open")
...
class TestLlamaParseParserLayoutOnlyProcessing(TestCase):
    ...
    def test_parse_document_layout_only_processing(
        ...
        This tests the layout-only processing path in _convert_json_to_doclattice.
        """
        mock_file = MagicMock()
        mock_file.read.return_value = b"mock pdf content"
        mock_open.return_value.__enter__.return_value = mock_file
...
class TestLlamaParseParserImageExtraction(TestCase):
    ...
    @patch("doclatticeserver.pipeline.parsers.llamaparse_parser.crop_image_from_pdf")
    @patch(
        "doclatticeserver.pipeline.parsers.llamaparse_parser.extract_images_from_pdf"
    )
    @patch("doclatticeserver.pipeline.parsers.llamaparse_parser.find_tokens_in_bbox")
    @patch(
        "doclatticeserver.pipeline.parsers.llamaparse_parser.extract_pawls_tokens_from_pdf"
...
class TestLlamaParseParserTextConversion(TestCase):
    ...
    def test_convert_text_to_doclattice(self):
        """Test _convert_text_to_doclattice method."""
        ...
        result = parser._convert_text_to_doclattice(self.doc, mock_docs)
    ...
    def test_convert_text_to_doclattice_empty_docs(self):
        """Test _convert_text_to_doclattice with empty documents."""
        parser = LlamaParseParser()
