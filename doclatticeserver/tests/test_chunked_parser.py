...
from doclatticeserver.pipeline.base.chunk_reassembler import (
    offset_annotation as _offset_annotation,
)
from doclatticeserver.pipeline.base.chunk_reassembler import (
    offset_relationship as _offset_relationship,
)
from doclatticeserver.pipeline.base.chunked_parser import (
    BaseChunkedParser,
...
class TestOffsetAnnotation(TestCase):
    ...
    def test_basic_offset(self):
        ...
        _offset_annotation(cast(DocLatticeAnnotationPythonType, ann), 10, "c0_")
...
class TestOffsetRelationship(TestCase):
    ...
    def test_basic_offset(self):
        ...
        _offset_relationship(cast(DocLatticeRelationshipPythonType, rel), "c1_")
...
class TestBaseChunkedParserIntegration(TestCase):
    ...
    @patch("doclatticeserver.pipeline.base.chunked_parser.default_storage.open")
    ...
    @patch("doclatticeserver.pipeline.base.chunked_parser.default_storage.open")
    @patch("doclatticeserver.pipeline.base.chunked_parser.time.sleep")
...
class TestReassembleAndFinalize(TestCase):
    def test_streams_results_and_returns_combined(self):
        from doclatticeserver.pipeline.chunk_artifacts import write_chunk_result
...
class TestOverlapWiring(TestCase):
    ...
    def test_prepare_chunk_inputs_uses_overlap_extended_ranges(self):
        from doclatticeserver.pipeline.chunk_artifacts import read_chunk_pdf
    ...
    def test_chunk_overlap_defaults_to_constant(self):
        from doclatticeserver.constants import DEFAULT_CHUNK_OVERLAP

        self.assertEqual(BaseChunkedParser.chunk_overlap, DEFAULT_CHUNK_OVERLAP)


def _annotation_signatures_by_id(result: DocLatticeDocExport) -> dict:
...
class TestChunkedOverlapGoldenEquivalence(TestCase):
    ...
    def _single_shot(self) -> DocLatticeDocExport:
        parser = _OverlapGoldenParser()
    ...
    def _chunked_with_overlap(self) -> DocLatticeDocExport:
        parser = _OverlapGoldenParser()
        parser.max_pages_per_chunk = 4
        parser.min_pages_for_chunking = 4
        parser.chunk_overlap = 2
