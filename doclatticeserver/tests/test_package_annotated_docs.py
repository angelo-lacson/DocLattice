...
from doclatticeserver.tasks.export_tasks import package_annotated_docs
...
class PackageAnnotatedDocsSkipTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        # ``DocLatticeExportDataJsonPythonType``). LabelSet is required
    ...
    def test_skips_failed_placeholder_and_packages_successful_docs(self) -> None:
        ...
        with patch(
            "doclatticeserver.tasks.export_tasks.finalize_export",
            side_effect=capture_fn,
        ), self.assertLogs(
            "doclatticeserver.tasks.export_tasks", level="WARNING"
        ) as log_cm:
    ...
    def test_raises_when_corpus_export_packager_returns_none(self) -> None:
        ...
        was silently written into ``DocLatticeExportDataJsonPythonType``).
        """
        good_doc = (
            "good.pdf",
            self.fake_pdf_b64,
        ...
        with patch(
            "doclatticeserver.tasks.export_tasks.package_corpus_for_export",
            return_value=None,
    ...
    def test_raises_when_label_set_packager_returns_none(self) -> None:
        ...
        with patch(
            "doclatticeserver.tasks.export_tasks.package_label_set_for_export",
            return_value=None,
        ), self.assertRaises(RuntimeError) as cm:
            package_annotated_docs(
                burned_docs=(good_doc,),
