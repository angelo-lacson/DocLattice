...
# Document file fields materialised from doclatticeserver/tests/fixtures/files/.
...
class _FixtureSetupMixin:
    ...
    * :meth:`_materialize_fixture_files` — copies the fixture files into the
      ...
      fixture (``doclatticeserver/conftest.py``) repoints ``MEDIA_ROOT`` at a
    ...
    def copy_fixture_file(fixture_path: str, dest_path: str) -> None:
        ...
        Args:
            fixture_path: Path relative to the 'files' directory in
                'doclatticeserver/tests/fixtures' (a leading 'files/' is
                stripped if present).
