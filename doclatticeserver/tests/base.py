...
class BaseFixtureTestCase(TransactionTestCase):
    ...
    fixtures: ClassVar[list[str]] = [
        "doclatticeserver/tests/fixtures/test_data.json",
    ]
    ...
    def copy_fixture_file(self, fixture_path: str, dest_path: str) -> None:
        ...
        Args:
            fixture_path: Path relative to the 'files' directory in 'doclatticeserver/tests/fixtures'
        ...
        src = Path("doclatticeserver/tests/fixtures/files") / fixture_path
    ...
    def setUp(self) -> None:
        ...
        for doc in self.docs:
            ...
            # -------------------------------------------------------------- #
            #  If fixture did not provide an md_summary_file, create one     #
            # -------------------------------------------------------------- #
            if not getattr(doc, "md_summary_file"):
                summary_rel_path = f"md_summaries/{doc.id}_summary.md"
                summary_abs_path = Path(settings.MEDIA_ROOT) / summary_rel_path
                summary_abs_path.parent.mkdir(parents=True, exist_ok=True)
                summary_abs_path.write_text(
                    f"# Summary for Document {doc.id}\n\n"
                    "Autogenerated test summary – replace with real content if needed.",
                    encoding="utf-8",
                )
                doc.md_summary_file = summary_rel_path
