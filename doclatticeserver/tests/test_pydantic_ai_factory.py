"""Regression tests for ``doclatticeserver.llms.agents.pydantic_ai_factory``.
...
DocLattice' chat() flow), so all agent construction must funnel
...
SENTINEL_INSTRUCTION = "DOCLATTICE_SENTINEL_INSTRUCTION_ISSUE_1451"
...
def test_instructions_survive_non_empty_message_history() -> None:
    ...
    DocLattice' ``chat()`` flow persists the user's HUMAN message
...
def test_factory_injects_in_run_history_processor() -> None:
    ...
    from doclatticeserver.llms.history_processors import (
