...
class TestEmbeddingsTask(unittest.TestCase):
    ...
    @patch("doclatticeserver.corpuses.models.Corpus")
    @patch("doclatticeserver.pipeline.utils.get_component_by_name")
    @patch("doclatticeserver.pipeline.utils.find_embedder_for_filetype")
    @patch("doclatticeserver.pipeline.utils.get_default_embedder")
...
class TestAnnotationSignals(unittest.TestCase):
    ...
    @patch(
        "doclatticeserver.annotations.signals.calculate_embedding_for_annotation_text"
    )
    ...
    @patch("doclatticeserver.annotations.signals.calculate_embedding_for_note_text")
    def test_process_note_on_create(self, mock_calc_embedding):
        ...
        """
        from doclatticeserver.annotations.signals import process_note_on_create_atomic
...
class TestEmbeddingTask(unittest.TestCase):
    ...
    @patch("doclatticeserver.tasks.embeddings_task.Annotation")
    ...
    @patch("doclatticeserver.tasks.embeddings_task.Corpus")
    @patch("doclatticeserver.tasks.embeddings_task.get_component_by_name")
    @patch("doclatticeserver.tasks.embeddings_task.get_default_embedder")
    @patch("doclatticeserver.tasks.embeddings_task.Annotation")
    @patch("doclatticeserver.tasks.embeddings_task.get_default_embedder_path")
...
class TestMultimodalEmbeddingTask(unittest.TestCase):
    ...
    def test_create_text_embedding_empty_text(self):
        ...
        from doclatticeserver.tasks.embeddings_task import _create_text_embedding
...
class TestEmbeddingGenerationError(unittest.TestCase):
    ...
    def test_embedding_generation_error_is_exception_subclass(self):
        ...
        from doclatticeserver.tasks.embeddings_task import EmbeddingGenerationError
...
class TestBytesDecoding(unittest.TestCase):
    ...
    @patch("doclatticeserver.tasks.embeddings_task.Document")
...
class TestArrayFormatHandling(unittest.TestCase):
    ...
    def test_microservice_embedder_handles_1d_array(self):
        ...
        from doclatticeserver.pipeline.embedders.sent_transformer_microservice import (
...
class TestMicroserviceEmbedderBatchUnit(unittest.TestCase):
    """Unit tests for MicroserviceEmbedder.embed_texts_batch() with mocked HTTP."""
    ...
    def setUp(self):
        ...
        self.embedder = MicroserviceEmbedder()

    @patch(
        "doclatticeserver.pipeline.embedders.sent_transformer_microservice.requests.post"
    )
    def test_batch_success_2d(self, mock_post):
        """Successful batch returns list of embedding vectors."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        result = self.embedder.embed_texts_batch(["a", "b", "c"])
    ...
    def test_batch_3d_squeeze(self, mock_post):
        """3D array response is squeezed to 2D."""
        embeddings_3d = [[[0.1] * 384], [[0.2] * 384]]
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"embeddings": embeddings_3d}
        mock_post.return_value = mock_response
    ...
    def test_batch_nan_per_element(self, mock_post):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"embeddings": embeddings_with_nan}
        mock_post.return_value = mock_response
    ...
    def test_batch_client_error_4xx(self, mock_post):
        """4xx client error returns None."""
        mock_response = MagicMock()
        mock_response.status_code = 422
        mock_post.return_value = mock_response

        result = self.embedder.embed_texts_batch(["a"])
    ...
    def test_batch_server_error_5xx(self, mock_post):
    ...
    def test_batch_exception(self, mock_post):
    ...
    def test_batch_truncates_over_100(self, mock_post):
        """Batch >100 texts is truncated to 100."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"embeddings": [[0.1] * 384] * 100}
        mock_post.return_value = mock_response

        texts = [f"Text {i}" for i in range(150)]
        self.embedder.embed_texts_batch(texts)
    ...
    def test_batch_api_key_header(self, mock_post):
        ...
        self.embedder.embed_texts_batch(
            ["a"],
            embeddings_microservice_url="http://test:8000",
            vector_embedder_api_key="test-key",
        )
    ...
    def test_batch_posts_to_batch_endpoint(self, mock_post):
        """Batch calls POST to /embeddings/batch."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"embeddings": [[0.1] * 384]}
        mock_post.return_value = mock_response
        ...
        self.embedder.embed_texts_batch(
            ["a"], embeddings_microservice_url="http://test:8000"
        )

        url = mock_post.call_args.args[0]
        self.assertEqual(url, "http://test:8000/embeddings/batch")

    @patch(
        "doclatticeserver.pipeline.embedders.sent_transformer_microservice.requests.post"
    )
    def test_batch_settings_none_fallback(self, mock_post):
        """Falls back to Settings() when self.settings is None."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"embeddings": [[0.1] * 384]}
        mock_post.return_value = mock_response

        self.embedder._settings = None
        result = self.embedder.embed_texts_batch(["a"])
