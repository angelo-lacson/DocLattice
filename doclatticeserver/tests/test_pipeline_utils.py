...
class TestParser(BaseParser):
    ...
    def parse_document(self, user_id: int, doc_id: int) -> Optional[DocLatticeDocExport]:
        # Return None or a dummy DocLatticeDocExport for testing purposes
        return None
'''
...
'''
        ...
        importlib.reload(
            importlib.import_module("doclatticeserver.pipeline.post_processors")
        )

        # Force import the new modules directly
        try:
            from doclatticeserver.pipeline.embedders.test_embedder import (  # noqa
                TestEmbedder,
                TestEmbedder384,
                TestEmbedder768,
            )
            from doclatticeserver.pipeline.parsers.test_parser import (  # noqa
                TestParser,
            )
            from doclatticeserver.pipeline.post_processors.test_post_processor import (  # noqa
                TestPostProcessor,
            )
            from doclatticeserver.pipeline.thumbnailers.test_thumbnailer import (  # noqa
                TestThumbnailer,
            )
    ...
    def test_get_all_subclasses(self):
        ...
        """
        # Test parsers
        parsers = get_all_subclasses("doclatticeserver.pipeline.parsers", BaseParser)
        parser_titles = [parser.title for parser in parsers]
        self.assertIn("Test Parser", parser_titles)

        # Test embedders
        embedders = get_all_subclasses(
            "doclatticeserver.pipeline.embedders", BaseEmbedder
        )
        embedder_titles = [embedder.title for embedder in embedders]
        ...
        thumbnailers = get_all_subclasses(
            "doclatticeserver.pipeline.thumbnailers", BaseThumbnailGenerator
        )
    ...
    def test_get_metadata_for_component(self):
        ...
        """
        from doclatticeserver.pipeline.parsers.test_parser import (
    ...
    def test_get_component_by_name(self):
        ...
        from doclatticeserver.pipeline.embedders.test_embedder import (
        ...
        from doclatticeserver.pipeline.thumbnailers.test_thumbnailer import (  # type: ignore
            TestThumbnailer,
        )
        ...
        from doclatticeserver.pipeline.post_processors.test_post_processor import (  # type: ignore
            TestPostProcessor,
        )
    ...
    def test_run_post_processors(self):
        ...
        test_export_data: DocLatticeExportDataJsonPythonType = {
            "annotated_docs": {},
            "corpus": {
                "title": "Test Corpus",
        ...
        # Run post-processor
        processor_paths = [
            "doclatticeserver.pipeline.post_processors.test_post_processor.TestPostProcessor"
        ]
    ...
    def test_get_dimension_from_embedder(self):
        ...
        # Test with path
        with patch(
            "doclatticeserver.pipeline.utils.get_component_by_name"
            ...
            with patch("doclatticeserver.pipeline.utils.settings") as mock_settings:
                mock_settings.DEFAULT_EMBEDDING_DIMENSION = 768
                self.assertEqual(
                    get_dimension_from_embedder("non.existent.Embedder"), 768
                )

    @patch("doclatticeserver.pipeline.utils.settings")
    def test_get_default_embedder_for_filetype(self, mock_settings: MagicMock) -> None:
        """
        Ensure we assign a real dictionary to mock_settings.DEFAULT_EMBEDDERS_BY_FILETYPE,
        preventing unwanted MagicMock behavior when calling get(filetype, None).
        """
        # Assign a real dictionary rather than a MagicMock
        mock_settings.DEFAULT_EMBEDDERS_BY_FILETYPE = {
            "application/pdf": "doclatticeserver.tests.test_pipeline_utils.TestEmbedder768",
            "text/plain": "doclatticeserver.tests.test_pipeline_utils.TestEmbedder768",
        }

        # If you need DEFAULT_EMBEDDERS_BY_FILETYPE_AND_DIMENSION as well, set it here
    ...
    @patch("doclatticeserver.pipeline.utils.get_preferred_embedder")
    @patch("doclatticeserver.pipeline.utils.get_default_embedder_for_filetype")
    @patch("doclatticeserver.pipeline.utils.get_default_embedder")
    def test_find_embedder_for_filetype(
        self, mock_get_default, mock_get_default_for_filetype, mock_get_preferred
    ):
        """
        Test find_embedder_for_filetype function.
        """
        ...
        mock_get_default_for_filetype.return_value = test_embedder_768
