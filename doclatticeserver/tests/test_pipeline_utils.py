...
class TestParser(BaseParser):
    ...
    def _parse_document_impl(self, user_id: int, doc_id: int) -> Optional[DocLatticeDocExport]:
...
'''
        ...
        try:
            from doclatticeserver.pipeline.embedders.temp_embedder import (  # noqa
                TestEmbedder,
                TestEmbedder384,
                TestEmbedder768,
            )
        ...
        sys.modules.pop(
            "doclatticeserver.pipeline.post_processors.test_post_processor", None
        )
        importlib.invalidate_caches()
        importlib.import_module(
            "doclatticeserver.pipeline.post_processors.test_post_processor"
        )
    ...
    def test_get_component_by_name(self):
        ...
        from doclatticeserver.pipeline.embedders.temp_embedder import (
        ...
        from doclatticeserver.pipeline.thumbnailers.test_thumbnailer import (  # type: ignore[import-not-found]
            TestThumbnailer,
        )
        ...
        from doclatticeserver.pipeline.post_processors.test_post_processor import (  # type: ignore[import-not-found]
            TestPostProcessor,
        )
    ...
    def test_get_dimension_from_embedder(self):
        ...
        self.assertEqual(
            get_dimension_from_embedder(
                "doclatticeserver.pipeline.embedders.temp_embedder.TestEmbedder"
            ),
            128,
        )
        self.assertEqual(
            get_dimension_from_embedder(
                "doclatticeserver.pipeline.embedders.temp_embedder.TestEmbedder384"
            ),
            384,
        )
    ...
    def test_get_default_embedder_for_filetype(self) -> None:
        ...
        mock_embedders = {
            "application/pdf": "doclatticeserver.pipeline.embedders.temp_embedder.TestEmbedder384",
            "text/plain": "doclatticeserver.pipeline.embedders.temp_embedder.TestEmbedder768",
        }

        with patch(
            "doclatticeserver.pipeline.utils.get_preferred_embedder"
        ) as mock_get_pref:
            ...
            with patch(
                "doclatticeserver.pipeline.utils.get_default_embedder",
                return_value=None,
            ):
    ...
    def test_find_embedder_for_filetype_error_handling(self) -> None:
        ...
        """
        from doclatticeserver.documents.models import PipelineSettings
        from doclatticeserver.pipeline.utils import find_embedder_for_filetype
...
class TestEmbedderInstanceCache(TestCase):
    ...
    def setUp(self) -> None:
        from doclatticeserver.pipeline.utils import invalidate_embedder_cache

        invalidate_embedder_cache()
        self.addCleanup(invalidate_embedder_cache)

    def test_instance_is_cached_per_class_path(self) -> None:
        from doclatticeserver.pipeline.utils import get_embedder_instance
