...
from doclatticeserver.conversations.models import Conversation, ChatMessage # noqa
...
This is an autogenerated summary for the document. Please replace it with real content if needed.
    ...
    async def _assert_streaming_flow(
        self, conversation_to_load: Conversation | None = None
    ) -> None: # Modified to accept optional conversation
        """
        Connect, send a query, and verify ASYNC_START → ASYNC_CONTENT →
        ASYNC_FINISH message sequence with stable ``message_id``.
        Can optionally load a pre-existing conversation.
        """
        ...
        if conversation_to_load:
            graphql_convo_id = to_global_id("ConversationType", conversation_to_load.id)
            encoded_graphql_convo_id = quote(graphql_convo_id)
            ws_path += f"&load_from_conversation_id={encoded_graphql_convo_id}"
    ...
    async def _create_and_populate_conversation(self) -> Conversation:
        ...
        await ChatMessage.objects.acreate(
            ...
            msg_type="HUMAN",
            content="This is a previous user message.",
            creator=self.user,
        )
        await ChatMessage.objects.acreate(
            conversation=conversation,
            msg_type="LLM",
            content="This is a previous LLM response.",
    ...
    @vcr.use_cassette( # Using the same cassette for now
        "fixtures/vcr_cassettes/test_document_conversation_ws.yaml",
        filter_headers=["authorization"],
    )
    async def test_streaming_flow_with_loaded_conversation__all_default_frameworks(self) -> None:
        """
        Tests the streaming flow when loading an existing conversation.
        Runs for both llama_index and pydantic_ai frameworks.
        """
        for framework in ("llama_index", "pydantic_ai"):
            with self.subTest(default_framework=framework):
                with override_settings(
                    ...
                    # Create and populate a conversation before running the flow
                    conversation = await self._create_and_populate_conversation()
                    await self._assert_streaming_flow(conversation_to_load=conversation)
