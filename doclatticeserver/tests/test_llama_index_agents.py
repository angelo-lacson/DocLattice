...
from doclatticeserver.llms.agents.llama_index_agents import (
    LlamaIndexCorpusAgent,
    LlamaIndexDocumentAgent,
    _convert_llama_index_node_to_source_node,
)
from doclatticeserver.llms.tools.llama_index_tools import (
...
class TestLlamaIndexAgents(TestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        embedder_path = "doclatticeserver.pipeline.embedders.sent_transformer_microservice.MicroserviceEmbedder"
    ...
    def test_source_node_conversion_string_annotation_id(self) -> None:
        ...
        text_node = TextNode(
            text="Test content", metadata={"annotation_id": "789"}, id_="test_node_id"
        )
    ...
    def test_source_node_conversion_fallback(self) -> None:
        ...
        text_node = TextNode(text="Test content", metadata={}, id_="test_node_id")

        source_node = _convert_llama_index_node_to_source_node(text_node)

        self.assertEqual(source_node.annotation_id, 0)  # Should fallback to 0

    @patch("doclatticeserver.llms.agents.llama_index_agents.OpenAIAgent")
    @patch("doclatticeserver.llms.agents.llama_index_agents.VectorStoreIndex")
    @patch(
        "doclatticeserver.llms.agents.llama_index_agents.DjangoAnnotationVectorStore"
    )
    @patch("doclatticeserver.llms.agents.llama_index_agents.Settings")
    async def test_document_agent_creation(
        ...
        mock_index.from_vector_store.assert_called_once_with(
            vector_store=mock_vector_store_instance, use_async=False
        )
    ...
    async def test_document_agent_chat(
        ...
        mock_response = MockAgentResponse(
            ...
            source_nodes=[
                NodeWithScore(
                    node=TextNode(
                        text="Source content", metadata={"annotation_id": self.anno1.id}
                    ),
                    score=0.9,
                )
            ],
        ...
        self.assertEqual(
            response.content, "This document is about testing LlamaIndex agents."
        )
    ...
    @patch("doclatticeserver.llms.agents.llama_index_agents.Settings")
    async def test_document_agent_streaming(
        self, mock_settings, mock_vector_store, mock_index, mock_openai_agent
    ):
        ...
        mock_source_nodes = [
            NodeWithScore(
                node=TextNode(
                    text="Streaming source", metadata={"annotation_id": self.anno2.id}
                ),
                score=0.8,
            )
        ]
        ...
        mock_agent_instance.astream_chat = AsyncMock(
            return_value=mock_streaming_chat_response
        )
        mock_openai_agent.from_tools.return_value = mock_agent_instance

        # Create and test agent
        config = AgentConfig(
            user_id=self.user.id, model_name="gpt-4o-mini", streaming=True
        )
        agent = await LlamaIndexDocumentAgent.create(self.doc1, self.corpus, config)
    ...
    @patch("doclatticeserver.llms.agents.llama_index_agents.Settings")
    async def test_corpus_agent_creation(
        self,
        mock_settings,
        mock_vector_store,
        mock_index,
    ...
    @patch("doclatticeserver.llms.agents.llama_index_agents.Settings")
    async def test_error_handling_in_chat(
        self, mock_settings, mock_vector_store, mock_index, mock_openai_agent
    ):
    ...
    @patch("doclatticeserver.llms.agents.llama_index_agents.Settings")
    async def test_message_storage_control(
        self, mock_settings, mock_vector_store, mock_index, mock_openai_agent
    ):
    ...
    async def test_custom_tools_integration(
        ...
        config = AgentConfig(user_id=self.user.id, model_name="gpt-4o-mini")
        agent = await LlamaIndexDocumentAgent.create(
            self.doc1, self.corpus, config=config, tools=[custom_tool]
        )
        ...
        tool_names = [
            getattr(tool, "metadata", getattr(tool, "_metadata", None))
            for tool in tools_passed
        ]
        tool_names = [
            getattr(meta, "name", None) if meta else None for meta in tool_names
        ]
    ...
    async def test_conversation_continuity(
        ...
        conversation = await Conversation.objects.acreate(
            title="Test Conversation", creator=self.user
        )
        ...
        await LlamaIndexDocumentAgent.create(self.doc1, self.corpus, config)
    ...
    def test_legacy_compatibility_interface(self) -> None:
        """Test backward compatibility with legacy DocLatticeDbAgent interface."""
        from doclatticeserver.llms.agents.core_agents import (
            CoreAgent,
        ...
        mock_core_agent.conversation_manager.conversation = MagicMock(
            spec=Conversation
        )  # If Conversation is a Django model
        mock_core_agent.conversation_manager.user_id = self.user.id

        # Create legacy wrapper
        legacy_agent = DocLatticeDbAgent(mock_core_agent)
    ...
    def test_node_conversion_edge_cases(self) -> None:
        ...
        text_node_invalid = TextNode(
            text="Content with invalid ID", metadata={"annotation_id": "not_a_number"}
        )
        source_node_invalid = _convert_llama_index_node_to_source_node(
            text_node_invalid
        )
