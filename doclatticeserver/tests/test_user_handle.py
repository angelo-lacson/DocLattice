...
from doclatticeserver.users.handle_generator import (
    PLAIN_ATTEMPTS,
    SUFFIXED_ATTEMPTS,
    _camel_case_pair,
    generate_handle,
)
from doclatticeserver.users.handle_wordlists import ADJECTIVES, NOUNS
...
class HandleGeneratorTests(TestCase):
    ...
    def test_generate_handle_skips_existing_values(self):
        ...
        with mock.patch(
            "doclatticeserver.users.handle_generator.ADJECTIVES",
            ("brave", "clever"),
        ), mock.patch(
            "doclatticeserver.users.handle_generator.NOUNS",
            ("bear", "fox"),
        ):
    ...
    def test_generate_handle_falls_back_to_suffix_when_namespace_saturated(self):
        ...
        with mock.patch(
            "doclatticeserver.users.handle_generator.ADJECTIVES", single_adj
        ), mock.patch("doclatticeserver.users.handle_generator.NOUNS", single_noun):
...
class HandleGeneratorTunablesTests(TestCase):
    ...
    def test_suffix_format_is_numeric(self):
        ...
        with mock.patch(
            "doclatticeserver.users.handle_generator.ADJECTIVES", ("only",)
        ), mock.patch("doclatticeserver.users.handle_generator.NOUNS", ("one",)):
...
class RegenerateUserSlugsCommandTests(TestCase):
    ...
    def _make_oauth_user_with_bad_slug(self, username, email):
        ...
        from doclatticeserver.shared.slug_utils import sanitize_slug
