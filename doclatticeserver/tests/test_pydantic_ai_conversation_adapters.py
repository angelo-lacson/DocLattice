...
from doclatticeserver.llms.vector_stores.pydantic_ai_conversation_vector_stores import (
...
class TestPydanticAIConversationAdaptersAsync(TransactionTestCase):
    ...
    def setUp(self):
        """Set up test data."""
        # The previous per-class ``PipelineSettings.update_or_create`` workaround
        # (PR #1773) was superseded by the autouse
        # ``_invalidate_pipeline_settings_singleton_cache`` fixture in
        # ``doclatticeserver/conftest.py``, which structurally fixes the
        # cross-test cache pollution that this class used to symptom-treat.
