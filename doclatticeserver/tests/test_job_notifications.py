...
from doclatticeserver.notifications.models import Notification, NotificationTypeChoices
...
class DocumentProcessedNotificationTests(TestCase):
    ...
    @patch("doclatticeserver.tasks.doc_tasks.broadcast_notification_via_websocket")
    ...
    @patch(
        "doclatticeserver.tasks.doc_tasks.broadcast_notification_via_websocket",
...
class AnalysisNotificationTests(TestCase):
    ...
    @patch("doclatticeserver.analyzer.views.broadcast_notification_via_websocket")
    def test_analysis_complete_notification(self, mock_broadcast):
        """ANALYSIS_COMPLETE notification should be created on success."""
        from doclatticeserver.analyzer.views import _create_analysis_notification
...
class ExportNotificationTests(TestCase):
    ...
    @patch("doclatticeserver.tasks.export_tasks.broadcast_notification_via_websocket")
    ...
    @patch(
        "doclatticeserver.tasks.export_tasks.broadcast_notification_via_websocket",
        side_effect=Exception("Broadcast error"),
    )
    def test_exception_handled(self, mock_broadcast):
        """Exceptions should be handled gracefully."""
...
class ExtractNotificationTests(TestCase):
    ...
    @patch(
        "doclatticeserver.tasks.extract_orchestrator_tasks.broadcast_notification_via_websocket"
    )
    def test_extract_complete_notification(self, mock_broadcast):
        """EXTRACT_COMPLETE notification should be created."""
        from doclatticeserver.tasks.extract_orchestrator_tasks import (
    ...
    @patch("doclatticeserver.tasks.extract_orchestrator_tasks.Extract")
