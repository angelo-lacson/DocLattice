...
from doclatticeserver.pipeline.parsers.warp_ingest_parser import (
...
_POST_PATH = "doclatticeserver.pipeline.parsers.warp_ingest_parser.requests.post"
_STORAGE_PATH = (
    "doclatticeserver.pipeline.parsers.warp_ingest_parser.default_storage.open"
)
_STORAGE_SIZE_PATH = (
    "doclatticeserver.pipeline.parsers.warp_ingest_parser.default_storage.size"
)
...
def _sample_export():
    """A realistic DocLatticeDocExport as returned under response['result']."""
    return {
        "title": "Test Document",
...
class TestWarpIngestParser(TestCase):
    ...
    def test_request_is_built_correctly(self, mock_open, mock_post):
        ...
        # Query params: doclattice render format + lowercase bool strings.
    ...
    @patch(
        "doclatticeserver.pipeline.parsers.warp_ingest_parser.maybe_add_cloud_run_auth"
    )
    @patch(_POST_PATH)
    @patch(_STORAGE_PATH)
...
class TestWarpIngestSaveParsedDataIntegration(TestCase):
    ...
    the DocLattice importer) that breaks this compatibility is caught here
    ...
    def test_warp_export_imports_without_normalization(self):
        WarpIngestParser().save_parsed_data(
            user_id=self.user.id,
            doc_id=self.doc.id,
            doclattice_data=cast(
                DocLatticeDocExport, _sample_export_with_relationships()
            ),
        )
