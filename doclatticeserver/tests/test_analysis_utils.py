...
from doclatticeserver.analyzer.models import Analyzer, Analysis, GremlinEngine
...
class TestAnalysisUtils(TestCase):
    ...
    def test_create_and_setup_analysis_gremlin(self):
        analysis = create_and_setup_analysis(self.analyzer_gremlin, self.corpus.id, self.user.id)
    ...
    def test_create_and_setup_analysis_task(self):
        analysis = create_and_setup_analysis(self.analyzer_task, self.corpus.id, self.user.id)
        self.assertIsInstance(analysis, Analysis)
    ...
    def test_create_and_setup_analysis_with_doc_ids(self):
        ...
        analysis = create_and_setup_analysis(self.analyzer_gremlin, self.corpus.id, self.user.id, doc_ids=doc_ids)
        self.assertIsInstance(analysis, Analysis)
        self.assertEqual(list(analysis.analyzed_documents.values_list('id', flat=True)), doc_ids)

    def test_create_and_setup_analysis_invalid_analyzer(self):
        invalid_analyzer = Analyzer(description="Invalid Analyzer", creator=self.user, manifest={})
