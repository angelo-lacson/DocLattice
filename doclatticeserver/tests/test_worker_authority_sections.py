...
class TestSectionBatchEndpoint(SectionBatchTestBase):
    @patch(
        "doclatticeserver.worker_uploads.views.process_pending_section_batches.apply_async"
    )
...
class TestSectionBatchDrain(SectionBatchTestBase):
    ...
    def _drain(self):
        from doclatticeserver.worker_uploads.tasks import (
    ...
    @patch(
        "doclatticeserver.worker_uploads.tasks."
    ...
    def test_drain_keeps_bootstrap_report_when_equivalence_upsert_fails(self):
        ...
        batch = self._stage()
        with patch(
            "doclatticeserver.enrichment.services."
