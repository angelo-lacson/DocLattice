...
class TestCorpusEmbedderAutoPopulation(TestCase):
    ...
    @patch(
        "doclatticeserver.corpuses.models.get_default_embedder_path",
        return_value="doclatticeserver.pipeline.embedders.test_embedder.TestEmbedder",
    )
...
class TestReEmbedCorpusMutation(TestCase):
    ...
    @patch("doclatticeserver.tasks.corpus_tasks.reembed_corpus.delay")
...
class TestReEmbedCorpusTask(TestCase):
    ...
    @patch(
        "doclatticeserver.tasks.embeddings_task.calculate_embeddings_for_annotation_batch.delay"
    )
    ...
    def test_reembed_caps_task_queue(self, mock_batch_delay):
        ...
        with patch(
            "doclatticeserver.constants.document_processing.MAX_REEMBED_TASKS_PER_RUN",
            2,
        ), patch(
            "doclatticeserver.constants.document_processing.EMBEDDING_BATCH_SIZE", 1
        ):
            result = reembed_corpus(self.corpus.pk, "new.Embedder")
...
class TestEmbedderConsistencyCheck(TestCase):
    ...
    def test_no_warning_when_embedders_match(self):
        ...
        from doclatticeserver.corpuses.checks import (
        ...
        w002 = [w for w in warnings if w.id == "doclattice.W002"]
...
class TestForkWithEmbedderOverride(TestCase):
    ...
    @patch("doclatticeserver.tasks.fork_tasks.fork_corpus.si")
