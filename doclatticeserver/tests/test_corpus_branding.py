...
Covers the four moving parts of the feature:
  ...
  * ``doclatticeserver/utils/image_generation.py`` — AI-preferred logo
    generation with a deterministic PIL monogram fallback.
  * ``CorpusService.update_icon`` — creator-gated icon write.
  * ``doclatticeserver/corpuses/services/branding.py`` — the async
...
from doclatticeserver.utils.image_generation import (
    _initials_from_text,
    _pick_color,
...
class AgenerateLogoImageTests(TestCase):
    ...
    def test_disabled_uses_fallback_without_calling_ai(self):
        with patch(
            "doclatticeserver.utils.image_generation._generate_ai_logo"
        ) as ai_mock:
...
class LogoCredentialResolutionTests(TestCase):
    ...
    def test_db_credentials_override_env_and_endpoint(self):
        from doclatticeserver.utils import image_generation
        ...
        with patch(
            "doclatticeserver.llms.model_factory.aget_provider_credentials",
            new=AsyncMock(
                return_value={
                    "api_key": "db-key",
...
class BuildLogoPromptSanitizationTests(TestCase):
    ...
    def test_quotes_and_newlines_are_stripped_from_title(self):
        from doclatticeserver.corpuses.services.branding import _build_logo_prompt
...
class RunCorpusBrandingAsyncTests(TransactionTestCase):
    ...
    def test_generates_readme_and_logo(self):
        ...
        # ``AgentAPI`` singleton bound by ``from doclatticeserver.llms import
        # agents`` — i.e. ``doclatticeserver.llms.api.agents`` (see
        # llms/__init__.py), NOT the ``doclatticeserver.llms.agents`` package
    ...
    def test_readme_skipped_when_article_exists(self):
        ...
        """
        from doclatticeserver.corpuses.services.branding import _generate_readme
    ...
    def test_readme_agent_failure_still_writes_structural_default(self):
        ...
        """
        from doclatticeserver.corpuses.caml_intelligence import (
