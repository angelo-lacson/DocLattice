...
from doclatticeserver.enrichment.authority_import_artifacts import (
    ArtifactCase,
...
def _provider_definition() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        class_name=(
            "doclatticeserver.tests.test_authority_import_artifacts."
...
class AuthorityImportArtifactTests(TestCase):
    ...
    def _collect(self, *, rights_approved: bool = False):
        with patch(
            "doclatticeserver.enrichment.authority_import_artifacts."
            "_provider_definitions",
            return_value=(
    ...
    def test_export_validation_failure_is_surfaced(self):
        ...
        with patch(
            "doclatticeserver.utils.validate_export.validate_export",
    ...
    def test_configured_supported_mime_types_fails_closed_on_registry_error(self):
        with patch(
            "doclatticeserver.pipeline.registry.get_allowed_mime_types",
    ...
    def test_registered_converter_extensions_fails_closed_on_registry_error(self):
        with patch(
            "doclatticeserver.pipeline.registry.get_all_file_converters_cached",
            side_effect=RuntimeError("registry unavailable"),
        ):
