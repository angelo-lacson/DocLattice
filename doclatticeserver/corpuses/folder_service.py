...
- :class:`doclatticeserver.documents.document_service.DocumentService`
  for document-level operations (creation, quota, validation, standalone
  lookup, document-level permissions).
- :class:`doclatticeserver.corpuses.corpus_objs_service.CorpusObjsService`
...
class DocumentFolderService(CorpusObjsService, DocumentService):
    ...
    ``doclatticeserver/tests/test_document_service.py`` asserts the
    invariant.
    """
