...
class Document(TreeNode, BaseOCModel, HasEmbeddingMixin):
    ...
    # runs (see doclatticeserver/pipeline/base/file_converter.py), the
    ...
    def update_summary(
        ...
        *,
        ...
        from doclatticeserver.documents.models import (  # avoid circular
...
class PipelineSettings(django.db.models.Model):
    ...
    Settings Structure:
        preferred_parsers: Dict mapping MIME types to parser class paths
            Example: {"application/pdf": "doclatticeserver.pipeline.parsers.docling_parser_rest.DoclingParser"}

        preferred_embedders: Dict mapping MIME types to embedder class paths
            Example: {"application/pdf":
                "doclatticeserver.pipeline.embedders...MicroserviceEmbedder"}

        preferred_thumbnailers: Dict mapping MIME types to thumbnailer class paths
            Example: {"application/pdf":
                "doclatticeserver.pipeline.thumbnailers...PdfThumbnailGenerator"}

        preferred_enrichers: Dict mapping MIME types to ORDERED LISTS of
            enricher class paths (the ingest-time enrichment chain)
            Example: {"application/pdf":
                ["doclatticeserver.pipeline.enrichers...PdfOutlineEnricher"]}

        parser_kwargs: Dict mapping parser class paths to their configuration kwargs
            Example: {"doclatticeserver.pipeline.parsers.docling_parser_rest.DoclingParser": {"force_ocr": false}}

        component_settings: Dict mapping component class paths to their settings overrides
            Example: {"doclatticeserver.pipeline.embedders.MicroserviceEmbedder": {"timeout": 30}}
    ...
    # persistence, each transforming the parsed DocLatticeDocExport. An
    ...
    # doclatticeserver.llms.llm_registry.resolve_model_spec.
    ...
    def get_tools_with_secrets(self) -> list[str]:
        ...
        """
        from doclatticeserver.constants.tools import TOOL_SETTINGS_PREFIX
