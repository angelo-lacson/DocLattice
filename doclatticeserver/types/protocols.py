"""
Shared ``Protocol`` definitions for DocLattice pluggable interfaces.
...
class VectorStoreProtocol(Protocol):
    """Minimum interface implemented by vector stores in DocLattice.
    ...
    include :class:`doclatticeserver.llms.vector_stores.core_vector_stores.CoreAnnotationVectorStore`.
...
class PipelineComponentProtocol(Protocol):
    ...
    The pipeline registry (``doclatticeserver.pipeline.utils``)
    ...
    inherits from
    :class:`doclatticeserver.pipeline.base.base_component.PipelineComponentBase`
...
class ToolProtocol(Protocol):
    ...
    :class:`doclatticeserver.llms.tools.tool_factory.CoreTool` without
...
class PermissionedQueryManagerProtocol(Protocol):
    ...
    DocLattice standardises permission filtering via
    :meth:`BaseVisibilityManager.visible_to_user`
    (``doclatticeserver/shared/Managers.py``). Any code receiving a
...
class StreamObserverProtocol(Protocol):
    ...
    .. important::
       ...
       :class:`doclatticeserver.llms.types.StreamObserver`. The two
       definitions are duplicated (rather than re-exported) on purpose:
       ``protocols.py`` is imported by
       ``doclatticeserver.shared.Managers`` during Django app loading,
       and ``doclatticeserver.llms.types`` lives in a package whose
    ...
    Mirrors :class:`doclatticeserver.llms.types.StreamObserver` so it
