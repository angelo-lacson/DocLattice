...
class CompactPageAnnotationType(TypedDict):
    ...
    :class:`DocLatticeSinglePageAnnotationType` in the v2 format.
...
class DocLatticeAnnotationPythonType(TypedDict):
    ...
    annotation_json: Union[
        dict[Union[int, str], DocLatticeSinglePageAnnotationType],
...
class DocLatticeCorpusV2Type(DocLatticeCorpusType):
    """
    Extended corpus type for V2 exports that includes additional configuration.
    Backward compatible with V1 by inheriting from DocLatticeCorpusType.
    """
...
class StructuralAnnotationSetExport(TypedDict):
    ...
    structural_relationships: list[DocLatticeRelationshipPythonType]
...
class DocLatticeExportDataJsonV2Type(TypedDict):
    ...
    doc_labels: dict[str, AnnotationLabelPythonType]
    text_labels: dict[str, AnnotationLabelPythonType]
    corpus: DocLatticeCorpusV2Type  # Enhanced with V2 fields
    label_set: DocLatticeLabelSetType
...
class DocLatticeExportDataJsonPythonTypeV3(TypedDict):
    """
    ...
    The V2 TypedDict (``DocLatticeExportDataJsonV2Type``) is preserved
...
class WorkerDocumentUploadMetadataType(TypedDict):
    ...
    labelled_text: NotRequired[list[DocLatticeAnnotationPythonType]]
