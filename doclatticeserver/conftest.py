...
def _invalidate_pipeline_settings_singleton_cache():
    ...
    ``doclatticeserver/documents/models.py``). The cache is per-worker
    ...
    (``doclatticeserver/pipeline/utils.py``) and the LLM-provider
    *credential* cache (``doclatticeserver/llms/model_factory.py``). All are
    ...
    from doclatticeserver.llms.model_factory import invalidate_credential_cache
