...
class Command(BaseCommand):
    ...
    def save_file_to_fixtures(self, file_obj, filename: str) -> str:
        ...
        """
        fixtures_dir = Path("doclatticeserver/tests/fixtures/files")
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        try:
            ...
            for i, annot in enumerate(Annotation.objects.all(), 1):
                ...
                if i % 100 == 0:
                    self.stdout.write(
                        f"  Progress: {i}/{annot_count} ({i/annot_count*100:.1f}%)"
                    )
            ...
            self.stdout.write(
                f"Generated {embed_count} embeddings for {annot_count} annotations"
            )
            ...
            fixture_path = Path("doclatticeserver/tests/fixtures/test_data.json")
            call_command(
                "dumpdata",
                *apps_to_dump,
                "--natural-foreign",
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "Successfully generated test fixtures in doclatticeserver/tests/fixtures/"
                )
            )
