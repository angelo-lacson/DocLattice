...
class Embedding(BaseOCModel):
    ...
    # ``doclatticeserver/utils/subtree_groups.py`` so semantic search can
        ...
        for source_annot, new_annot in source_to_new.items():
            ...
            if source_annot.image_content_file:
                ...
                if pawls_data is None and self.pawls_parse_file:
                    ...
                    try:
                        from doclatticeserver.utils.compact_pawls import (
                            expand_pawls_pages,
                        )
                ...
                if pawls_data:
                    from doclatticeserver.utils.multimodal_embeddings import (
...
class Annotation(BaseOCModel, HasEmbeddingMixin):
    ...
    # (``doclatticeserver/utils/extraction_grounding.py``). Backs the
    ...
    class Meta:
        ...
        constraints = [
            ...
            # ``doclatticeserver/utils/extraction_grounding.py``). Without
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        with transaction.atomic():
            ...
            if not skip_revision and (
                ...
                from doclatticeserver.annotations.models import (  # local import to avoid cycles
                    NoteRevision,
                )
    ...
    def version_up(
        ...
        *,
        ...
        with transaction.atomic():
            ...
            from doclatticeserver.annotations.models import (  # avoid circular import
                NoteRevision,
            )
