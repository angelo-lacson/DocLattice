...
The PAWLs token data is already present in the parser's ``DocLatticeDocExport``
...
class PdfOutlineEnricher(BaseEnricher):
    ...
    def _enrich_document_impl(
        ...
        **all_kwargs,
        ...
        new_entries: list[DocLatticeAnnotationPythonType] = []
        dropped = 0
        for node in nodes:
            ...
            single_page: DocLatticeSinglePageAnnotationType = {
                "bounds": bounds,
                "tokensJsons": tokens_jsons,
                "rawText": page_raw_text,
            }
            annotation_json: dict[int | str, DocLatticeSinglePageAnnotationType] = {
                str(p): single_page
            }
            new_entries.append(
                {
