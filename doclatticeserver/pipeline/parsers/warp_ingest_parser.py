...
scanned pages) that renders directly to the DocLattice structural export
...
This mirrors the :class:`~doclatticeserver.pipeline.parsers.docling_parser_rest.DoclingParser`
...
``docs/pipelines/warp_ingest_parser.md``) and DocLattice talks to it over
...
# render_format the client requests: Warp-Ingest emits an DocLatticeDocExport
# directly under the ``result`` key for this value.
WARP_INGEST_RENDER_FORMAT = "opencontracts"


class WarpIngestParser(BaseParser):
    """Parse PDFs via a Warp-Ingest microservice and return an DocLatticeDocExport.
    ...
    description = (
        ...
        "rule-based parser that renders directly to the DocLattice format."
    )
    ...
    def _parse_document_impl(
        ...
        Returns:
            DocLatticeDocExport with structural annotations, relationships and
    ...
    @staticmethod
    def _extract_export(response_data: Any, doc_id: int) -> DocLatticeDocExport:
        """Pull the DocLatticeDocExport out of a Warp-Ingest response.

        Warp-Ingest wraps the export as ``{"page_dim": ..., "num_pages": ...,
        "result": <DocLatticeDocExport>}`` for ``render_format=opencontracts``.
        ...
        The export already uses the DocLattice field names (snake_case
        ...
        if not isinstance(export, dict) or not any(
            ...
            raise DocumentParsingError(
                ...
                f"DocLattice export payload. Response: {snippet}",
                is_transient=False,
            )

        return cast(DocLatticeDocExport, export)
