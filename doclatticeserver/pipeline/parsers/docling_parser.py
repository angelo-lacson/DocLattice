...
import os
import json
from doclatticeserver.utils.layout import reassign_annotation_hierarchy
import pdf2image
...
# ) -> None:
#     """
#     Traverse a list of DocLatticeAnnotationPythonType items (labelled_text), compute
...
#     Args:
#         labelled_text (list[DocLatticeAnnotationPythonType]): The annotated text elements
#             to traverse.
...
def convert_docling_item_to_annotation(item: Union[TextItem, SectionHeaderItem, ListItem], spatial_indices_by_page: Dict[int, STRtree], tokens_by_page: Dict[int, List[PawlsTokenPythonType]], token_indices_by_page: Dict[int, np.ndarray]) -> Optional[DocLatticeAnnotationPythonType]:
    ...
    annotation_json: Dict[int, DocLatticeSinglePageAnnotationType] = {
        page_no: {
            "bounds": {
                "left": bbox.l,
                "top": bbox.t,
...
class DoclingParser(BaseParser):
    """
    Parser that uses Docling to convert PDF documents into DocLattice format.
    """
    ...
    def parse_document(
        ...
        """
        Parses the document and converts it into the DocLatticeDocExport format.
        ...
        Returns:
            Optional[DocLatticeDocExport]: The parsed document data or None if parsing fails.
        """
        ...
        try:
            ...
            # Create DocLattice document structure
            ...
            if kwargs.get('llm_enhanced_hierarchy', False):
                ...
                enriched_data = reassign_annotation_hierarchy(doclattice_data['labelled_text'])
                doclattice_data['labelled_text'] = enriched_data
                
            # print_labelled_text_hierarchy(doclattice_data['labelled_text'], "labelled_text_hierarchy.txt")
   ...
   # These are LLM-powered post-processors that should be moved elsewhere. They work with any DocLatticeDocExport (in theory)
