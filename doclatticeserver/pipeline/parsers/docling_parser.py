...
class DoclingParser(BaseParser):
    ...
    def parse_document(
        ...
        try:
            ...
            # logger.info(f"DocLattice data: {doclattice_data}")
            # with open("doclattice_data.json", "w") as f:
            #     json.dump(doclattice_data, f, indent=4)
            #########################################################

            # # Return parsed data
            enriched_data = self._assign_hierarchy(doclattice_data['labelled_text'])
    ...
    def convert_chunks_to_annotations(
        ...
        tokens_by_page: Dict[int, List[PawlsTokenPythonType]],
        token_indices_by_page: Dict[int, np.ndarray]
    ) -> List[DocLatticeAnnotationPythonType]:
        """
        Converts the chunks from a DoclingDocument into DocLattice annotations.
        ...
        # ---------------------------------------------------------------------------
        # STEP 6: Build DocLattice annotations from the chunk results.
        #
        ...
        #
        # ---------------------------------------------------------------------------
        annotations: List[DocLatticeAnnotationPythonType] = []
    ...
    def _assign_hierarchy(
        ...
        Args:
            annotations (list[DocLatticeAnnotationPythonType]): The list of annotations.

        Returns:
            list[DocLatticeAnnotationPythonType]: The updated list of annotations,
        ...
        # -------------------------------------------------------------------------
        updated_annotations_map: dict[int, DocLatticeAnnotationPythonType] = {}
        ...
        updated_annotations: list[DocLatticeAnnotationPythonType] = [
