...
``DocLatticeDocExport`` JSON to ``default_storage`` under a per-document
...
from doclatticeserver.constants.document_processing import CHUNK_SCRATCH_PREFIX
...
def chunk_output_key(doc_id: int, chunk_index: int) -> str:
    """Storage key for a chunk's output DocLatticeDocExport JSON."""
...
def write_chunk_result(doc_id: int, chunk_index: int, result: Mapping[str, Any]) -> str:
    """Write a chunk's DocLatticeDocExport result JSON to storage; return key.

    Accepts any JSON-serializable mapping (``DocLatticeDocExport`` is one);
...
def read_chunk_result(key: str) -> "DocLatticeDocExport":
    """Read a chunk's DocLatticeDocExport result JSON from storage."""
    with default_storage.open(key, "rb") as fh:
