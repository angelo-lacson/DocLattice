...
def get_all_thumbnailers() -> List[Type[BaseThumbnailGenerator]]:
    ...
    """
    return get_all_subclasses('doclatticeserver.pipeline.thumbnailers', BaseThumbnailGenerator)

def get_components_by_mimetype(mimetype: str, detailed: bool = False) -> Dict[str, List[Any]]:
    """
...
def get_component_by_name(component_name: str) -> Type:
    ...
    """
    try:
        module = importlib.import_module(f"doclatticeserver.pipeline.parsers.{component_name}")
        for name, obj in inspect.getmembers(module, inspect.isclass):
    ...
    try:
        module = importlib.import_module(f"doclatticeserver.pipeline.embedders.{component_name}")
        for name, obj in inspect.getmembers(module, inspect.isclass):
    ...
    try:
        module = importlib.import_module(f"doclatticeserver.pipeline.thumbnailers.{component_name}")
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if issubclass(obj, BaseThumbnailGenerator) and obj != BaseThumbnailGenerator:
                return obj
    except ModuleNotFoundError:
        pass
