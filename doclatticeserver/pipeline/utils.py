...
def get_component_by_name(component_name: str) -> type:
    ...
    try:
        module = importlib.import_module(
            f"doclatticeserver.pipeline.parsers.{component_name}"
        )
        for name, obj in inspect.getmembers(module, inspect.isclass):
    ...
    try:
        module = importlib.import_module(
            f"doclatticeserver.pipeline.embedders.{component_name}"
        )
        for name, obj in inspect.getmembers(module, inspect.isclass):
    ...
    try:
        module = importlib.import_module(
            f"doclatticeserver.pipeline.thumbnailers.{component_name}"
        )
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if (
