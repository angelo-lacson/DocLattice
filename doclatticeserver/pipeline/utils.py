...
from doclatticeserver.pipeline.base.base_component import PipelineComponentBase
...
from doclatticeserver.pipeline.base.file_converter import (
    BaseFileConverter,
...
def get_all_parsers() -> list[type[BaseParser]]:
    ...
    """
    return get_all_subclasses("doclatticeserver.pipeline.parsers", BaseParser)
...
def get_all_embedders() -> list[type[BaseEmbedder]]:
    ...
    """
    return get_all_subclasses("doclatticeserver.pipeline.embedders", BaseEmbedder)
...
def get_default_llm_spec() -> str:
    ...
    :func:`doclatticeserver.llms.llm_registry.resolve_model_spec` as
...
def run_enrichers(
    ...
    (``DocLatticeDocExport -> DocLatticeDocExport``), so enrichers compose.
    ...
    Returns:
        DocLatticeDocExport: The final, possibly-enriched document data.
    """
    current_export_data = export_data
...
def get_default_reranker_instance(
    ...
    *not* bump ``modified``, so workers can stay pinned on the stale cached
    ...
    from doclatticeserver.pipeline.base.reranker import RerankerUnavailableError
