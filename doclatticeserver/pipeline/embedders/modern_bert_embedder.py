...
class ModernBERTEmbedder(BaseEmbedder):
    ...
    description = "Generates embeddings using the ModernBERT sentence transformer model."
    author = "DocLattice"
    ...
    def __init__(self):
        ...
        self.model_path = os.path.join(self.cache_dir, "sentence-transformers", "ModernBERT-base")
        ...
        if self.model is None:
            try:
                ...
                if os.path.exists(self.model_path):
                    logger.info(f"Loading ModernBERT model from cache: {self.model_path}")
                    self.model = SentenceTransformer(self.model_path)
                else:
                    # If not cached, download from Hugging Face
                    logger.info(f"Loading ModernBERT model from Hugging Face: {self.model_name}")
