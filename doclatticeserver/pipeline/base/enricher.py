...
``save_parsed_data()``. It accepts the parser's ``DocLatticeDocExport`` and
returns an ``DocLatticeDocExport`` (same type in, same type out), so several
...
``doclatticeserver.pipeline.utils``) wraps every enricher call and skips one
...
class BaseEnricher(PipelineComponentBase, ABC):
    ...
    def _enrich_document_impl(
        ...
        **all_kwargs,
        ...
        Returns:
            DocLatticeDocExport: The enriched document data. Implementations
            may mutate and return ``export_data`` or return a new dict, but
            MUST return an ``DocLatticeDocExport``.
        """
        ...
    ...
    def enrich_document(
        ...
        **direct_kwargs,
        ...
        Returns:
            DocLatticeDocExport: The enriched document data.
        """
