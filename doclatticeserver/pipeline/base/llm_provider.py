...
object by :mod:`doclatticeserver.llms.model_factory`.
...
``doclatticeserver.pipeline.llm_providers`` package on first access
...
class BaseLLMProvider(PipelineComponentBase, ABC):
    ...
    * ``provider_key`` — pydantic-ai's provider prefix (e.g. ``"openai"``,
      ...
      :mod:`doclatticeserver.llms.model_factory`.
