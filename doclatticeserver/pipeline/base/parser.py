...
class BaseParser(PipelineComponentBase, ABC):
    ...
    def save_parsed_data(
        ...
        Args:
            ...
            doclattice_data (DocLatticeDocExport): The parsed document data, including:
                - labelled_text (List[DocLatticeAnnotationPythonType]):
                  Annotation list to be imported.
                - relationships (Optional[List[DocLatticeRelationshipPythonType]]):
        ...
        txt_content = doclattice_data.get("content", "")
        ...
        pawls_file_content = doclattice_data.get("pawls_file_content")
        if pawls_file_content:
            ...
            document.page_count = doclattice_data.get("page_count", 1)
        ...
        text_labels_data_dict = {
            ...
            for label_data in doclattice_data.get("labelled_text", [])
        }
        ...
        annotation_id_map = import_annotations(
            ...
            annotations_data=doclattice_data.get("labelled_text", []),
        ...
        relationship_data = doclattice_data.get("relationships", [])
    ...
    def _create_structural_annotation_set(self, document: Document, user) -> None:
        ...
        in :func:`doclatticeserver.utils.structural_sets.create_structural_annotation_set`
    ...
    def _run_enrichment_stage(
        self, user_id: int, doc_id: int, parsed_data: DocLatticeDocExport
    ) -> DocLatticeDocExport:
        """
        ...
        Args:
            ...
            parsed_data: The DocLatticeDocExport produced by parse_document().

        Returns:
            DocLatticeDocExport: The parsed data, enriched if any enrichers
            are configured for the document's file type.
        """
        try:
            from doclatticeserver.documents.models import PipelineSettings
            from doclatticeserver.pipeline.utils import run_enrichers
    ...
    def process_document(
        ...
        Returns:
            DocLatticeDocExport: The parsed document data.
        ...
        # Enrichment stage: chainable DocLatticeDocExport transforms run
