...
into a single ``DocLatticeDocExport``.
...
from doclatticeserver.annotations.compact_json import offset_annotation_json
    ...
    **Limitation -- cross-chunk parent-child relationships:**
    ...
    def _parse_single_chunk_impl(
        ...
        **all_kwargs,
        ...
        Returns:
            ``DocLatticeDocExport`` with page indices *local to the chunk*
    ...
    def _post_reassemble_hook(
        ...
        user_id: int,
        doc_id: int,
        reassembled: DocLatticeDocExport,
        pdf_bytes: bytes,
        **all_kwargs,
    ) -> DocLatticeDocExport:
        """
    ...
    def _dispatch_sequential(
        ...
        **all_kwargs,
        ...
        """
        results: list[DocLatticeDocExport] = []
    ...
    def _dispatch_concurrent(
        ...
        **all_kwargs,
        ...
        """
        results: list[Optional[DocLatticeDocExport]] = [None] * len(chunk_data)
        ...
        return cast(list[DocLatticeDocExport], results)
...
def _reassemble_chunk_results(
    chunk_results: list[DocLatticeDocExport],
    page_offsets: list[int],
) -> DocLatticeDocExport:
    """
    Merge a list of per-chunk ``DocLatticeDocExport`` dicts into one.
    ...
    * Concatenates text content
    ...
    Returns:
        A single ``DocLatticeDocExport`` with globally-correct page indices.
    """
    ...
    combined_annotations: list[DocLatticeAnnotationPythonType] = []
    combined_relationships: list[DocLatticeRelationshipPythonType] = []
...
def _offset_relationship(
    relationship: DocLatticeRelationshipPythonType,
    id_prefix: str,
) -> None:
