...
Model Context Protocol server providing read-only access to public DocLattice resources.
...
if TYPE_CHECKING:
    from doclatticeserver.users.types import UserOrAnonymous
...
def _build_www_authenticate_header(scope: MutableMapping[str, Any]) -> bytes:
    ...
    *type* but will not auto-discover a login flow.
    """
    realm = b'Bearer realm="doclattice"'
    ...
    return (f'Bearer realm="doclattice", resource_metadata="{metadata_url}"').encode(
        "ascii", errors="ignore"
    )
...
def create_scoped_mcp_server(corpus_slug: str) -> Server:
    ...
    """
    scoped_server = Server(f"doclattice-corpus-{corpus_slug}")
