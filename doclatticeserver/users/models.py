...
from doclatticeserver.constants.users import (
...
from doclatticeserver.users.handle_generator import generate_handle
...
class User(AbstractUser):
    """Default user for DocLatticeServer."""
    ...
    # DocLattice is going to be deployed publicly on a shoestring budget initially.
...
class SystemStats(django.db.models.Model):
    ...
    ``doclatticeserver.tasks.stats_tasks.refresh_system_stats`` so headline
    ...
    def compute_values() -> dict[str, int]:
        ...
        (``doclatticeserver.tasks.telemetry_tasks.send_usage_heartbeat``) so
