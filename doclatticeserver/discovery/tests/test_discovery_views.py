...
class LlmsTxtTest(TestCase):
    ...
    def test_follows_llmstxt_spec_format(self):
        ...
        response = self.client.get("/llms.txt")
        content = response.content.decode()
        # H1 title — rebranded from DocLattice to cite in v3 release line
...
class WellKnownOAuthProtectedResourceTest(TestCase):
    ...
    def test_authed_path_is_advertised_as_protected_resource(self):
        ...
        from doclatticeserver.discovery.views import _OAUTH_PROTECTED_RESOURCES
        from doclatticeserver.mcp.server import MCP_AUTHED_PATH
