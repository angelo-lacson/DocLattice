#!/usr/bin/env python3
"""
oc_remote_ingest.py — run the DocLattice ingestion pipeline on a remote host
and stream FAITHFUL, fully-processed documents into a target DocLattice
...
* Structural set + thumbnail: materialised server-side by the worker-upload
  ingestion path (see doclatticeserver/worker_uploads/tasks.py).
...
This script runs INSIDE the DocLattice image (it imports the real parser),
...
class _Parser:
    ...
    def _ensure(self) -> None:
        ...
        with self._lock:
            if self._parser is not None:
                return
            # Ensure the DocLattice repo root is importable. When this file is
            # run directly (``python .../oc_remote_ingest.py``) sys.path[0] is the
            # script's own directory, so ``config`` / ``doclatticeserver`` are
            ...
            from doclatticeserver.pipeline.parsers.docling_parser_rest import (
                DoclingParser,
            )
    ...
    def ensure_ready(self) -> None:
        ...
        and to make ``config`` / ``doclatticeserver`` importable before
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        ...
        description="Remote parse + worker-upload driver for DocLattice.",
    )
    p.add_argument(
