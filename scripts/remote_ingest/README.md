...
Run the DocLattice ingestion pipeline (Docling parse + embeddings) on a
beefy **off-cluster** host and stream **fully-processed, faithfully-mirrored**
documents into a target DocLattice corpus — without giving the remote host
...
DocLattice has two metadata mechanisms: the freeform `Document.custom_meta`
...
- `ctx.export` / `ctx.content` — the parsed `DocLatticeDocExport` + text layer.
...
2. `DoclingParser.parse_pdf_bytes(bytes)` → `DocLatticeDocExport` (PAWLs,
...
- The DocLattice repo (the worker image is built from it).
- Outbound HTTPS to the target.
- Enough RAM for the Docling microservice (~2 GB) plus the worker.

The driver itself runs inside the DocLattice image and uses
