- **Authority packs can now ship their own source provider.** The pipeline
  ...
  `doclatticeserver/enrichment/data/authority_packs/` and for out-of-tree pack
  ...
  DocLattice install brings the provider with it — no more dropping a `.py` into
  core's `pipeline/authority_source_providers/` package
  (`doclatticeserver/pipeline/registry.py`, `config/settings/base.py`). Provider
