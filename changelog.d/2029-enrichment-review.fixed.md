- **Closed a duplicate-notification race in the analysis-status signal
  (`doclatticeserver/notifications/signals.py`, issue #2029 — PR #2008
  ...
  `doclatticeserver/notifications/migrations/0007_notification_analysis_idempotency_constraint.py`.
...
- **Forwarded `request` to the corpus UPDATE check in
  ...
  (`doclatticeserver/analyzer/services/analysis_lifecycle_service.py`).** The
