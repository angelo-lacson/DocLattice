- **LLM provider credential follow-ups (PR #1897 review, issue #1917).** Code-quality
  ...
  - Added a public, documented `PipelineSettings.clear_cache()` classmethod
    (`doclatticeserver/documents/models.py`) as the canonical way to drop the
    ...
    underscore method.
  - `doclatticeserver/tests/test_llm_model_factory.py`: assert the credentialed
  ...
  - The correctness/security items from the same review — narrowing the
    ...
    comment — were already addressed in `doclatticeserver/llms/model_factory.py`
