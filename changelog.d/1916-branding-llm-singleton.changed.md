- **Corpus auto-branding now resolves LLM config from the `PipelineSettings`
  singleton.** The README agent (`doclatticeserver/corpuses/services/branding.py`)
  ...
  (`doclatticeserver/utils/image_generation.py`) now reads the OpenAI provider's
...
- The branding README agent now reuses the canonical CAML authoring guide instead
  ...
  from `template_seeds.py` into a shared `doclatticeserver/corpuses/caml_authoring.py`
