- **Chunked ingest: fix chunk-scratch storage leak on the in-process fallback.**
  ...
  (`doclatticeserver/tasks/doc_tasks.py`) skips the chord and parses in-process, the
  ...
  `doclatticeserver/tests/test_chunk_tasks.py::TestChunkTasks::test_ingest_doc_large_pdf_falls_back_when_chunk_count_exceeds_limit`.
- **Chunked ingest: don't let a cleanup-storage exception strand a document in
  ...
  (`doclatticeserver/pipeline/chunk_artifacts.py`); cloud storage backends
  ...
  (`doclatticeserver/tasks/doc_tasks.py`), Celery's outer `except Exception`
