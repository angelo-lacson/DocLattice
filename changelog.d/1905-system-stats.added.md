- **Materialised install-wide statistics (`SystemStats`).** A new singleton
  model (`doclatticeserver/users/models.py`) holds pre-computed headline
  ...
  (`doclatticeserver/tasks/stats_tasks.py`;
  `SYSTEM_STATS_REFRESH_INTERVAL_SECONDS` in
  `doclatticeserver/constants/stats.py`). Surfaced via the GraphQL
