- **Graduated the `pipeline & parsers` test domain out of the mypy baseline
  (#1738).** Removed the 14 `[mypy-doclatticeserver.tests.test_*]`
...
- **Fixed a latent typing gap in `get_components_by_mimetype`
  (`doclatticeserver/pipeline/utils.py`).** The signature declared
  ...
  `doclatticeserver/tasks/doc_tasks.py`. The full project surface
  (`mypy --config-file mypy.ini doclatticeserver config`) stays clean under
