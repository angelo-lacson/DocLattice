- **Auto-generate a corpus logo and `Readme.CAML` article on creation.** When a
  ...
  (`doclatticeserver/corpuses/signals.py`) dispatches a best-effort branding
  task (`generate_corpus_branding` in `doclatticeserver/tasks/corpus_tasks.py`).
  The orchestrator (`doclatticeserver/corpuses/services/branding.py`) runs a
  ...
  (`doclatticeserver/utils/image_generation.py`) with a deterministic PIL
