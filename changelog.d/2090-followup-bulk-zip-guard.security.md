- **Bound memory use on the remaining unguarded ZIP-member reads flagged in PR #2090 review.**
  ...
  unbounded:
  - `process_documents_zip` (bulk document ZIP upload, `doclatticeserver/tasks/import_tasks.py`)
    ...
    `read_zip_member_bounded()` helper (`doclatticeserver/utils/zip_security.py`), bounded by
