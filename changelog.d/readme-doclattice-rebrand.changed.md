- **Reverted the v3 product name from `cite` back to DocLattice and
  ...
  `DocLattice` reclaims that equity and is consistent with the unchanged
  `doclatticeserver` backend module. `README.md` now leads builder-first —
  ...
  (`doclatticeserver/llms/api.py`), the MCP server (`/mcp/`), structured
