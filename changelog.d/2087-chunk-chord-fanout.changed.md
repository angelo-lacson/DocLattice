- **Chunked ingest: cap chord fan-out to bound worker-pool pressure.** `ingest_doc`
  (`doclatticeserver/tasks/doc_tasks.py`) previously dispatched an unbounded Celery
  ...
  (`DEFAULT_MAX_CHORD_TASKS = 10` in `doclatticeserver/constants/document_processing.py`,
