- **Cap user/LLM-triggered `crawl_authorities` runs at the service layer.**
  `CrawlAuthoritiesService.crawl` (`doclatticeserver/enrichment/services/crawl_authorities_service.py`)
  now sanitizes its own bounds, so both the LLM-tool path
  (`doclatticeserver/llms/tools/core_tools/corpus_references.py`) and the Celery-task path are
  ...
  The numeric clamp helper moved to a reusable `doclatticeserver/utils/numbers.py::clamp_int`
  ...
  is gone.
  Tests: `doclatticeserver/tests/test_crawl_authorities_security.py` (incl. a Celery-task-path
  clamp regression), `doclatticeserver/tests/test_numbers_utils.py`.
  Review follow-up: the Celery task's `input_schema` (`doclatticeserver/tasks/corpus_analysis_tasks.py`)
