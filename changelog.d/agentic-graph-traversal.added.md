- **Agentic reference-graph traversal ("contracts-as-codebase").** Added four
  ...
  `doclatticeserver/llms/tools/core_tools/graph_navigation.py`; registered in
  `doclatticeserver/llms/tools/tool_registry.py` (new `ToolCategory.GRAPH`)
  and wired into both agent factories in
  `doclatticeserver/llms/agents/pydantic_ai_agents.py`. The tools route
  ...
  nudge. Caps live in `doclatticeserver/enrichment/constants.py` (`NAV_*`).
- **Traversal A/B benchmark harness.** Added
  `doclatticeserver/benchmarks/traversal_benchmark.py` and the
  ...
  `doclatticeserver/benchmarks/fixtures/traversal_questions.yaml`; run steps
