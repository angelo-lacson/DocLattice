- `config/graphql/core/relay.py` / `config/graphql/corpus_types.py`: fixed a
  ...
  (`doclatticeserver/tests/test_doc_annotations_prefetch_n_plus_one.py::test_corpus_tree_cte_does_not_scale_with_document_count`
  ...
  - Root cause: `resolve_visible_fk` (FK traversal) and `get_node_from_global_id`
    ...
    permissions change mid-test (`doclatticeserver/tests/permissioning/test_permissioning.py`
