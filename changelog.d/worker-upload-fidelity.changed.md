- **Worker-upload ingestion now produces a faithful mirror of the parser
  ...
  `doclatticeserver/worker_uploads/tasks.py::_process_single_upload` so a
  ...
  - **Structural annotation set materialisation.** Previously the parser's
    ...
    `doclatticeserver/utils/structural_sets.py::create_structural_annotation_set`
  ...
  - **Embedding ownership.** `import_annotations` gained a
    ...
    (`doclatticeserver/utils/importing.py`). When a worker upload supplies
