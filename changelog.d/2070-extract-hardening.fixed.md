- **Extraction: stop reading oversized documents just to discard them.**
  `doc_extract_query_task` (`doclatticeserver/tasks/data_extract_tasks.py`) read the entire
  ...
  `doclatticeserver/constants/extraction.py`) — the byte size above which the text cannot
...
- **Extraction: report the request budget actually in force.** `_classify_none_result` /
  `_failure_message_for_classification` (`doclatticeserver/tasks/data_extract_tasks.py`) no
  ...
  `doclatticeserver/llms/agents/pydantic_ai_agents.py::_structured_response_raw` now logs the
  resolved `UsageLimits.request_limit` rather than the hardcoded default. Tests:
  `doclatticeserver/tests/test_data_extract_failure_classification.py`,
  `doclatticeserver/tests/test_pydantic_ai_agents.py`.
- **Extraction: distinguish an empty extraction artifact from an oversized one.**
  ...
  `doclatticeserver/tests/test_extract_prompt_wiring.py::test_empty_extract_file_logs_and_skips_injection`.
