- **Closed a CGNAT SSRF gap in the authority-source fetch guard (issue #2026).**
  `doclatticeserver/utils/safe_http.py::_assert_public_ip` rejected resolved
  ...
  `doclatticeserver/constants/safe_http.py::CGNAT_SHARED_ADDRESS_SPACE_CIDR`.
  ...
  `doclatticeserver/tests/test_safe_http.py` rejects the CGNAT block and
