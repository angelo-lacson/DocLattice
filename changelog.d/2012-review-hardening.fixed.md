...
- The concurrent enrichment orchestrator (`EnrichmentService._aresolve_documents`,
  `doclatticeserver/enrichment/services/enrichment_service.py`) now isolates
  ...
  (`doclatticeserver/tests/test_enrichment_in_flight.py`).
- `upsert_equivalence` (`doclatticeserver/enrichment/services/authority_equivalence_ingest.py`)
...
- `ENRICHMENT_LLM_MAX_CONCURRENCY` / `ENRICHMENT_DOC_MAX_CONCURRENCY` overrides
  (`doclatticeserver/enrichment/constants.py`) now use `is not None` rather than
