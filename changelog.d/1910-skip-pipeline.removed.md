- **Removed the bulk-ZIP `skip_pipeline` inline-application path.** With deferred
  ...
  `import_zip_with_folder_structure` (`doclatticeserver/tasks/import_tasks.py`)
  ...
  (`doclatticeserver/utils/importing.py`) is retained — it is still used by the
  V2 corpus importer (`doclatticeserver/tasks/import_tasks_v2.py`).
