- **Authority-backfill workflow: production entry points, reactive re-link,
  ...
  - **Wanted-authorities queue** — `CorpusReferenceService.wanted_authorities(user, corpus_id=None)`
    (`doclatticeserver/enrichment/services/corpus_reference_service.py`)
  ...
  - **Bootstrap entry points** — new composite
    `bootstrap_authority_corpus(...)` (`doclatticeserver/enrichment/authorities.py`)
    ...
    (`doclatticeserver/corpuses/management/commands/bootstrap_authority.py`,
    JSON spec file, `--public`, `--no-relink`, `--corpus-id`).
  - Tests: `doclatticeserver/tests/test_enrichment_backfill.py` (service
