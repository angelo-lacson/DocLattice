- **Obligation finding cards rendered a blank heading.**
  ...
  `ObligationCard` (`doclatticeserver/enrichment/finding_cards.py`) has never
  ...
  added `CardFieldContractTests` in `doclatticeserver/tests/test_finding_cards.py`
...
- **`finalize_once` only refused a COMPLETED report**
  (`doclatticeserver/research/services/research_reports.py`). CANCELLED and
...
- **The salvage path still emitted the legacy `budget_exhausted` warning**
  (`doclatticeserver/tasks/research_tasks.py`) beside the structured
...
- **`requires_responses_api` matched model-name prefixes unanchored**
  (`doclatticeserver/llms/model_factory.py`), so a hypothetical `gpt-5.60-…`
