- **Extraction: fence the per-column constraint fields against prompt injection.**
  ...
  (`doclatticeserver/tasks/data_extract_tasks.py`) now wraps each value with
  ...
  `doclatticeserver/tests/test_extract_prompt_wiring.py` (fencing, fence-breakout escaping,
  single-notice).
