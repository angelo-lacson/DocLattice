- **Deep research audit log / findings scratchpad no longer lose a concurrent
  ...
  (`doclatticeserver/research/services/research_reports.py`) did a non-atomic
  `refresh_from_db` → append → `save()`. Every tool call now routes through
  `append_tool_call` via `_audited` (`doclatticeserver/tasks/research_tasks.py`),
...
- **Authority packs sharing a directory basename no longer collide in
  `sys.modules`.** `doclatticeserver/pipeline/registry.py` keyed each pack's
...
- **A publisher-sidecar storage error no longer aborts an entire pack import.**
  `_attach_publisher_original_file` at `doclatticeserver/tasks/import_tasks_v2.py:540`
