- **Inline citations now render on PDF documents.** The enrichment writer
  (`doclatticeserver/enrichment/writer.py`) projected nothing visible on
...
- **Shared span→token projection utility.** The format-aware document-text
  ...
  `doclatticeserver/utils/extraction_grounding.py` to
  `doclatticeserver/utils/span_projection.py`
