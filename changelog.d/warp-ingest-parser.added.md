- **Warp-Ingest PDF parser (REST microservice).** Added an alternative PDF
  ...
  that renders straight to the DocLattice export format.
  - New parser `doclatticeserver/pipeline/parsers/warp_ingest_parser.py::WarpIngestParser`
  ...
  - Settings wired in `config/settings/base.py` (`WARP_INGEST_PARSER_SERVICE_URL`,
    ...
    `doclatticeserver/constants/document_processing.py`; documented in the
  ...
  - Docs: `docs/pipelines/warp_ingest_parser.md` (linked from the pipeline
    ...
    `doclatticeserver/tests/test_doc_parser_warp_ingest.py`.
