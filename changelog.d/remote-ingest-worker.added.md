- **Remote-ingest worker: offload parsing + enrichment to off-cluster hosts.**
  ...
  - `doclatticeserver/worker_uploads/management/commands/mint_worker_token.py`
  ...
  - `BaseChunkedParser.parse_pdf_bytes()`
    (`doclatticeserver/pipeline/base/chunked_parser.py`) — a new database-free
    entry point that turns raw PDF bytes into an `DocLatticeDocExport` (chunk
