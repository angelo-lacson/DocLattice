- **Authority discovery providers (Phase 2 of the Authority Packs proposal, closes #2054).**
  ...
  (`doclatticeserver/pipeline/base/base_authority_discovery_provider.py`): unlike
...
- Added `AuthorityFrontierService.seed_from_discovery`
  (`doclatticeserver/enrichment/services/authority_frontier_service.py`): the
...
- Extended the pipeline registry (`doclatticeserver/pipeline/registry.py`) to
...
- Added the `discover_authority_candidates` management command
  (`doclatticeserver/annotations/management/commands/discover_authority_candidates.py`)
