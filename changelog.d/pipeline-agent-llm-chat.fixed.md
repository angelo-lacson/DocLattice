- **Interactive chat now honors `AgentConfiguration.preferred_llm` (the
  ...
  model — diverging from the Celery (`doclatticeserver/tasks/agent_tasks.py`)
  and delegation (`doclatticeserver/llms/tools/delegation_tools.py`) paths,
  ...
  `doclatticeserver/tests/websocket/test_unified_agent_consumer.py`.
