- **Memoized the per-agent-build LLM credential read (#1921).**
  `doclatticeserver/llms/model_factory.py::_get_db_credentials` ran on every
  ...
  (`doclatticeserver/pipeline/utils.py`). Repeat builds skip the decryption,
  ...
  `doclatticeserver/tests/test_llm_model_factory.py::TestCredentialCache`.
