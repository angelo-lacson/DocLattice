- **Deferred annotation import: re-anchor producer annotations onto the final PAWLs layer.**
  ...
  (`doclatticeserver/documents/models.py`, migration `documents/0041`) and
  re-anchored after ingest.
  - `remap_pending_annotations` (`doclatticeserver/tasks/doc_tasks.py`) now
    runs as a standard step in the post_save ingest chain
    (`extract_thumbnail → ingest_doc → remap_pending_annotations →
    set_doc_lock_state`, `doclatticeserver/documents/signals.py`) and bails in
  ...
  - `anchor_annotations` (`doclatticeserver/utils/annotation_anchoring.py`)
  ...
  - A standalone validator `validate_dumb_anchor_sidecar`
    (`doclatticeserver/utils/validate_export.py`) checks a producer's sidecar
    ...
    import** (`doclatticeserver/tasks/import_tasks.py`) — when the producer
    ...
    union) were extracted to `doclatticeserver/utils/pdf_token_matching.py`
