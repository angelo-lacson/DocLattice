- **Extraction: restore dropped `Column` constraint fields.** `Column.instructions`,
  ...
  (`doclatticeserver/tasks/data_extract_tasks.py`) now folds all three back into the prompt
  the agent runs, and the dead `get_column_extraction_params` helper (zero callers) was
  removed. Regression test: `doclatticeserver/tests/test_extract_prompt_wiring.py`.
