- **Bulk import: annotation-to-annotation relationships, richer annotation metadata, and cross-batch document relationships.**
  ...
  - **Annotation-to-annotation relationships in sidecars.** A dumb-anchor
    ...
    (`doclatticeserver/tasks/import_tasks.py`) persists them verbatim onto the
    ...
    (`doclatticeserver/tasks/doc_tasks.py`, `_wire_pending_relationships`)
  ...
  - **`link_url` + `data` survive the re-anchor.** `anchor_annotations`
    (`doclatticeserver/utils/annotation_anchoring.py`) now carries the OC_URL
    ...
    (`doclatticeserver/utils/importing.py`) persists `data` onto
    `Annotation.data` (it already handled `link_url`). `DocLatticeAnnotationPythonType`
  ...
  - **Validation.** `validate_dumb_anchor_sidecar`
    (`doclatticeserver/utils/validate_export.py`) now validates the
