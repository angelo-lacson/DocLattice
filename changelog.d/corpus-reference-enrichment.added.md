- **Corpus reference enrichment agent.** A corpus-scoped agent tool pair that
  ...
  - New deterministic engine in `doclatticeserver/enrichment/`
  ...
  - New `CorpusReference` model (`doclatticeserver/annotations/models.py`,
  ...
  - Two agent tools (`doclatticeserver/llms/tools/core_tools/corpus_references.py`,
...
- **Authority corpora + cross-corpus law linking.**
  - `doclatticeserver/enrichment/authorities.py`: `AuthorityCorpusBootstrapper`
...
- **Analyzer framework: corpus-scoped analyzers.**
  ...
  - Enrichment adapter task
    (`doclatticeserver/tasks/corpus_analysis_tasks.py`) registers the engine
