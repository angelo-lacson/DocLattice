- **Deep-research durable context management: living plan + agent memory + crash recovery.**
  ...
  - **Memory store** — `ResearchReport.memory` (new JSON field). `write_memory`
    ...
    (`doclatticeserver/research/constants.py`), surfaced to the model as
  ...
  - Files: `doclatticeserver/research/models.py` (+migrations
    ...
    `doclatticeserver/tasks/research_tasks.py`, `config/settings/base.py`
    (beat entry). Tests:
    `doclatticeserver/tests/research/test_research_memory.py`.
