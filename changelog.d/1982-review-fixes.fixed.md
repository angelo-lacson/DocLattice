- **Intelligence setup: large corpora no longer silently skip enrichment.**
  ...
  (`doclatticeserver/corpuses/services/corpus_actions.py`) — the trusted-caller
...
- **Intelligence-setup status no longer demands deployment-unavailable pieces.**
  ...
  (`doclatticeserver/corpuses/services/intelligence_setup.py`) excludes the
...
- **In-flight weave reported as started.** When an enrichment analysis is
  ...
  (`doclatticeserver/corpuses/services/intelligence_setup.py`) no longer leaves
