- **De-joined `Annotation.visible_to_user` — `EXISTS` subqueries, no more
  ...
  (`doclatticeserver/shared/QuerySets.py`) previously reached structural-set
  ...
  (`doclatticeserver/tests/permissioning/test_authorization_invariants.py`)
  and by new de-join regression tests
  (`doclatticeserver/tests/test_annotation_visibility_exists.py`).
- **New composite index `annot_structural_modified_idx` on
  `Annotation(structural, modified)`** (migration
  `doclatticeserver/annotations/migrations/0077_annotation_structural_modified_index.py`,
