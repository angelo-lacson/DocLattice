- **Post-merge review follow-ups for the bulk-trash primitive (#2039, follow-up
  ...
  (`doclatticeserver/corpuses/services/lifecycle.py`) now initialises
  ...
  follow-up, with #1951 retained as the query-count fix it actually was. The
  `CorpusPathService` docstring
  (`doclatticeserver/corpuses/services/paths.py`) is corrected to describe its
  mixed convention accurately — it previously claimed *all* methods were
  underscore-prefixed, which the public `disambiguate_path` /
  `reconcile_paths_after_folder_change` contradict — and records that the
  ...
  deletes at release). `test_query_count_independent_of_document_count` gains a
  `msg=` on its query-count floor so a future failure is self-documenting.
