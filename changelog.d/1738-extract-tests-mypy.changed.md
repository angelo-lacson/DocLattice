- **Graduated the `extract` test domain out of the mypy baseline (#1738).**
  Removed the 6 `[mypy-doclatticeserver.tests.test_*]` `ignore_errors` blocks
  for the structured-data-extraction feature — Extracts plus their
  Fieldsets/Columns, Datacells, and metadata columns (all in
  `doclatticeserver/extracts/models.py`): `test_extract_mutations`,
  ...
  surface (`mypy --config-file mypy.ini doclatticeserver config`) stays clean
