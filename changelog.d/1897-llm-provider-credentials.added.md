- **Live, DB-configurable LLM provider credentials & endpoints.**
  LLM providers (`doclatticeserver/pipeline/llm_providers/*.py`) are now
  ...
  helpers on `BaseLLMProvider` (`doclatticeserver/pipeline/base/llm_provider.py`).
  ...
  - New resolver `doclatticeserver/llms/model_factory.py`
  ...
  - Wired into all five `make_pydantic_ai_agent` call sites — document, corpus
    and structured-output agents (`doclatticeserver/llms/agents/pydantic_ai_agents.py`)
    plus the memory-curation tasks (`doclatticeserver/tasks/memory_tasks.py`).
  ...
  - `_construct_model` handles only `google-gla` (AI-Studio, API-key auth);
    ...
    `google-gla`.
  - Tests: `doclatticeserver/tests/test_llm_model_factory.py` covers provider
