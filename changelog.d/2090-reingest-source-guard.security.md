- **Bound memory use when reingesting corpus-export ZIP members — on both the reingest and baked
  ...
  `_import_document_with_annotations` (`doclatticeserver/tasks/import_tasks_v2.py`) previously did
  ...
  `DEFAULT_MAX_CORPUS_REINGEST_SOURCE_BYTES` in `doclatticeserver/constants/document_processing.py`);
...
- **`MAX_CORPUS_REINGEST_SOURCE_BYTES` disable is now a negative sentinel, not `0`.** Previously the
  ...
  `doclatticeserver/tests/test_import_v2_reingest_remap.py::TestSourceReingestability`.
