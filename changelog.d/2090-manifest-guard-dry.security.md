- **Bound the last unguarded ZIP-member read in the V2 importer: the `data.json` manifest.**
  `import_corpus_v2_from_bytes` (`doclatticeserver/tasks/import_tasks_v2.py`) read the manifest —
  ...
  `DEFAULT_MAX_CORPUS_MANIFEST_SIZE_BYTES` in `doclatticeserver/constants/document_processing.py`;
...
- **Consolidated the three hand-rolled bounded-read implementations onto one.** `read_zip_member_bounded()`
  (`doclatticeserver/utils/zip_security.py`) now accepts `max_bytes: int | None`, with `None` as the
