- Add `CrawlAuthoritiesService.crawl()` BFS engine
  (`doclatticeserver/enrichment/services/crawl_authorities_service.py`):
...
- Add `crawl_authorities` Celery task (`@corpus_analyzer_task`) in
  `doclatticeserver/tasks/corpus_analysis_tasks.py`: wraps
...
- Add `CrawlAuthoritiesService` to the enrichment services `__init__.py`
  (`doclatticeserver/enrichment/services/__init__.py`).
- Add comprehensive test coverage in
  `doclatticeserver/tests/test_crawl_authorities.py`: idempotency
