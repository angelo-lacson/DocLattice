- **Corpus file-tool review polish (PR #1940 follow-up, issue #1943 items 3–5).**
  - **`require_user` helper (`doclatticeserver/llms/tools/core_tools/_helpers.py`).**
    ...
    access) (item 4).
  - **`delete_document` tool description (`doclatticeserver/llms/tools/tool_registry.py`).**
  ...
  - **Direct unit test for `sanitize_corpus_filename`**
    (`doclatticeserver/tests/test_shared_utils.py`). The canonical
