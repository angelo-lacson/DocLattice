- **Authority discovery: municipal citation grammar (Tier-2a, issue #1995).**
  ...
  - **Known municipal codes** — a new `MUNICIPAL_CODE_ABBREVIATIONS` table
    (`doclatticeserver/enrichment/abbreviations.py`) maps exact code names
  ...
  - **Open-vocabulary shape grammar** — `GenericCitationExtractor` now matches
    ...
    (`doclatticeserver/enrichment/grammars.py`), keyed under the SAME
  ...
  - `classify_prefix` (`doclatticeserver/enrichment/constants.py`) recognises
    ...
    `doclatticeserver/tests/test_generic_grammars.py`,
