- **Deleted the dead `THUMBNAIL_TASKS` dict** (`config/settings/base.py`) —
  ...
  paths (`doclatticeserver.tasks.doc_tasks.extract_pdf_thumbnail`/
...
- **Corrected `PipelineSettings.get_component_settings()`'s docstring** (`doclatticeserver/documents/models.py`):
  it claimed a Django-settings fallback was "handled by
  `PipelineComponentBase.get_component_settings()`", but that method
  (`doclatticeserver/pipeline/base/base_component.py`) implements no such
