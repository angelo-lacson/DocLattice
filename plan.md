...
|---|-------|-----------|---------------------|
| 1 | `GET_CORPUS_WITH_HISTORY` | CorpusLandingView, CorpusDetailsView | `DocLatticeNode.get_node_from_global_id()` → `Corpus.objects.visible_to_user(user).get(id=pk)` |
...
**File**: `doclatticeserver/documents/query_optimizer.py:340-406`
**Affects**: Query 2
...
**File**: `doclatticeserver/documents/query_optimizer.py:481-487`
**Affects**: Query 2
...
**File**: `doclatticeserver/shared/Managers.py:133-175`
**Affects**: Query 3
...
| 2 | DocumentRelationship nested `select_related` for creator FKs | `doclatticeserver/documents/query_optimizer.py` | Very Low | High — eliminates 3N queries |
...
|---|-----|-------|------|--------|
| 5 | DocumentRelationship optimizer uses subqueries instead of materialized Python sets | `doclatticeserver/documents/query_optimizer.py` | Low | High — O(N) → O(1) memory, fewer DB round-trips |
| 6 | Pre-compute DocumentRelationship permissions to avoid silent errors | `doclatticeserver/documents/query_optimizer.py` + `config/graphql/permissioning/permission_annotator/mixins.py` | Low | Medium — eliminates N exceptions per response |

### Phase 3: Higher-Risk Optimization (1 change)

| # | Fix | Files | Risk | Impact |
|---|-----|-------|------|--------|
| 7 | Slim down Document `visible_to_user()` prefetch for lightweight queries | `doclatticeserver/shared/Managers.py` | Medium | High — saves ~7 queries on every TOC request |
