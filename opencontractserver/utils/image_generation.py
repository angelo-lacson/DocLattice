...
Images API (configured via ``OPENAI_API_KEY`` + ``CORPUS_LOGO_GENERATION_ENABLED``)
and falls back to a deterministic PIL "monogram" — initials on a stable colored
background — whenever image generation is disabled, unconfigured, or errors.
...
async def agenerate_logo_image(
    *,
    ...
    if enabled and api_key:
        try:
            return await _generate_ai_logo(prompt, api_key)
        except Exception:
...
async def _generate_ai_logo(prompt: str, api_key: str) -> tuple[bytes, str]:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        resp = await client.post(OPENAI_IMAGE_ENDPOINT, headers=headers, json=payload)
